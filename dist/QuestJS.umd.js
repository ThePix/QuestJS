(function (global, factory) {
  ...
  (global = global || self, factory(global.QuestJS = global.QuestJS || {}));
  ...
  function Cmd (name, hash) {
    ...
    this.script = function (objects, matches) {
      ...
      for (let i = 0; i < objects[0].length; i++) {
        if (!objects[0][i][this.attName]) {
          ...
          if (result === world.SUPPRESS_ENDTURN) {
    ...
    this.processCommand = function (char, item, multi, match, verb) {
      ...
      if (typeof result !== 'boolean' && result !== world.SUPPRESS_ENDTURN) {
  ...
  //  return item.isAtLoc(options.npc.name, world.PARSER) && item.getWorn() && !item.ensemble;
  // }
  ...
  random$1.chance = function (percentile) {
    return random$1.int(99) < percentile
  };
  ...
  random$1.fromArray = function (arr, deleteEntry) {
      ...
      if (/^\d+$/.test(dice)) {
        ...
        if (parts.length === 2 && /^\d+$/.test(parts[0]) && /^[0-9\:]+$/.test(parts[1])) {
          ...
          for (let i = 0; i < number; i++) {
            if (/^\d+$/.test(parts[1])) {
              total += negative * random$1.int(1, parseInt(parts[1]));
            } else {
              total += negative * parseInt(random$1.fromArray(parts[1].split(':')));
            }
          }
        } else {
  ...
  const DEFAULT_ROOM = {
    ...
    getRandomExit: options => random.fromArray(undefined.getExits(options)),
  ...
  const world = {
    ...
    setRoom: function (char, roomName, dir) {
      ...
      if (settings.clearScreenOnRoomEnter) clearScreen$1();
  ...
  */
  ...
  function picture$1 (filename, width, height) {
  ...
  io.unscamblePick = function () {
    const c = String.fromCharCode(random.int(33, 125));
    return c === '<' ? '~' : c
  };
  ...
  exports.clearScreen = clearScreen$1;
  ...
  exports.msg = msg;
  exports.msgBlankLine = msgBlankLine;

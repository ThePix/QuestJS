...
tp.handleIf = function(arr, params, reverse) {
  ...
  if (attValue === undefined || typeof attValue === "boolean") {
    flag = attValue
...
tp.text_processors.ifHere = function(arr, params) { return tp.handleIfHere(arr, params, false) }
tp.text_processors.ifNotHere = function(arr, params) { return tp.handleIfHere(arr, params, true) }
...
tp.findSubject = function(arr, params) {
  let subject
  if (params[arr[0]]) {
    // The subject is in the paramters
    ...
    subject = w[arr[0]]
    // The subject is a named object
    //log(subject)
    if (subject === undefined) {
      const s = "In text processor findSubject, could not find a key called `" + arr[0] + "` in the parameters for the string `" + params.tpOriginalString + "`"
      console.error(s)
...
tp.nameFunction = function(arr, params, isPossessive) {
  ...
  if (arr[1] === 'count') options.article = COUNT
  if (params[subject.name + '_count']) options[subject.name + '_count'] = params[subject.name + '_count']
  if (params[arr[0] + '_count']) options[subject.name + '_count'] = params[arr[0] + '_count']
  let n = 3
  while (arr[n]) {
...
tp.conjugations = function(func, arr, params) {
  const subject = tp.findSubject(arr, params)
  if (!subject) return false
  const options = {capitalise:arr[2] === 'true'}
  let n = 3
  while (arr[n]) {
    options[arr[n]] = params[arr[n]]
    n++
  }
  return func(subject, arr[1], options)
}

...
tp.text_processors['!'] = function(arr, params) { return ""; };
...
tp._findObject = function(name, params, arr) {
  ...
  if (name === "currentLocation" || name === '@') return currentLocation
  if (name === "settings") return settings
  if (name === "params" || name === '*') return params
  if (w[name]) return w[name]
  const ary = name.split('.')
  if (ary.length === 1) return params
  if (ary.length > 2) {
    console.log(ary)
    return errormsg("The text process cannot handle attributes of attributes, so failed to deal with: " + name)
  }
  arr.unshift(ary[1])
  return w[ary[0]]
}
...
tp.text_processors.show = function(arr, params) {
  ...
  return tp.handleShow(arr, name, params, {})
}

tp.text_processors.showOrNot = function(arr, params) {
  const name = arr.shift()
  const def = arr.shift()
  return tp.handleShow(arr, name, params, {default:def})
}

tp.text_processors.decimal = function(arr, params) {
  const name = arr.shift()
  const places = arr.shift()
  return tp.handleShow(arr, name, params, {default:0, places:places})
}

tp.text_processors.hex = function(arr, params) {
  const name = arr.shift()
  return tp.handleShow(arr, name, params, {default:0, base:16})
}

tp.text_processors.bin = function(arr, params) {
  const name = arr.shift()
  return tp.handleShow(arr, name, params, {default:0, base:2})
}

tp.handleShow = function(arr, name, params, options) {
  if (params[name] !== undefined) {
    if (typeof params[name] === 'object') return tp.getWhatever(params[name][arr[0]], params, params[name], arr, options)
    return tp.getWhatever(params[name], params, undefined, arr, options)
  }
  ...
  return tp.getWhatever(obj[arr[0]], params, obj, arr, options)
}
tp.text_processors.object = tp.text_processors.show

tp.getWhatever = function(val, params, obj, arr, options) {
  if (val === undefined || val === null) return options.default ? options.default : ''
  if (val === false) return lang.tp_false
  if (val === true) return lang.tp_true
  if (val === undefined) return ''
  if (typeof val === 'string') return val
  if (typeof val === 'number') return tp.getNumber(val, options)
  if (val && typeof val === 'object') {  // because null is an object!
    ...
    return tp.getNumber(val[arr[0]], options)
  }
  ...
  return tp.getWhatever(func(params), params, obj, arr, options)
}


tp.getNumber = function(val, options) {
  if (options.places) return val.toFixed(options.places)
  if (options.base) return val.toString(options.base)
  return val.toString()
}
...
tp.handleIf = function(arr, params, reverse) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) return errormsg("Failed to find object or parameter '" + name + "' in text processor 'if/ifNot' (" + params.tpOriginalString + ")")
...
tp.handleIfLessMoreThan = function(arr, params, moreThan, orEqual) {
  ...
  if (typeof attValue !== "number") return errormsg("Trying to use ifLess/MoreThan(OrEqual) with a non-numeric attribute, '" + name + "' (" + params.tpOriginalString + ").")
  let value = arr.shift()
  if (isNaN(value)) return errormsg("Trying to compare a numeric attribute, '" + name + "' with a string (" + params.tpOriginalString + ").")
  value = parseInt(value)
...
tp.text_processors.ifAt = function(arr, params) { return tp.handleIfAt(arr, params, false) }
tp.text_processors.ifNotAt = function(arr, params) { return tp.handleIfAt(arr, params, true) }
tp.handleIfAt = function(arr, params, reverse) {
  let name = arr.shift(), flag
  const obj = tp._findObject(name, params, arr)
  if (!obj) return errormsg("Failed to find object '" + name + "' in text processor 'if/ifNotPlayer' (" + params.tpOriginalString + ")")
  flag = obj === currentLocation
  if (reverse) flag = !flag
  return (flag ? arr[0] : (arr[1] ? arr[1] : ""))
}


tp.text_processors.tone = function(arr, params) { return tp.handleTone(arr, params, false) }
tp.text_processors.toneNot = function(arr, params) { return tp.handleTone(arr, params, true) }
tp.handleTone = function(arr, params, reverse) {
  let name = arr.shift()
  let flag = player.tone === name
  if (reverse) flag = !flag
  return (flag ? arr[0] : (arr[1] ? arr[1] : ""))
}
...
tp.nameFunction = function(arr, params, isPossessive, isList) {
  ...
  options.nothing = lang.list_nothing
...
tp.conjugations = function(func, arr, params) {
  ...
  if (!subject) return false
  if (!subject.pronouns) {
    errormsg("Hmm, trying to conjugate (or something) in TP, but " + subject.name + " has no pronouns set.")
    log(subject)
    return false
  }

...
tp.text_processors.contents = function(arr, params) {
  ...
  return formatList(obj.getContents(world.LOOK), {article:INDEFINITE, sep:arr[0], lastJoiner:arr[1], nothing:arr[2]})
}
...
tp.text_processors.hereDesc = function(arr, params) {
  let s
  const attName = game.dark ? "darkDesc" : "desc"
...
tp.nameFunction = function(arr, params, isPossessive) {
  ...
  if (arr[0] === 'the-pa') {options.article = DEFINITE; options.poss_adj = true }
  if (arr[0] === 'a') options.article = INDEFINITE
  if (arr[0] === 'a-pa') {options.article = INDEFINITE; options.poss_adj = true }
  if (arr[2] && options.poss_adj) {
    options.poss_adj = tp._findObject(arr[2], params, [])
  }
  ...
  return arr[1] === 'true' ? sentenceCase(lang.getName(subject, options)) : lang.getName(subject, options);
}
...
tp.text_processors.pa2 = function(arr, params) {
  ...
  return arr[0] === 'true' ? sentenceCase(chr1.pronouns.poss_adj) : chr1.pronouns.poss_adj;
};

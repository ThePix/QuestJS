...
tp.text_processors.dialogue = function(arr, params) {
  ...
  if (arr.length < 1) errormsg("Failed to find enough parts in text processor 'dialog' (<i>" + params.tpOriginalString + "</i>)")
  ...
  if (style.startsWith('.')) {
    ...
    if (arr.length < 2) errormsg("Failed to find enough parts in text processor 'dialog' without a class (<i>" + params.tpOriginalString + "</i>)")

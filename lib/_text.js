...
const tp = {
  ...
  setLoadString:function(s) {
    const parts = s.split("=");
    if (parts.length !== 2) errormsg("Bad format in saved data (" + s + ")")
  ...
  getSaveString:function() {
    let s = saveLoad.encodeArray(tp.usedStrings);
    return s;
  },
}
...
tp.processText = function(str, params) {
  ...
  return str.replace(/@@@colon@@@/g, ':').replace(/@@@lcurly@@@/g, '{').replace(/@@@rcurly@@@/g, '}')
};
...
tp.text_processors.select = function(arr, params) {
  ...
  const l = o[arr[0]]
  if (!l) errormsg('Failed to find an attribute called "' + arr[0] + '" for "' + name + '" in text processor select.')
  const n = o[arr[1]]
  if (!l) errormsg('Failed to find an attribute called "' + arr[1] + '" for "' + name + '" in text processor select.')
  return l[n]
};
...
tp._findObject = function(name, params, arr) {
  ...
  if (name === "here") return w[game.player.loc]
...
tp.text_processors.show = function(arr, params) {
  ...
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'show' (" + params.tpOriginalString + ")");
    return false;
  }
  name = arr.shift();
  const val = obj[name];
...
tp.text_processors.number = function(arr, params) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'number' (" + params.tpOriginalString + ")");
    return false;
  }
  ...
  errormsg("Failed to find a number for object '" + name + "' in text processor (" + params.tpOriginalString + ")");
  return false;
};
...
tp.text_processors.money = function(arr, params) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'money' (" + params.tpOriginalString + ")");
    return false;
  }
  ...
  errormsg("Failed to find a price for object '" + name + "' in text processor (" + params.tpOriginalString + ")");
  return false;
};
...
tp.text_processors.handleIf = function(arr, params, reverse) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'if' (" + params.tpOriginalString + ")");
    return false;
  }
  name = arr.shift();
...
tp.text_processors.handleIfHere = function(arr, params, reverse) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'ifHere' (" + params.tpOriginalString + ")");
    return false;
  }
...
tp.text_processors.handleIfLessMoreThan = function(arr, params, moreThan, orEqual) {
  ...
  const obj = tp._findObject(name, params, arr)
  if (!obj) {
    errormsg("Failed to find object '" + name + "' in text processor 'ifLessMoreThan' (" + params.tpOriginalString + ")");
    return false;
  }
  name = arr.shift();
  if (typeof obj[name] !== "number") {
...
tp.text_processors.exitsHere = function(arr, params) {
  const list = w[game.player.loc].getExitDirs()

...
map.init = function() {
  ...
  while (rooms.length > 0) {
    ...
    if (room.mapMultiLoc && !room.mapDraw) room.mapDraw = map.multiLocDraw
...
map.update = function() {
  ...
  for (let key in w) {
    ...
    if (room.mapMultiLoc) {
      ...
      room.mapDraw(region, level, mapBase, mapOtherLevels)
    }  
    else if (room.mapDrawString) {
      destinationLayer.push(room.mapDrawString)
    }
    else if (room.mapDraw) {
      const s = room.mapDraw()
    ...
    for (let dir of lang.exit_list) {
      ...
      if (exit.mapDrawString) {
        ...
        // For mapMultiLoc, assume a straight exit

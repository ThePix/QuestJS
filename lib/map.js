...
map.update = function() {
  ...
  const mapFeatures = []
  const mapLabels = []
  mapExits.push('<g stroke="' + settings.mapExitColour + '" stroke-width="3" fill="red" id="exit-layer">')
  ...
  result.push(map.marker(w[game.player.loc].mapX, w[game.player.loc].mapY))
  
  let s = '<defs>'
  s += '<marker id="arrowbig" markerWidth="10" markerHeight="10" refX="0" refY="2" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,4 L4,2 z"  /></marker>'
  s += '<marker id="arrowsmall" markerWidth="10" markerHeight="10" refX="0" refY="1" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,2 L2,1 z" /></marker>'
  s += '</defs>'
  result.unshift(s)
...
map.quadratic = function(points, attrs) {
  let s = '<path d="M '
  s += points[0][0] + ' ' + points[0][1]
  s += ' q '
  s += points[1][0] + ' ' + points[1][1] + ' '
  s += points[2][0] + ' ' + points[2][1]

...
map.init = function() {
  ...
  while (rooms.length > 0) {
    ...
    console.log(room.mapMultiLoc)
    console.log(!room.mapDraw)
    ...
    for (let dir of lang.exit_list) {
      ...
      if (exitRoom.mapMultiLoc || room.mapMultiLoc) {
        exit.mapMultiLoc = true
        map.mapMultiRoomFromExit(room, exitRoom, exit, dir)
      }
      else {
        map.mapRoomFromExit(room, exitRoom, exit, dir, rooms)
      }
      if (exitRoom.mapMultiLoc && !exitRoom.mapDraw) {
        console.log("Mapping room: " + exitRoom.name)
        exitRoom.mapDraw = map.multiLocDraw
      }
    }
...
map.mapDraw = function(lists, region, level, room) {
  ...
  for (let dir of lang.exit_list) {
    ...
    else if (exit.mapMultiLoc) {
      // For an exit going TO a mapMultiLoc, 
...
map.multiLocDraw = function(lists, region, level, room) {
  console.log(room.name)
  for (let el of this.locations) {
    ...
    console.log('mapMultiLoc')
    // if a multi-location room, give it the special draw function
    destinationLayer.push(room.mapDrawBase ? room.mapDrawBase(region, level) : map.mapDrawDefault(room, el))
    console.log(room.transitDoorDir)
    ...
    for (let dir of lang.exit_list) {
      ...
      if (exit.mapDrawBase) {
        ...
        console.log("HERE")
        // For an exit going FROM a mapMultiLoc, 
...
/*
map.multiLocDraw = function(region, level, mapBase, mapOtherLevels) {
  console.log("multidraw")
  for (let el of this.locations) {
    if (el.mapRegion !== region) continue
    if (el.mapZ === level) {
      mapBase.push(map.mapDrawDefault(this, el))
    }
    else {
      mapOtherLevels.push(map.mapDrawDefault(this, el))
    }
  }
}
map.multiLocDraw = function(region, level) {
  console.log("multidraw label")
  let s = ''
  for (let el of this.locations) {
    if (el.mapRegion !== region || el.mapZ !== level) continue
    s += map.mapDrawLabelDefault(this, el)
  }
  return s
}*/

// loc has the coordinates, but defaults to o
// (used by multiLocDraw)

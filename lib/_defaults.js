...
const DEFAULT_OBJECT = {
  ...
  changePronouns:function(pronouns) {
    for (const key in pronouns) this['_save_pronoun_as_it_changed_' + key] = pronouns[key]
    this._save_pronouns_as_they_changed = true
  ...
  afterLoadForTemplate:function() {
    if (this._save_pronouns_as_they_changed) {
      this.pronouns = {}
      for (const key in this) {
        if (key.startsWith('_save_pronoun_as_it_changed_')) {
          const name = key.replace('_save_pronoun_as_it_changed_', '')
          this.pronouns[name] = this[key]
        }
      }
    }  
...
const DEFAULT_ROOM = {
  ...
  hasExit:function(dir, options) {
    ...
    if (game.dark && !this[dir].isLit()) return false
    if (options.includeHidden && this[dir]) return true    
...
for (const s of settings.exit_states) {
  ...
  DEFAULT_ROOM['setExit' + s] = function(dir, locked) {
    if (!this[dir]) return false
    this['exit_' + name + '_' + dir] = locked
    return true
  }
  
  DEFAULT_ROOM['isExit' + s] = function(dir) {
    return this['exit_' + name + '_' + dir]
  }
}
...
/*
  ...
  // Returns false if the exit does not exist or is not an Exit object
  // Returns true if successful
  setExitLocked:function(dir, locked) {
    if (!this[dir]) return false
  ...
  // Returns false if the exit does not exist or is not an Exit object
  // Returns true if successful
  setExitHidden:function(dir, hidden) {
    if (!this[dir]) return false

...
function updateExitLinks(){
	const exits = w[game.player.loc].getExitDirs();
	let link = $(`.exit-link`);
	if (link.length > 0){
...
findCmd('Inv').script = function() {
  if (settings.linksEnabled) {
	  msg(lang.inventory_prefix + " " + getAllChildrenLinks(game.player) + ".", {char:game.player});
  ...
  msg(lang.inventory_prefix + " " + formatList(listOfOjects, {lastJoiner:lang.list_and, modified:true, nothing:lang.list_nothing, loc:game.player.name}) + ".", {char:game.player});
  return settings.lookCountsAsTurn ? world.SUCCESS : world.SUCCESS_NO_TURNSCRIPTS;
};
...
tp.text_processors.exits = function(arr, params) {
  ...
  const list = [];
  w[game.player.loc].getExitDirs().forEach(exit => {

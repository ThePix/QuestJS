...
function formatList(itemArray, options) {
  ...
  if (options.separateEnsembles) {
    ...
    for (let i = 0; i < itemArray.length; i++) {
      if (itemArray[i].ensembleMaster && itemArray[i].ensembleMaster.isAtLoc(itemArray[i].loc)) {
        for (let j = 0; j < itemArray[i].ensembleMaster.members.length; j++) {
          toRemove.push(itemArray[i].ensembleMaster.members[j])
        }
      }
    } 
  }
...
function isHeldNotWorn(item) {
  return item.isAtLoc(game.player.name) && world.ifNotDark(item) && !item.getWorn();
}
...
function isWornBy(item, options) {
  return item.isAtLoc(options.npc.name) && item.getWorn() && !item.ensemble;
}
...
const getContents = function(includeScenery) {
  const list = [];
  const threshold = includeScenery ? DSPY_SCENERY - 1: DSPY_SCENERY
  for (let key in w) {
    if (w[key].isAtLoc(this.name) && w[key].getDisplay() > threshold) {
      list.push(w[key]);
    }
  }
  return list;
...
function getDir(s) {
  for (let i = 0; i < EXITS.length; i++) {
    ...
    if (EXITS[i].name === s) return EXITS[i].name;
    if (EXITS[i].abbrev.toLowerCase() === s) return EXITS[i].name;
    if (new RegExp("^(" + EXITS[i].alt + ")$").test(s)) return EXITS[i].name;
  }
  return false;
}

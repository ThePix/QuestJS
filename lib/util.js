...
function printOrRun(char, item, attname, options) {
  ...
  if (typeof item[attname] === "string") {
    ...
    errormsg("Unsupported type for printOrRun: " + (typeof item[attname]));
    return false;
  }
}
...
util.getNameModifierForContainer = function(options) {
  const contents = this.getContents(world.LOOK);
  if (contents.length > 0 && (!this.closed || this.transparent)) {
    return " (" + lang.contentsForData[this.contentsType].prefix + this.listContents(world.LOOK) + lang.contentsForData[this.contentsType].suffix + ")"
  }
  return ''
}

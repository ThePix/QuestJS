...
function formatList(itemArray, options) {
  ...
  if (!options.separateEnsembles) {
    ...
    for (let i = 0; i < itemArray.length; i++) {
      if (itemArray[i].ensembleMaster && itemArray[i].ensembleMaster.isAllTogether()) {
        toRemove.push(itemArray[i]);
        if (!toAdd.includes(itemArray[i].ensembleMaster)) toAdd.push(itemArray[i].ensembleMaster);
      }
    }
...
function respond(params, list, func) {
  ...
  //console.log(util.responses[i].rating)
  //console.log(rating < util.responses[i].rating)

...
//@DOC
// Returns a new array based on ary, by extracting the value of the given attribute, which defaults to "name".
// If an element has a function for the given attribute, the function is run and the return value used
//(undefined if none is returned). If an element does not have the attribute, undefined is used.
// If you have an array of objects or exit, use this to get an array of names.
array.atts = function(ary, attName = 'name') {
  return ary.map(el => (typeof el[attName] === 'function' ? el[attName]() : el[attName]))
}
...
array.clone = function(ary, options) {
  ...
  if (options.attribute) newary = newary.map(el => el[options.attribute])

...
const events = {
  ...
  runEvents:function() {
    for (var key in this.eventQueue) {
      if (this.eventQueue[key].turn <= game.turnCount && (!this.eventQueue[key].condition || this.eventQueue[key].condition())) {
        this.eventQueue[key].script();
        if (this.eventQueue[key].period) {
          this.eventQueue[key].turn += this.eventQueue[key].period;
        }
        else {
          delete this.eventQueue[key]
        }
      }
    }
  },
  add:function(name, ev) {
    ev.turn += game.turnCount;

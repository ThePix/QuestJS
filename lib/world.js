...
function createObject(name, listOfHashes) {
  ...
  if (w[name]) {
    console.log("Attempting to use the name `" + name + "`, there there is already an item with that name in the world.")
...
const world = {
      ...
      if (ex) {
        ...
        if (ex.alsoDir) {
          for (let j = 0; j < ex.alsoDir.length; j++) {
            ...
            item[ex.alsoDir[j]].getDisplay() = DSPY_SCENERY;
          }
        }
      }
    }
  ...
  ifNotScenery:function(item) {
    return (item.getDisplay() > DSPY_SCENERY); 
  },

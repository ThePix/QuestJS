...
const world = {
      ...
      if (ex) {
        ...
        if (ex.alsoDir) {
          for (let j = 0; j < ex.alsoDir.length; j++) {
            ...
            item[ex.alsoDir[j]].display = DSPY_SCENERY;
          }
        }
      }
    }
  ...
  enterRoom:function(room) {
    if (typeof room === "string") room = w[room];
    if (room.beforeEnter === undefined) {
      errormsg("This room, " + room.name + ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.");
      return;
    }
    room.beforeEnter();

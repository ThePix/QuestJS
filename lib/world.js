  ...
  this.script = function(objects) {
    ...
    for (var i = 0; i < objects[0].length; i++) {
      if (!objects[0][i][this.attName]) {
        ...
        var result = printOrRun(objects[0][i], this.attName, multi);
...
function NpcCmd(name, hash) {
  ...
  this.script = function(objects) {
    ...
    if (!npc.getAgreement()) {
      ...
      // I think possibly because the player did something in the game world,
      // so a turn has passed.
      return FAILED; 
    }
    ...
    for (var i = 0; i < objects[0].length; i++) {
      if (!objects[1][i][this.attName]) {
        this.default(objects[1][i], objects[1].length > 1 || parser.currentCommand.all);
      }
      else {
        debugmsg(0, "HJERE4: " + this.attName);
        debugmsg(0, "HJERE4: " + npc.name);
        debugmsg(0, "HJERE4: " + multi);
        var result = printOrRun(objects[1][i], this.attName, multi, npc);
        success = result || success;
      }
    }
    if (success) {

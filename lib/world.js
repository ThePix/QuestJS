...
const game = createObject("game", [{
  ...
  registerTimedEvent:function(event, triggerTime, interval) {
    this.timerEvents.push({event:event, triggerTime:triggerTime + game.elapsedRealTime, interval:interval});
  },
  gameTimer:function() {
    ...
    for (let i = 0; i < game.timerEvents.length; i++) {
      if (game.timerEvents[i].triggerTime && game.timerEvents[i].triggerTime < game.elapsedRealTime) {
        game.timerEvents[i].event();
        if (game.timerEvents[i].interval) {

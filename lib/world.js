...
function createObject(name, listOfHashes) {
  ...
  var item = {
    ...
    // This is only concerned with whether the item is at the specified location,
    // not worrying about in a container at that location, whether it is dark,
    // or anything like that. Can be set the name or the object. If sent an array
    // it will return true if the item is at any of the locations.
    isAt:function(loc, fns, options) {
      if (Array.isArray(loc)) {
        for (var i = 0; i < loc.length; i++) {
          if (this.isAt(loc[i], fns, options)) {
            return true; 
          }
        }
        return false;
        ...
        //debugmsg("Here: " + options);
        //if (options) {
        //  debugmsg("Here: " + options.container);
        //  if (options.container) debugmsg("Here: " + options.container.name);
        //}
        if (this.display < DSPY_SCENERY) { return false; }
        if (!this.testOptions(fns, options)) {
          return false;
        }
        return this.isAtLoc(loc);
      }
    },
    ...
    isReachable:function() {
      return scope([ifNotWithNpc, ifNotClosed, ifNotDark]).includes(this);
    },
    isVisible:function() {
      return scope([ifNotWithNpc, ifTransparent, ifNotDark]).includes(this);
    },
    isObvious:function() {
      return scope([ifNotWithNpc, ifTransparent, ifNotDark, ifNotScenery]).includes(this);
    },
...
var game = {
  ...
  update:function(player) {
    ...
    var listOfOjects = scope([ifTransparent]);
...
function Exit(name, hash) {
  this.name = name;
  this.use = function(exit, char, dir) {
    if (exit.isLocked()) {
      if (exit.lockedmsg) {
        msg(exit.lockedmsg);
      }

...
function createItem() {
  ...
  if (settings.itemCreateFunc) settings.itemCreateFunc(name, args)
...
function createRoom() {
  ...
  if (settings.roomCreateFunc) settings.roomCreateFunc(name, args)
  return createObject(name, args)
}
...
const world = {
  ...
  enterRoomAfterScripts:function() {
    ...
    for (let s of game.player.followers) {
      const follower = w[s]
      if (follower.loc !== game.player.loc) follower.moveWithDescription(game.room.name)
    }

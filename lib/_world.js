...
const world = {
  ...
  update:function() {
    ...
    if (!game.player.loc || !w[game.player.loc]) {
      errormsg(game.player.loc === undefined ? "No player location set." : "Player location set to '" + game.player.loc + "', which does not exist.");
    ...
    currentLocation = w[game.player.loc]
    world.scopeSnapshot()
  ...
  enterRoom:function(dir) {
    if (currentLocation.beforeEnter === undefined) {
      errormsg("This room, " + currentLocation.name + ", has no 'beforeEnter` function defined.  This is probably because it is not actually a room (it was not created with 'createRoom' and has not got the DEFAULT_ROOM template), but it an item. It is not clear what state the game will continue in.");
      return;
    }
    currentLocation.beforeEnter();
    if (currentLocation.visited === 0) { currentLocation.beforeFirstEnter(); }
    world.enterRoomAfterScripts(dir);
  },

  // Called when entering a new room, after beforeEnter and beforeFirstEnter re done
  enterRoomAfterScripts:function(dir) {
    currentLocation.description()
    player.handleMovingFollowers(currentLocation, dir) 
    currentLocation.visited++
    currentLocation.afterEnter()
    if (currentLocation.visited === 1) { currentLocation.afterFirstEnter(); }

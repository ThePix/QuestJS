...
log(w.r_prototype_bone.verbFunctions.length)
...
log(w.r_prototype_bone.verbFunctions.length)
  ...
  initItem:function(item) {
    if (item.clonePrototype) return errormsg('ERROR: Trying to initiaslise a cloned object. This is not permitted, as the prototype will not have been initialised before it was cloned. Instead, clone the object in settings.setup.')
    for (const f of item.initialiserFunctions) f(item)

...
const world = {
  ...
  initItem:function(item) {
    ...
    if (settings.playMode === 'dev' && !settings.disableChecks) {
      ...
      if (item.abstract || item.conversationQuestion) {
        // do nothing
      }
      if (item.room) {
...
function Exit(name, hash) {
  ...
  this.isHidden = function() { return this.origin.isExitHidden(this.dir) || (game.dark && !this.illuminated) }

...
const world = {
  ...
  ABORT_SUCCESS: 11,  // Used for command actions
  ABORT_FAILED: 12,  // Used for command actions
  FAILED:3,
  PARSER_FAILURE:-2,
  ...
  init:function() {
    ...
    tp.usedStrings = [] // if we test descriptions when objects are initialised, need to reset the text process
  ...
  initItem:function(item) {
    ...
    for (let exit of lang.exit_list) {
      ...
      if (ex) {
        ex.origin = item
        ex.dir = exit.name
        if (ex.locked) item.setExitLocked(ex.dir, true)
        if (ex.hidden) item.setExitHidden(ex.dir, true)
        if (ex.illuminated || ex.lit) item.setExitLit(ex.dir, true)
        if (ex.linkFlag) {
          const reverseDir = ex.reverse()
          const dest = w[ex.name]
          if (!dest) {
            log('WARNING: The destination for the Link on ' + item.name + ' goes to an unknown room: ' + ex.name)
            continue
          }
          if (dest[reverseDir]) {
        ...
        if (ex.initFunction) ex.initFunction()
        if (ex.alsoDir) {
  ...
  endTurn:function(result) {
    ...
    if (!world.suppressEndTurn && (result === world.SUCCESS || (settings.failCountsAsTurn && result === world.FAILED))) {
      // If world.suppressEndTurn is set, do not do this, instead reset world.suppressEndTurn
      ...
      world.resetPauses()
      world.update()
      delete player._hasJust
      world.saveGameState()
      endTurnUI(true)
    }
    else {
      endTurnUI(false)
      delete world.suppressEndTurn
    }
  },
...
class Exit {
  constructor(name, hash) {
    if (!hash) hash = {}
    if (typeof hash === "string") hash = {msg:hash}
    this.name = name
    ...
    for (const s of settings.exit_states) {
      this['is' + s] = function() { return this.origin['isExit' + s](this.dir) }
      this['set' + s] = function(state) { return this.origin['setExit' + s](this.dir, state) }
    }
    

    /*
    this.isLocked = function() { return this.origin.isExitLocked(this.dir); }
    this.setLock = function(locked) { return this.origin.setExitLocked(this.dir, locked); }
    this.isHidden = function() { return this.origin.isExitHidden(this.dir) }
    this.setHide = function(hidden) { return this.origin.setExitHidden(this.dir, hidden); }
    */    
...
class WayIn extends Exit {
  constructor(name, itemName, hash) {
    super(name, hash)
    this.wayInItemName = itemName
    this.initFunction = function() {
      if (!this.wayInItemName) return errormsg('The WayIn exit for ' + this.origin.name + ' requires an item name.')
      if (!w[this.wayInItemName]) return errormsg('The WayIn exit for ' + this.origin.name + ' has a bad item name (found "' + this.wayInItemName + '").')
      w[this.wayInItemName].goInDirection = this.dir
      this.alsoDir = ['in']
    }
  }  
}

class WayOut extends Exit {
  constructor(name, hash) {
    super(name, hash)
    this.initFunction = function() {
      this.alsoDir = ['out']
...
function insertItemName(str, item) {
  if (typeof item === 'string') item = w[item]
  return str.replace('#', lang.getName(item, {article:DEFINITE}))
}
...
class ClimbExit extends Exit {
  constructor(name, itemName, hash) {
    ...
    this.initFunction = function() {
      if (!this.climbItemName) return errormsg('The ClimbExit exit for ' + this.origin.name + ' requires an item name.')
      if (!w[this.climbItemName]) return errormsg('The ClimbExit exit for ' + this.origin.name + ' has a bad item name (found "' + this.climbItemName + '").')
      if (this.dir === 'up') {
        w[this.climbItemName].climbupverb = function(options) {
          return w[this.loc].up.use(options.char)
        }
        this.alsoDir = ['climb_up']
      }
      if (this.dir === 'down') {  // else will not work, as it would catch the new climb_up direction
        w[this.climbItemName].climbdownverb = function(options) {
          return w[this.loc].down.use(options.char)
        }
        this.alsoDir = ['climb_down']
      }
      if (this.dir === 'up' || this.dir === 'climb_up') {
        this.alias = 'up'
        if (!this.npcEnterMsg) this.npcEnterMsg = insertItemName(lang.npc_enter_climb_up, itemName)
        if (!this.npcLeaveMsg) this.npcLeaveMsg = insertItemName(lang.npc_leave_climb_up, itemName)
        w[this.climbItemName].goUpDirection = 'up'
      }
      if (this.dir === 'down' || this.dir === 'climb_down') {
        this.alias = 'down'
        if (!this.npcEnterMsg) this.npcEnterMsg = insertItemName(lang.npc_enter_climb_down, itemName)
        if (!this.npcLeaveMsg) this.npcLeaveMsg = insertItemName(lang.npc_leave_climb_down, itemName)
        w[this.climbItemName].goDownDirection = 'down'
      }
    }
  }  
}
...
class StairsUp extends Exit {
  constructor(name, itemName, hash) {
    super(name, hash)
    this.climbItemName = itemName
    this.initFunction = function() {
      if (!this.climbItemName) return errormsg('The StairsUp exit for ' + this.origin.name + ' requires an item name.')
      if (!w[this.climbItemName]) return errormsg('The StairsUp exit for ' + this.origin.name + ' has a bad item name (found "' + this.climbItemName + '").')
      w[this.climbItemName].goUpDirection = this.dir
      w[this.climbItemName].climbupverb = function(options) {
        return w[this.loc].up.use(options.char)
      }
      this.alsoDir = ['up', 'climb_up']
      if (!this.npcEnterMsg) this.npcEnterMsg = insertItemName(lang.npc_enter_up_stairs, itemName)
      if (!this.npcLeaveMsg) this.npcLeaveMsg = insertItemName(lang.npc_leave_up_stairs, itemName)
      w[this.climbItemName].goUpDirection = 'up'
    }
  }  
}

class StairsDown extends Exit {
  constructor(name, itemName, hash) {
    super(name, hash)
    this.climbItemName = itemName
    this.initFunction = function() {
      if (!this.climbItemName) return errormsg('The StairsDown exit for ' + this.origin.name + ' requires an item name.')
      if (!w[this.climbItemName]) return errormsg('The StairsDown exit for ' + this.origin.name + ' has a bad item name (found "' + this.climbItemName + '").')
      w[this.climbItemName].goDownDirection = this.dir
      w[this.climbItemName].climbdownverb = function(options) {
        return w[this.loc].down.use(options.char)
      }
      this.alsoDir = ['down']
      if (!this.npcEnterMsg) this.npcEnterMsg = insertItemName(lang.npc_enter_down_stairs, itemName)
      if (!this.npcLeaveMsg) this.npcLeaveMsg = insertItemName(lang.npc_leave_down_stairs, itemName)
      w[this.climbItemName].goDownDirection = 'down'
    }
  }  
}
...
class NonExit extends Exit {
  constructor(msg) {
    super('_', {noExitMsg:msg})
    this.isHidden = function() { return true }
  }
}


class BarredExit extends Exit {
  constructor(msg) {
    super('_', {msg:msg, use:util.cannotUse})
  }
}

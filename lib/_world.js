...
function cloneObject(item, loc, newName) {
  if (!world.isCreated) { return errormsg("Attempting to clone item after the game has started") }
  return _copyCloneObject(item, loc, newName, true)
}
...
function copyObject(item, loc, newName) {
  ...
  return _copyCloneObject(item, loc, newName, false)
}
  
  
function _copyCloneObject(item, loc, newName, isClone) {
  ...
  // If this is a copy - we are doing this before the game starts - we have to be careful not to duplicate name modifiers
  const copy = isClone ? {} : {...item,
  ...
  if (isClone) {
    ...
    // clones need a prototype and special string for saving
    // copies do not because they will be created every time
    if (!copy.clonePrototype) {
      copy.clonePrototype = item
    }
    copy.getSaveStringPreamble = function(item) {
...
const world = {
  ...
  initItem:function(item) {
    if (item.clonePrototype) return errormsg('Trying to initialize a cloned object. This is not permitted, as the prototype will not have been initialised before it was cloned. Instead, clone the object in settings.setup.')

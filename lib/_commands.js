...
"use strict";
  ...
  new Cmd('MetaAchievements', {
    objects:[
      {special:'text'},
    ],
    script:function(objects) {
      saveLoad.listAchievements()
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  })
...
/*  new Cmd('Smell', {
  ...
  new Cmd('Examine', {
    ...
    script:function(objects) {
      for (const obj of objects[0]) {
        const options = {item: obj, char:player}
        const add = obj.examineAddendum ? obj.examineAddendum(options) : ''
        options.add = add
        
        if (typeof obj.examine === "string") {
          msg(obj.examine + add, options)
        }
        else if (typeof obj.examine === "function"){
          const s = obj.examine(options)
          if (s) msg(s + add, options)
        }
        else {
          msg(this.defmsg, options)
        }
        if (!obj.examine_count) obj.examine_count = 0
        obj.examine_count++
        if (obj.afterExamine) obj.afterExamine(options)
      }
      return world.SUCCESS;
    },
  ...
  new Cmd('OpenWith', {
    rules:[cmdRules.testManipulate, cmdRules.isPresentOrContained],
    objects:{
      item:{scope:parser.isPresent, multiple:true, attName:"open"},
      obj:{scope:parser.isHeld, multiple:true},
    },
  ...
  new Cmd('LockWith', {
    rules:[cmdRules.testManipulate, cmdRules.isPresentOrContained],
    objects:{
      item:{scope:parser.isPresent, attName:"lock"},
      obj:{scope:parser.isHeld, attName:'key'},
    },
  ...
  new Cmd('UnlockWith', {
    rules:[cmdRules.testManipulate, cmdRules.isPresentOrContained],
    objects:{
      item:{scope:parser.isPresent, attName:"unlock"},
      obj:{scope:parser.isHeld, attName:'key'},
    },
  ...
  new Cmd('Back', {
    ...
    script:function() {
      if (!player.previousLoc) return failedmsg(lang.no_back)      
      const ex = currentLocation.findExit(player.previousLoc)
      if (!ex) return failedmsg(lang.cannot_back)
      return ex.use(player) ? world.SUCCESS : world.FAILED
    },
  })
  new Cmd('Climb', {
    objects:[
...
/*  new Cmd('ClimbUpVerb', {
    ...
    npcCmd:true,
    defmsg:lang.cannot_climb_item,
  })
  new Cmd('ClimbDownVerb', {
    objects:[
      {scope:parser.isHere},
    ],
    npcCmd:true,
  ...
  new Cmd('TasteItem', {
    ...
    attName:"taste",
    objects:[
      {scope:parser.isPresent, attName:"taste"},
    ],
    defmsg:lang.cannot_taste,
  })
  ...
  // Vari-verbs
  // These have various verbs, that may, or may not, be important
  // For example one command covers DRINK, SIP and QUAFF, and often that is fine
  // but just in case you want to differentiate in the response, the text is stored
  new Cmd('Eat', {
    npcCmd:true,
    rules:[cmdRules.isHeldNotWorn, cmdRules.testManipulate],
    objects:[
  ...
  new Cmd('OneWord', {
    ...
    script:function(objects) {
      let text = objects[0]
      if (lang.oneWordSubsts[text]) text = lang.oneWordSubsts[text]

      if (currentLocation[text]) {
        printOrRun(player, currentLocation, text);
      }
      else if (currentLocation._region && regions[currentLocation._region][text]) {
        msg(regions[currentLocation._region][text])
      }
      else {
        msg(lang['no_' + text], {char:player});
      }
      player._hasJust = text
      return world.SUCCESS;
    },
  })
  ...
  new Cmd('Tone', {
    ...
    script:function(objects) {
      log(objects[0])
      player.tone = objects[0]
      metamsg(lang.tone_is_now, {text:objects[0]})
      if (settings.afterChangeTone) settings.afterChangeTone()
      return world.SUCCESS_NO_TURNSCRIPTS
    },
  })
...
if (settings.playMode === 'dev') {
  ...
  new Cmd('DebugListCommands', {
    script:function(arr) {
      let count = 0;
      for (let cmd of commands) {
        if (!cmd.name.match(/^(Npc|Debug|Meta)/)) {
          let s = cmd.name + " (" + this.listRegexes(cmd) + ")"
          const npcCmd = commands.find(el => el.name === "Npc" + cmd.name)
          if (npcCmd) s += " - NPC too"
          debugmsg(s);
          count++;
        }
      }        
      debugmsg("... Found " + count + " commands (debug and meta-commands not listed).");
      return world.SUCCESS_NO_TURNSCRIPTS; 
    },
    objects:[
    ],
    listRegexes:function(cmd) {
      const ary = []
      for (const re of cmd.regexes) ary.push(re.toString())
      return ary.join(', ')
    },
  })
...
function handleStandUp(objects) {
  ...
  if (objects.length === 0) {
    ...
    char = objects[0][0]
    if (!char.npc) return failedmsg(lang.not_npc, {char:player, item:char})
    if (!char.getAgreement("Posture", {posture:"stand"})) return world.FAILED
  }  
  
  if (!char.testPosture()) return world.FAILED
  if (!char.posture || char.posture === 'standing') return failedmsg(lang.already, {item:char});

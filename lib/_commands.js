...
function handleFillWithFluid(char, vessel, fluid) {
  ...
  return vessel.doFill(char, source, fluid) ? world.SUCCESS: world.FAILED;
}
...
function handleEmptyInto(char, vessel, sink) {
  ...
  if (!char.getAgreement("Fill", vessel)) return world.FAILED;
  if (!vessel.isAtLoc(char.name)) return failedmsg(lang.not_carrying, {char:char, item:obj});
  return vessel.doEmpty(char, sink) ? world.SUCCESS: world.FAILED;
}
...
function handleInOutContainer(char, objects, verb, func) {
  ...
  for (const obj of objects[0]) {
    const flag = func(char, container, obj, multiple)
...
function handleSingleDropInContainer(char, container, obj, multiple) {
  if (!char.getAgreement("Drop/in", obj)) return
  if (!container.testForRecursion(char, obj)) return false
  if (obj.testDropRestrictions && !obj.testDropRestrictions(char, isMultiple, container)) return false
...
function handleSingleTakeOutContainer(char, container, obj, multiple) {
...
function handleSingleGiveToNpc(char, npc, obj, multiple) {
  if (!char.getAgreement("Give", obj)) return false
  if (npc.testGiveRestrictions && !npc.testGiveRestrictions(char, obj)) return false

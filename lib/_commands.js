...
const commands = [
  ...
  new Cmd('Give', {
    ...
    matchItems:function(s) {
      ...
      const scope = parser.scope(parser.isVisible)
  ...
  new Cmd('NpcGive', {
    ...
    matchItems:function(s) {
      ...
      if (possibleChars.length === 1) {
        ...
        const actualChars = possibleChars.filter(el => (el.npc && el !== char))
      ...
      const npcs = scope.filter(el => (el.npc && el !== char))
      const items = array.fromTokens(arr[0].split(' '), scope, {})
      if (!items) return this.setError(parser.NO_OBJECT, lang.object_unknown_msg(arr[0]))
...
for (const s of ['In', 'Out', 'Up', 'Down', 'Through']) {
  commands.push(new Cmd('Go' + s + 'Item', {
    ...
    dirType:s,
    script:function(objects) { return currentLocation.goItem(objects[0][0], this.dirType) },
  }))
  commands.push(new Cmd('NpcGo' + s + 'Item', {
    ...
    dirType:s,
    script:function(objects) { return currentLocation.goItem(objects[1][0], this.dirType, objects[0][0]) },
  }))
}

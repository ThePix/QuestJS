...
const commands = [
  ...
  new Cmd('Smell', {
    script:function() {
      if (currentLocation.smell) {
        printOrRun(game.player, currentLocation, "smell");
      }
      else {
  ...
  new Cmd('Listen', {
    script:function() {
      if (currentLocation.listen) {
        printOrRun(game.player, currentLocation, "listen");
      }
      else {
  ...
  new Cmd('EmptyFluidInto', {
    ...
    script:function(objects) {
      return handleEmptyFluidInto(game.player, objects[0], objects[1][0]);
    },
  }),
  new Cmd('NpcEmptyFluidInto', {
    ...
    script:function(objects) {
      ...
      if (!npc.npc) return failedmsg(lang.not_npc, {char:game.player, item:npc})
      objects.shift()
      return handleEmptyFluidInto(npc, objects[0], objects[1][0])
    },
  }),
  ...
  new Cmd('TieTo', {
    ...
    script:function(objects) {
      ...
      return rope.handleTieTo(game.player, objects[1][0]) 
    },
  }),
  new Cmd('NpcTieTo', {
    rules:[cmdRules.canManipulate, cmdRules.isHeld],
  ...
  new Cmd('Untie', {
    ...
    script:function(objects) {
      const rope = objects[0][0]
      if (!rope.rope) return failedmsg(lang.rope_not_attachable, {rope:rope})
      return rope.handleUntieFrom(game.player) 
    },
  }),
  new Cmd('NpcUntie', {
    rules:[cmdRules.canManipulate, cmdRules.isHere],
  ...
  new Cmd('UntieFrom', {
    ...
    script:function(objects) {
      const rope = objects[0][0]
      if (!rope.rope) return failedmsg(lang.rope_not_attachable, {rope:rope})
      return rope.handleUntieFrom(game.player, objects[1][0]) 
    },
  }),
...
function handleEmptyFluidInto(char, fluid, sink) {
  for (const key in w) {
    const o = w[key]
    if (o.vessel && o.containedFluidName === fluid && o.loc === char.name) {
      return handleEmptyInto(char, o, sink)
    }
  }
...
function handleSingleDropInContainer(char, container, obj, multiple, count) {
  ...
  if (obj.testDrop && !obj.testDrop(char, {multiple:multiple, container:container})) return false
  if (container.testDropIn && !container.testDropIn(char, {multiple:multiple, item:obj})) return false
  if (!obj.isAtLoc(char.name)) return failedmsg(prefix(obj, multiple) + lang.not_carrying, {char:char, item:obj})

  obj.moveToFrom(char, container.name, char.name)
  msg(prefix(obj, multiple) + obj.msgDropIn, {char:char, item:obj, item_count:count, container:container})
  return true;
}
...
function handleSingleTakeOutContainer(char, container, obj, multiple, count) {
  ...
  if (container.testTakeOut && !container.testTakeOut(char, {multiple:multiple, item:obj})) return false

  obj.moveToFrom(char, char.name, container.name)
  msg(prefix(obj, multiple) + obj.msgTakeOut, {char:char, item:obj, item_count:count, container:container})
  return true
}
...
function handleSingleGiveToNpc(char, npc, obj, multiple) {
  ...
  if (npc.testGive && !npc.testGive(char, {multiple:multiple, item:obj})) return false
  ...
  obj.moveToFrom(char, npc.name, char.name)
  return true
}

...
const commands = [
  ...
  new Cmd('LookExit', {
    script:function(objects) {
      ...
      if (!exit || exit.isHidden()) return failedmsg(lang.not_that_way, tpParams)
      if (exit.isLocked()) return failedmsg(lang.locked_exit(player, exit))
...
for (const el of lang.questions) {
  commands.push(new Cmd('Question' + verbify(el.q), {
...
if (settings.playMode === 'dev') {
  ...
  commands.push(new Cmd('DebugWarpName', {
    script:function(arr) {
      const o = w[arr[0]]
      if (!o) {

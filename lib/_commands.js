...
const commands = [
  ...
  new Cmd('Drop', {
    ...
    default:function(options) { falsemsg(item.isAtLoc(options.char) ? lang.cannot_drop : lang.not_carrying, options) },
  }),
  new Cmd('Wear2', {
    npcCmd:true,
  ...
  new Cmd('Say', {
    script:function(arr) {
      ...
      if (settings.givePlayerSayMsg) msg(lang.nounVerb(player, arr[0], true) + ", '" + sentenceCase(arr[1]) + ".'");
      for (let chr of l) {
      ...
      if (settings.givePlayerSayMsg) {
        msg(lang.say_no_response(player, arr[0], arr[1]));
      }
      else {      
        msg(lang.say_no_response_full(player, arr[0], arr[1]));
      }
      return world.SUCCESS;
    },
    objects:[
  ...
  new Cmd('FillWith', {
    ...
    script:function(objects) {
      return handleFillWithFluid(player, objects[0][0], objects[1]);
    },
  }),
  new Cmd('NpcFillWith', {
    rules:[cmdRules.canManipulate, cmdRules.isHeld],
  ...
  new Cmd('EmptyInto', {
    ...
    script:function(objects) {
      return handleEmptyInto(player, objects[0][0], objects[1][0]);
    },
  }),
  ...
  new Cmd('EmptyFluidInto', {
    ...
    script:function(objects) {
      return handleEmptyFluidInto(player, objects[0], objects[1][0]);
    },
  }),
...
function handleFillWithFluid(char, vessel, fluid) {
  const options = {char:char, container:vessel, fluid:fluid}
  ...
  if (!vessel.findSource(options)) return failedmsg(lang.no_fluid_here, options);
  return vessel.doFill(options) ? world.SUCCESS: world.FAILED;
}
...
function handleInOutContainer(char, objects, verb, func) {
  ...
  const options = {char:char, container:container, multiple:objects[0].length > 1 || parser.currentCommand.all}

...
function processtext(str, params) {
  ...
  try {
    ...
    errormsg(ERR_TP, "Text processor string caused an error, returning unmodified (reported error: " + err + ")");
    return str;
  }
}
...
tp.addDirective = function(name, fn) {
  //debugmsg("Adding '" + name + "' to the text processor.");
...
tp.text_processors = {
  ...
  objects:function(arr, params) {
    ...
    return formatList(listOfOjects, {article:"a", lastJoiner:" and ", modified:true, nothing:"nothing", loc:game.player.loc});
  },

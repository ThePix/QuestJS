...
class IntoZoneExit extends Exit {
  constructor(name, x, y, hash) {
    super(name, hash)
    this.zoneX = x
    this.zoneY = y
    this.beforeMove = function(char) {
      char.positionX = this.zoneX
      char.positionY = this.zoneY
      log("x=" + char.positionX + " y=" + char.positionY)
    }
  }  
}


const zoneUseExit = function(char, exit) {
  if (!exit) exit = this
  //log(exit)
  ...
  for (let el of this.origin.zoneExits) {
    if (char.positionX === el.x && char.positionY === el.y && dir === el.dir) {
      ...
      if (el.isLocked) return falsemsg(el.msgLocked || lang.locked_exit, tpParams)
      msg(el.msg || lang.go_successful, tpParams)
      this.origin.afterZoneExit(dir)
  ...
  for (let el of this.origin.getBorders()) {
    if (el.border(newX, newY, true)) return falsemsg(el.borderMsg.replace('#', dir))
  }
...
const ZONE = function(defaultToBlocked) {
  ...
  for (let ex of lang.exit_list) {
    ...
    res[ex.name] = new Exit("_", {
      use:zoneUseExit,
      data:ex,

      beforeMoveOld:function(char) {
        log(char.name)
        char.positionX += this.data.x
        char.positionY += this.data.y
        log("X=" + char.positionX + " Y=" + char.positionY)
        log(this)
        //return 'desert'
      },
  ...
  res.getDesc = function(el) {
    return (typeof el.desc === 'function' ? el.desc(player.positionX, player.positionY) : el.desc) + this.getFeatureDescs()
  }
  
  res.afterZoneExit = function(dir) {
    for (let name in w) {
      const o = w[name]

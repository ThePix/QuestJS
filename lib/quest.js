...
quest.start = function(name) {
  console.log(name)
  const result = quest.getState(name, game.player)
  if (result.progress !== undefined) return false // quest already started
  game.player[result.progressName] = quest.ACTIVE
...
quest.complete = function(name) {
  const data = quest.getState(name, game.player)
  if (result.progress !== quest.ACTIVE) return false
  game.player[result.progressName] = quest.SUCCESS
  game.player[result.stateName] = false
...
quest.fail = function(name) {
  const data = quest.getState(name, game.player)
  if (result.progress !== quest.ACTIVE) return false
  game.player[result.progressName] = quest.FAILED
  game.player[result.stateName] = false
...
quest.moot = function(name) {
  const data = quest.getState(name, game.player)
  if (result.progress !== quest.ACTIVE) return false
  game.player[result.progressName] = quest.MOOT
  game.player[result.stateName] = false

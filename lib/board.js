...
board.setup = function(settings) {
  ...
  document.querySelector('<div id="map" style="position: fixed;bottom: 0px; left: 200px;width:' + this.settings.width + 'px;height:' + this.settings.height + 'px;">This will be the board...</div>').insertAfter('#dialog')
  document.querySelector('#main').style.margin-bottom = '420px'
  this.update()
}
...
board.update = function() {
  ...
  document.querySelector('#map').innerHTML = '<svg width="' + this.settings.width + '" height="' + this.settings.height + '" viewBox="0 0 ' + this.settings.width + ' ' + this.settings.height + '" xmlns="http://www.w3.org/2000/svg">' + this.map.join() + this.labels.join() + '</svg>'
  return true
}  

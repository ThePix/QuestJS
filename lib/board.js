...
board.setup = function(settings) {
  ...
  $('<div id="map" style="position: fixed;bottom: 0px; left: 200px;width:' + this.settings.width + 'px;height:' + this.settings.height + 'px;">This will be the board...</div>').insertAfter('#dialog')
...
board.update = function() {
  ...
  if (this.settings.compass) this.map.push(board.getCompass(this.settings.compass.x, this.settings.compass.y))
  if (this.settings.switches) this.map.push(board.getSwitches())
  if (this.settings.extras) this.map.push(this.settings.extras())
  $('#map').html('<svg width="' + this.settings.width + '" height="' + this.settings.height + '" viewBox="0 0 ' + this.settings.width + ' ' + this.settings.height + '" xmlns="http://www.w3.org/2000/svg">' + this.map.join() + this.labels.join() + '</svg>')
  return true
}  
...
board.getSwitches = function() {
  ...
  for (let el of this.settings.switches) {
    ...
    s += (this.settings[el.att] ? el.on : el.off) + '" stroke="black" onclick="board.toggle(\'' + el.att + '\')"/>'

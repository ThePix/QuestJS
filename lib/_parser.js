...
parser.execute = function() {
  ...
  try {
    ...
    if (parser.currentCommand.objects.length > 0 && typeof parser.currentCommand.objects[0] === "object" && !parser.currentCommand.all) {
...
//    }
    ...
    if (cmdParams.special) {
      ...
      const specialError = parser.specialText[cmdParams.special].error(arr[i])
      if (!specialError) {
        const special = parser.specialText[cmdParams.special].exec(arr[i])
        if (special) res.objects.push(special)
        score = 1
      }
      else {

...
parser.parse = function(inputText) {
  ...
  if (parser.override) {
    ...
    if (settings._oldCursor) document.querySelector('#cursor').innerHTML = settings.cursor = settings._oldCursor
    return
  }
...
// A command can call this to stop processing later commands
// should the user have types multiple commands on one line
    ...
    for (let j = 0; j < parser.currentCommand.tmp.objects[i].length; j++) {
      if (parser.currentCommand.tmp.objects[i][j] instanceof Array) {
        if (parser.currentCommand.tmp.objects[i][j].length === 1) {
          ...
          io.disableWorldEndTurn = true
          fn(lang.disambig_msg, parser.currentCommand.tmp.objects[i][j], function(result, input) {
...
parser.overrideWith = function(fn, cursor, echoCursor) {
  if (cursor) {
    settings._oldCursor = settings.cursor
    settings.cursor = echoCursor || cursor
    document.querySelector('#cursor').innerHTML = processText(cursor)
  }
  parser.override = fn
}


// Called once we have a command and items all resolved
// All the data will be in parser.currentCommand
parser.execute = function() {
  ...
  try {
    ...
    const outcome = parser.currentCommand.execute(parser.currentCommand.tmp.objects)
...
// Custom parser tpyes use special text.
// Special text is something matched by a specific command other than an item.
// FILL BUCKET WITH WATER would have special text for WATER
// https://github.com/ThePix/QuestJS/wiki/Custom-parser-types
...
parser.specialText.direction = {
  error:function(text) {
    const dir = getDir(text)
    if (dir) return false
    return processText(lang.not_a_direction, {text:text})
  },
  exec:function(text) {
    return getDir(text)
  },
...
parser.generateSpecialText = function(name, array, notKnownMsg) {
  parser.specialText[name] = {
    error:function(text) {
      if (array.length === 0) return lang.not_implemented
      if (array.includes(text)) return false
      for (const el of array) {
        if (Array.isArray(el) && el.includes(text)) return false
      }
      return processText(notKnownMsg, {text:text})
    },
    exec:function(text) {
      if (array.includes(text)) return text
      for (const el of array) {
        if (Array.isArray(el) && el.includes(text)) return el[0]
      }
...
parser.generateSpecialText('fluid', settings.fluids, lang.not_a_fluid_here)
parser.generateSpecialText('tone', settings.tones, lang.not_a_tone)

...
parser.execute = function() {
  ...
  try {
    ...
    if (outcome === undefined && settings.playMode === 'dev') log("WARNING: " + parser.currentCommand.cmd.name + " command did not return a result to indicate success or failure.")
    inEndTurnFlag = true
    world.endTurn(outcome)
  } catch (err) {
    if (inEndTurnFlag) {
      io.printError("Hit a coding error trying to process world.endTurn after that command.", err)
    }
    else {
      io.printError("Hit a coding error trying to process the command `" + parser.currentCommand.cmdString + "'.", err)
    }
...
parser.matchItemsToCmd = function(s, candidate) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (!cmdParams) {
      errormsg("The command \"" + cmd.name + "\" seems to have an error. It has more capture groups than there are elements in the 'objects' attribute.", true)
      return false
    }
    if (arr[i] === undefined) {
      errormsg("The command \"" + cmd.name + "\" seems to have an error. It has captured undefined. This is probably an issue with the command's regular expression.", true)
      return false
    }
    let score = 0;
    ...
    if (cmdParams.special) {
      ...
      if (!specialError) {
        ...
        parser.msg("-> special match failed: " + specialError)
      }
    }

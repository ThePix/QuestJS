  ...
  parser.matchItemsToCmd = function(s, candidate) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (cmd.objects[i].text) {
        ...
        // If there is an allScope, use that; otherwise use scope. Use fallbackScope (which is actually a list) if that is absent
        log(cmd.objects[i].allScope)
        log(cmd.objects[i].scope)
        let list = cmd.objects[i].scope ? parser.scope(cmd.objects[i].allScope ? cmd.objects[i].allScope : cmd.objects[i].scope) : fallbackScope;
        ...
        for (let s of objectNames) {
          ...
          if (n === 0) {
            ...
            if (!Array.isArray(objs2)) objs2 = [objs2]
            for (const obj of objs2) {
              if (objs.some(el => el.name === obj.name)) {
                parser.msg("..Skipping duplicate: " + obj.name)
              }
              else {
                if (n > score) { score = n; }
                objs.push(obj)

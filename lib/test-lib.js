...
test.start = function(title) {
  this.title(title)
  const s = localStorage.getItem(saveLoad.getName(test.saveFilename));
  if (s != null) {
    saveLoad.loadTheWorld(s, 4)
    if (settings.afterLoad) settings.afterLoad(test.saveFilename)
  }
  else {
...
test.assertCmd = function(cmdStr, expected, extraOutput) {
  test.assertOut(expected, extraOutput, function() {
    parser.parse(cmdStr)
  })
}

test.assertOut = function(expected, extraOutput, f) {
  test.totalCount++
  test.subCount++

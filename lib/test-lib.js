...
util.test.printTitle = function () {
  debugio.msg(util.test.currentTitle + ': Error (util.test ' + util.test.subCount + ')')
  util.test.failCount++
}
    ...
    for (let i = 0; i < Math.max(util.test.util.testOutput.length, expected.length); i++) {
      if (typeof expected[i] === 'string') {
        if (expected[i] !== util.test.util.testOutput[i]) {
          debugio.msg('Expected (A): ' + expected[i])
          debugio.msg('...Found (A): ' + util.test.util.testOutput[i])
          if (extraOutput) {
        ...
        if (util.test.util.testOutput[i] === undefined || !expected[i].util.test(util.test.util.testOutput[i])) {
          debugio.msg('Expected: ' + expected[i])
          debugio.msg('...Found: ' + util.test.util.testOutput[i])
        }
      } else if (expected[i] === undefined) {
        debugio.msg('Expected nothing')
        debugio.msg('...Found: ' + util.test.util.testOutput[i])
      } else {
        debugio.msg('Found an unrecognised type for expected (should be string or regex): ' + (typeof expected[i]))
      }
    }
  }
}
...
util.test.assertEqual = function (expected, found, extraOutput) {
  ...
  if (Array.isArray(expected)) {
    if (!util.arrayCompare(expected, found)) {
      util.test.printTitle()
      debugio.msg('Expected (A): ' + expected)
      debugio.msg('...Found (A): ' + found)
    }
  } else if (expected === found) {
    // debugio.msg(".");
  } else {
    util.test.printTitle()
    debugio.msg('Expected: ' + expected)
    debugio.msg('...Found: ' + found)
    if (extraOutput) {
...
util.test.fail = function (io.msg) {
  util.test.printTitle()
  debugio.msg('Failure: ' + io.msg)
}

util.test.results = function (time) {
  const elapsed = parseInt(Date.now()) - time
  debugio.msg('Number of util.tests: ' + util.test.totalCount)
  debugio.msg('Number of fails: ' + util.test.failCount)
  debugio.msg('Elapsed time: ' + elapsed + ' ms (' + (Math.round(elapsed / util.test.totalCount * 10) / 10) + ' ms/util.test)')
}

    ...
    for (let i = 0; i < Math.max(test.testOutput.length, expected.length); i++) {
      if (typeof expected[i] === "string") {
        if (expected[i] !== test.testOutput[i]) {
          debugmsg("Expected (A): " + test.print(expected[i]))
          debugmsg("...Found (A): " + test.print(test.testOutput[i]))
          debugmsg(" ")
          if (extraOutput) {
            if (typeof expected[i] === "string" && typeof test.testOutput[i] === "string") {
              for (let j = 0; j < expected[i].length; j++) {
                if (expected[i][j] !== test.testOutput[i][j]) {
                  ...
                  console.log("Expected: " + test.print(expected[i].charCodeAt(j)))
                  console.log("Found: " + test.print(test.testOutput[i].charCodeAt(j)))
                }
              }
            }
            else {
        ...
        debugmsg("...Found: " + test.print(test.testOutput[i]))
        debugmsg(" ")
      }
      else {
...
test.print = function(o) {
  if (o === null) return '[null]'
  if (o === undefined) return '[undefined]'
  if (o.name) return '[an object named "' + o.name + '"]'
  return o
}  
...
test.assertEqual = function(expected, found, extraOutput) {
  ...
  if (Array.isArray(expected)) {
    ...
    test.printTitle()
    debugmsg("Expected: " + test.print(expected))
    debugmsg("...Found: " + test.print(found))
    debugmsg(" ")
    if (extraOutput) {

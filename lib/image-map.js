...
const map = {
  ...
  defaults:{
    ...
    mapDrawRegion:function(region, i) {
      let s = '<g>'
      if (region.r) {
        s += '<circle cx="' + (region.x/settings.mapScale+18) + '" cy="' + (region.y/settings.mapScale-23) + '" '
        s += 'r="' + (region.r/settings.mapScale) + '" '
      }
      else if (region.rx) {
        s += '<ellipse cx="' + (region.x/settings.mapScale+18) + '" cy="' + (region.y/settings.mapScale-23) + '" '
        s += 'rx="' + (region.rx/settings.mapScale) + '" ry="' + (region.ry/settings.mapScale) + '" '
      }
      else if (region.pts) {
        s += '<polygon points="' + region.pts + '" '
      }
      else {
        s += '<rect x="' + (region.x/settings.mapScale+18) + '" y="' + (region.y/settings.mapScale-23) + '" '
        s += 'width="' + (region.w/settings.mapScale) + '" height="' + (region.h/settings.mapScale) + '" '
      }
      s += 'fill="' + (region.fill || 'blue') + '" '
      s += 'cursor="pointer" '
      if (settings.mapRegionOpacity !== undefined) s += 'opacity="' + settings.mapRegionOpacity + '" '
      if (region.script) {
        s += 'onclick="settings.mapRegions[' + i + '].script()">'
      }
      else if (region.jump) {
        s += 'onclick="map.jumpLocation(\'' + region.jump + '\')">'
      }
      else {
        s += 'onclick="runCmd(\'' + region.cmd + '\')">'
      }
      s += '</g>'
      return s
    },
...
map.jumpLocation = function(loc) {
  const room = w[loc]
  if (!room) return errormsg("Trying to jump to an unknown location, " + loc + " in map.jumpLocation.")
  
  const exit = new Exit(loc, {dir:'teleport', origin:currentLocation})

  player.msg(lang.stop_posture(player))
  player.movingMsg(exit) 
  player.moveChar(exit)
...
map.init = function() {
  ...
  if (!settings.mapScrollable) return
...
map.redraw = function(offX, offY) {
  ...
  for (const point of settings.mapPointsOfInterest) {
  ...
  settings.mapRegions.forEach(function (region, i) {
    if (!region.mapRegion) region.mapRegion = settings.mapImages[0].name
    if (region.mapRegion !== player.mapRegion) return
    if (!region.isActive || region.isActive()) result.push(settings.mapDrawRegion(region, i))  
  })
  result.push('</g>')

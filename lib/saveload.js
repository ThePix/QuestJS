...
An object is saved as its name followed by an equals sign followed by the data. Each datam is separated by a semi-colon. Each datum consists of the name, the type and the value, separated by colons.

If a datum is an object, and has a name attribute, the name is saved as type qobject.
If the datam is an array and the first element is a string, it is assumed that all the elements are strings, and it is saved as an array
If the datam is a number, a string or true it is saved as such
Any other objects or values will not be saved.

Assumes the title and version do not include ch
...
const saveLoad = {
  ...
  getSaveHeader:function(comment) {
    ...
    let s = saveLoad.encodeString(TITLE) + "!";
    s += saveLoad.encodeString(VERSION) + "!";
  ...
  encodeString:function(s) {
    for (let d of saveLoad.replacements) {
      s = replaceAll(s, new RegExp(d.unescaped, "g"), "@@@" + d.escaped + "@@@");
    }
    return s;
    return '"' + s + '"';
  },
  ...
  decodeString:function(s) {
    for (let d of saveLoad.replacements) {
      s = replaceAll(s, new RegExp("@@@" + d.escaped + "@@@", "g"), d.unescaped);
    }
    return s;
  },

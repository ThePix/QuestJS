...
function Cmd(name, hash) {
  ...
  if (!this.regex && !this.regexes) {
    this.regexes = Array.isArray(lang[this.name]) ? lang[this.name] : [lang[this.name]]
  }
}
...
function ExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    if (!game.room.hasExit(this.dir)) {
      failedmsg(lang.not_that_way, {char:game.player, dir:this.dir});
      return world.FAILED;
    }
    else {
...
cmdRules.isHeldNotWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) {
    failedmsg(prefix(item, isMultiple) + lang.wearing, {char:char, garment:item});
    return false;
  }
  ...
  if (item.loc) {
    ...
    if (holder.npc || holder.player) {
      failedmsg(prefix(item, isMultiple) + lang.char_has_it, {holder:holder, item:item});
      return false;
    }
  }
    
  failedmsg(prefix(item, isMultiple) + lang.not_carrying, {char:char, item:item});
  return false;
};
...
cmdRules.isWorn = function(cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, world.PARSER)) {
    failedmsg(prefix(item, isMultiple) + lang.not_wearing, {char:char, garment:item});
    return false;
  }
...
cmdRules.isHeld = function(cmd, char, item, isMultiple) {
  ...
  failedmsg(prefix(item, isMultiple) + lang.not_carrying, {char:char, item:obj});
  return false;
};
...
cmdRules.isHere = function(cmd, char, item, isMultiple) {
  ...
  failedmsg(prefix(item, isMultiple) + lang.not_here, {char:char, item:item});
  return false;
};
...
cmdRules.canTalkTo = function(cmd, char, item) {
  ...
  if (!item.npc) {
    failedmsg(prefix(item, isMultiple) + lang.not_able_to_hear, {char:char, item:item});
    return false;
  }
  return true;
};

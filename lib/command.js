...
function initCommands() {
  ...
  commands.forEach(function(el) {
    ...
    if (el.npcCmd) {
      //if (parser.debug) console.log("creating NPC command for " + el.name)
      ...
      let cmd = new NpcCmd("Npc" + el.name + "1", data);
      cmd.regex = new RegExp("^(.+), ?" + regexAsStr);
      if (el.useThisScriptForNpcs) cmd.script = el.script;
      newCmds.push(cmd);
      
      cmd = new NpcCmd("Npc" + el.name + "2", data);
      cmd.regex = new RegExp("^tell (.+) to " + regexAsStr);
      if (el.useThisScriptForNpcs) cmd.script = el.script;
      newCmds.push(cmd);
    }
  });

...
function Cmd(name, hash) {
  ...
  this.default = function(item, isMultiple, char) { 
    if (typeof this.defmsg === "string") {
      msg(prefix(item, isMultiple) + this.defmsg);
    }
...
function NpcCmd(name, hash) {
  ...
  this.script = function(objects) {
    ...
    for (let i = 0; i < objects[1].length; i++) {
      ...
      if (!npc["getAgreement" + this.shortName] && npc.getAgreement && !npc.getAgreement(this.shortName, objects[1][i])) {
        continue;
      }
...
cmdRules.charCanManipulateRule = function(char, item, isMultiple) {
...
cmdRules.charCanTalkRule = function(char, item, isMultiple) {
  if (!char.canTalk()) {
    return false;
  }
  if (!item.canTalk()) {
...
cmdRules.charCanPostureRule = function(char, item, isMultiple) {
  if (!char.canPosture()) {
    return false;
  }
  return true;

...
function NpcCmd (name, hash) {
  ...
  this.script = function (objects) {
    ...
    let success = false; let handled // -review whats this?-
    if (objects.length !== 2) {
    ...
    if (!game.room.hasExit(this.dir)) {
      ...
      if (typeof ex === 'object') {
        ...
        if (typeof flag !== 'boolean') {
          io.errorio.msg('Exit failed to return a Boolean value, indicating success of failure; assuming success')
          return util.SUCCESS
        }
...
function initCommands () {
  ...
  lang.exit_list.forEach(function (el) {
    if (!el.nocmd) {
      ...
      regex += ')$'
      let cmd = new ExitCmd('Go' + util.sentenCecase(el.name), el.name, {

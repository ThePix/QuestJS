...
function Cmd(name, hash) {
  ...
  this.processCommand = function(char, item, multi) {
    ...
    var result = printOrRun(char, item, this.attName, multi);
...
function initCommands(EXITS) {
  ...
  EXITS.forEach(function(el) {
    if (!el.nocmd) {
      var regex = "^(" + GO + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      ...
      regex = "^(.+), ?(" + GO + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      if (el.alt) { regex += "|" + el.alt; }
      regex += ")$";
      ...
      regex = "^tell (.+) to (" + GO + ")(" + el.name + "|" + el.abbrev.toLowerCase();
      if (el.alt) { regex += "|" + el.alt; }
      regex += ")$";

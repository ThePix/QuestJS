...
function Cmd (name, hash) {
  ...
  this.default = function (item, isMultiple, char) {
    if (typeof this.defmsg === 'string') {
      io.failedmsg(prefix(item, isMultiple) + this.defmsg)
    } else if (typeof this.defmsg === 'function') {
      io.failedmsg(prefix(item, isMultiple) + this.defmsg(char, item))
    } else {
...
function ExitCmd (name, dir, hash) {
  ...
  this.script = function (objects) {
    if (!game.room.hasExit(this.dir)) {
      io.failedmsg(lang.not_that_way(game.player, this.dir))
      return FAILED
    } else {
...
function NpcExitCmd (name, dir, hash) {
  ...
  this.script = function (objects) {
    const npc = objects[0][0]
    if (!game.room.hasExit(this.dir)) {
      io.failedmsg(lang.not_that_way(npc, this.dir))
      return FAILED
    }
...
cmdRules.isHeldNotWorn = function (cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, display.PARSER)) {
    io.failedmsg(prefix(item, isMultiple) + lang.wearing(char, item))
    return false
  }
  ...
  if (item.loc) {
    ...
    if (holder.npc || holder.player) {
      io.failedmsg(prefix(item, isMultiple) + lang.char_has_it(holder, item))
      return false
    }
  }

  io.failedmsg(prefix(item, isMultiple) + lang.not_carrying(char, item))
  return false
}
...
cmdRules.isWorn = function (cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, display.PARSER)) {
    io.failedmsg(prefix(item, isMultiple) + lang.not_wearing(char, item))
    return false
  }
...
cmdRules.isHere = function (cmd, char, item, isMultiple) {
  ...
  io.failedmsg(prefix(item, isMultiple) + lang.not_here(char, item))
  return false
}
...
cmdRules.canTalkTo = function (cmd, char, item) {
  ...
  if (!item.npc) {
    io.failedmsg(prefix(item, isMultiple) + lang.not_able_to_hear(char, item))
    return false
  }
  return true
}

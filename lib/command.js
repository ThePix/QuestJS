...
cmdRules.isHeldNotWorn = function(char, item, isMultiple) {
  if (!item.getWorn() && item.isAtLoc(char.name)) {
    return true;
  }
...
cmdRules.isWorn = function(char, item, isMultiple) {
  if (item.getWorn() && item.isAtLoc(char.name)) {
    return true;
  }
...
cmdRules.isHeld = function(char, item, isMultiple) {
  if (item.isAtLoc(char.name)) {
    return true;
  }
...
cmdRules.isHere = function(char, item, isMultiple) {
  if (item.isAtLoc(char.loc)) return true;
  if (item.isAtLoc(char.name)) return true;
...
// Item's location is the char's location or the char
// or item is reachable, but not held by someone else
cmdRules.isHereNotHeld = function(char, item, isMultiple) {
  if (item.isAtLoc(char.loc)) return true;
...
cmdRules.canManipulate = function(char, item) {
  if (!char.canManipulate(item)) {
    return false;
  }
  return true;
};

cmdRules.canTalkTo = function(char, item) {
...
cmdRules.canPosture = function(char, item) {
  if (!char.canPosture(item)) {
    return false;
  }
  return true;
};
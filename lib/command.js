'use strict'
import { util, io, parser, game, commands, lang, world, w } from './main'
...
function Cmd (name, hash) {
  ...
  this.default = function (item, isMultiple, char) {
    if (typeof this.defio.msg === 'string') {
      io.failedio.msg(util.prefix(item, isMultiple) + this.defio.msg)
    } else if (typeof this.defio.msg === 'function') {
      io.failedio.msg(util.prefix(item, isMultiple) + this.defio.msg(char, item))
    } else {
      io.errorio.msg("No default set for command '" + this.name + "'.")
    }
    return false
  }
  ...
  this.script = function (objects, matches) {
    ...
    for (let i = 0; i < objects[0].length; i++) {
      if (!objects[0][i][this.attName]) {
        ...
        if (result === util.SUPPRESS_ENDTURN) {
    ...
    if (success) {
      return (this.noTurnscripts || suppressEndturn ? util.SUCCESS_NO_TURNSCRIPTS : util.SUCCESS)
    } else {
      return util.FAILED
    }
  }
  ...
  this.processCommand = function (char, item, multi, match, verb) {
    for (const rule of this.rules) {
      if (typeof rule !== 'function') {
        io.errorio.msg("Failed to process command '" + this.name + "' as one of its rules is not a function (see console).")
        console.log('Failed:')
        console.log(this)
        console.log(rule)
      }
    ...
    let result = io.printOrRun(char, item, this.attName, { multi: multi, match: match, verb: verb })
    if (typeof result !== 'boolean' && result !== util.SUPPRESS_ENDTURN) {
...
function NpcCmd (name, hash) {
  ...
  this.script = function (objects) {
    ...
    let success = false
    if (objects.length !== 2) {
      io.errorio.msg('The command ' + name + ' is trying to use a facility for NPCs to do it, but there is no object list; this facility is only for commands in the form verb-object.')
      return util.FAILED
    }
    ...
    if (success) {
      npc.pause()
      return (this.noTurnscripts ? util.SUCCESS_NO_TURNSCRIPTS : util.SUCCESS)
    } else {
      return util.FAILED
    }
  }
...
function ExitCmd (name, dir, hash) {
  ...
  this.script = function (objects) {
    if (!game.room.hasworld.Exit(this.dir)) {
      io.failedio.msg(lang.not_that_way(game.player, this.dir))
      return util.FAILED
    } else {
      ...
      if (typeof ex === 'object') {
        ...
        if (typeof flag !== 'boolean') {
          io.errorio.msg('world.Exit failed to return a Boolean value, indicating success of failure; assuming success')
          return util.SUCCESS
        }
        return flag ? util.SUCCESS : util.FAILED
      } else {
        io.errorio.msg('Unsupported type for direction')
        return util.FAILED
      }
    }
  }
...
function NpcExitCmd (name, dir, hash) {
  ...
  this.script = function (objects) {
    const npc = objects[0][0]
    if (!game.room.hasworld.Exit(this.dir)) {
      io.failedio.msg(lang.not_that_way(npc, this.dir))
      return util.FAILED
    }
...
function useWithDoor (char, dir) {
  ...
  if (obj === undefined) {
    io.errorio.msg("Not found an object called '" + this.door + "'. Any exit that uses the 'useWithDoor' function must also set a 'door' attribute.")
  }
  ...
  if (!obj.locked) {
    obj.closed = false
    io.msg(lang.open_and_enter(char, doorName))
  ...
  if (obj.testKeys(char)) {
    obj.closed = false
    obj.locked = false
    io.msg(lang.unlock_and_enter(char, doorName))
    world.setRoom(char, this.name, false)
    return true
  }
  io.msg(lang.try_but_locked(char, doorName))
  return false
};
...
function initCommands () {
  ...
  commands.forEach(function (el) {
    ...
    if (!(el.regex instanceof RegExp)) {
      commands.alert('No regex for ' + el.name)
    }
    if (el.npcCmd) {
  ...
  lang.exit_list.forEach(function (el) {
    if (!el.nocmd) {
      ...
      regex += ')$'
      let cmd = new world.ExitCmd('Go' + util.sentenCecase(el.name), el.name, {
      ...
      if (el.alt) { regex += '|' + el.alt }
      regex += ')$'
      cmd = new NpcExitCmd('NpcGo' + util.sentenCecase(el.name) + '1', el.name, {
        regex: new RegExp(regex)
      })
      commands.push(cmd)
      ...
      if (el.alt) { regex += '|' + el.alt }
      regex += ')$'
      cmd = new NpcExitCmd('NpcGo' + util.sentenCecase(el.name) + '2', el.name, {
        regex: new RegExp(regex)
      })
      commands.push(cmd)
    }
...
function extractChar (cmd, objects) {
  ...
  if (cmd.forNpc) {
    char = objects[0][0]
    if (!char.npc) {
      io.failedio.msg(lang.not_npc(char))
      return util.FAILED
    }
    objects.shift()
  } else {
...
cmdRules.isHeldNotWorn = function (cmd, char, item, isMultiple) {
  if (!item.getWorn() && item.isAtLoc(char.name, util.display.PARSER)) {
    return true
  }

  if (item.isAtLoc(char.name, util.display.PARSER)) {
    io.failedio.msg(util.prefix(item, isMultiple) + lang.wearing(char, item))
    return false
  }
  ...
  if (item.loc) {
    ...
    if (holder.npc || holder.player) {
      io.failedio.msg(util.prefix(item, isMultiple) + lang.char_has_it(holder, item))
      return false
    }
  }

  io.failedio.msg(util.prefix(item, isMultiple) + lang.not_carrying(char, item))
  return false
}
...
cmdRules.isWorn = function (cmd, char, item, isMultiple) {
  ...
  if (item.isAtLoc(char.name, util.display.PARSER)) {
    io.failedio.msg(util.prefix(item, isMultiple) + lang.not_wearing(char, item))
    return false
  }
...
cmdRules.isHere = function (cmd, char, item, isMultiple) {
  if (item.isAtLoc(char.loc, util.display.PARSER)) return true
  if (item.isAtLoc(char.name, util.display.PARSER)) return true
  ...
  if (item.scopeStatus === util.REACHABLE) {
    return true
  }

  io.failedio.msg(util.prefix(item, isMultiple) + lang.not_here(char, item))
  return false
}
...
cmdRules.canTalkTo = function (cmd, char, item) {
  ...
  if (!item.npc) {
    io.failedio.msg(util.prefix(item, 'isMultiple') + lang.not_able_to_hear(char, item)) // -fixme "isMultiple" is undefined-
    return false
  }
  return true
}

...
function NpcCmd(name, hash) {
  ...
  this.script = function(objects) {
    ...
    debugmsg("name=" + "getAgreement" + this.shortName);
    debugmsg("npc=" + npc.name);
    debugmsg("npc[\"getAgreement\" + this.shortName]=" + npc["getAgreement" + this.shortName]);
...
function NpcExitCmd(name, hash, dir) {
  ...
  this.script = function(objects) {
    ...
    else if (npc.getAgreementGo && !npc.getAgreementGo(name, dir)) {
      return FAILED;
    }
    else {
      var ex = game.room[this.dir];
      if (typeof ex === "string") {
        npc.moveFromTo(npc.loc, ex);

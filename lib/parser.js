    ...
    for (let j = 0; j < parser.currentCommand.objects[i].length; j++) {
      if (parser.currentCommand.objects[i][j] instanceof Array) {
        if (parser.currentCommand.objects[i][j].length === 1) {
          ...
          showMenu(lang.disambig_io.msg, parser.currentCommand.objects[i][j], function (result) {
...
parser.matchItemsToCmd = function (s, cmd) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (cmd.objects[i].text) {
      ...
      if (list.length === 0) {
        res.error = cmd.nothingForAll ? cmd.nothingForAll : nothing_io.msg
        res.score = -1
        return res
      }
...
parser.io.msg = function (...ary) {
  if (parser.debug) {
    for (const s of ary) debugio.msg('PARSER&gt; ' + s)
  }
}

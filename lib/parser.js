...
const parser = {};
  ...
  parser.execute = function() {
    ...
    const outcome = parser.currentCommand.cmd.script(parser.currentCommand.objects);
  ...
  parser.convertInputTextToCommandCandidate = function(inputText) {
    ...
    const cmdString = inputText.toLowerCase();
  ...
  parser.matchItemsToCmd = function(s, cmd) {
    const res = {cmd:cmd, objectTexts:[], objects:[]};
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (cmd.objects[i].text) {
        ...
        if (list.length === 0) {
          res.error = cmd.nothingForAll ? cmd.nothingForAll : NOTHING;
          res.score = -1;
          return res;
        }
        ...
        for (let j = 0; j < objectNames.length; j++) {
          ...
          if (objNameMatch === null) {
            errorms("Failed to match to ARTICLE_FILTER_REGEX - probably an error in ARTICLE_FILTER_REGEX!");
            return null;
          }

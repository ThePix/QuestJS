...
var parser = {};
  ...
  parser.convertInputTextToCommandCandidate = function(inputText) {
    var s = inputText.toLowerCase().split(' ').filter(function(el) { return !CMD_IGNORED_WORDS.includes(el); }).join(' ');
    var cmdString = s;
  ...
  parser.matchItemsToCmd = function(s, cmd) {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (!cmd.objects[i]) {
        errormsg(ERR_GAME_BUG, "That command seems to have an error. It has more capture groups than there are elements in the 'objects' attribute.");
        return false;
      }
      if (cmd.objects[i].ignore) {
        continue;
      }
      var objectNames, score = 0;
      ...
      if (cmd.objects[i].text) {
        ...
        if (CMD_ALL_EXCLUDE_REGEX.test(arr[i])) {
          ...
          objectNames = s.split(CMD_JOINER_REGEX).map(function(el){ return el.trim(); });
        ...
        objectNames = arr[i].split(CMD_JOINER_REGEX).map(function(el){ return el.trim() });
  ...
  parser.scoreObjectMatch = function(s, item, cmdParams) {
    ...
    if (s === itemName) {
      item.cmdMatch = s;
      return item[cmdParams.attName] ? 4 : 2;
    }
    ...
    if (new RegExp("\\b" + s).test(itemName)) {
      item.cmdMatch = s;
      return item[cmdParams.attName] ? 3 : 1;
    }
    return -1;
  };

...
var parser = {};
  ...
  parser.convertInputTextToCommandCandidate = function(inputText) {
    var s = inputText.toLowerCase().split(' ').filter(function(el) { return !IGNORED_WORDS.includes(el); }).join(' ');
    var cmdString = s;
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (cmd.objects[i].text) {
        ...
        if (ALL_EXCLUDE_REGEX.test(arr[i])) {
          ...
          var s = arr[i].replace(ALL_EXCLUDE_REGEX, "").trim();

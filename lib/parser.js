...
parser.isForSale = function(item) {
  return item.isForSale(game.player.loc, display.PARSER) && world.ifNotDark(item);
}
...
parser.isNpcOrHereisNpcOrHere = function(item) {
  return (item.isAtLoc(game.player.loc, display.PARSER) && world.ifNotDark(item)) || item.npc;
}

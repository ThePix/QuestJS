...
var parser = {};
  ...
  parser.matchItemsToCmd = function(s, cmd) {
    ...
    //debugmsg("*fallbackScope: " + fallbackScope);
    //var fallbackScope = scope([ifTransparent, ifNotDark]);
    ...
    for (var i = 0; i < arr.length; i++) {
      ...
      if (cmd.objects[i].text) {
        ...
        //debugmsg("objectNames=" + formatList(objectNames));
        ...
        //debugmsg("scope function: " + cmd.objects[i].scope);
        //debugmsg("fallbackScope: " + fallbackScope);
        //for (var k = 0; k < scopes.length; k++) { debugmsg("scope: " + formatList(scopes[k])); }
        
        var objs = [];
        var objs2, n;
        for (var j = 0; j < objectNames.length; j++) {
          //debugmsg(0, "cmd.objects[i]=" + JSON.stringify(cmd.objects[i]));
          //debugmsg("objectNames[j]=" + objectNames[j]);
          [objs2, n] = this.findInScope(objectNames[j], scopes, cmd.objects[i]);
          if (n === 0) {
            //debugmsg("cmd=" + cmd.name);
  ...
  parser.scoreObjectMatch = function(s, item, cmdParams) {
    //debugmsg(0, "cmdParams=" + JSON.stringify(cmdParams));

...
class Skill {
  ...
  targetEffect(attack, target) {
    ...
    const effect = rpg.findEffect(this.targetEffectName === true ? this.name : this.targetEffectName)
    effect.apply(attack, target, this.duration)
...
class Effect {
  ...
  apply(attack, target, duration) {
    ...
    target.activeEffects.push(this.name)
  }  
...
class Attack {
  ...
  resolve(target, isPrimary, count) {
    ...
    for (let name of terminatingEffects) {
      this.msg(rpg.findEffect(name).terminate(target))
    }
  ...
  applyDamage(target, prefix = 'damage') {
    ...
    if (this[prefix + 'Bonus'] || this[prefix + 'Number']) {
      this.msg(`${lang.damage}: ${this.damageNumber}d${this[prefix + 'Sides']}+${this[prefix + 'Bonus']}`, 3)
      let damage = this[prefix + 'Bonus'];
      for (let i = 0; i < this[prefix + 'Number']; i++) {
        const roll = random.int(1, this[prefix + 'Sides'])
        damage += roll
        this.msg(`${lang.damageRoll} (d${this[prefix + 'Sides']}): ${roll}`, 5)
      }
...
rpg.isSpellAvailable = function(char, spell) {
...
rpg.teleport = function(char, loc) {
...
rpg.hasEffect = function(obj, effect) {
...
const RPG_TEMPLATE = {
  ...
  isHostile:function() { return this.hostility > 4 },

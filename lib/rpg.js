...
const skills = {
  ...
  terminate:function(effect, item) {
    ...
    let s = "The {i:" + (effect.alias? effect.alias : effect.name) + "}" + (item === game.player ? '' : " on " + lang.getName(item, {article:DEFINITE})) + " terminates."
...
class Attack {
  static createAttack(attacker, target, skill) {
    ...
    // Find the skill
    if (attacker === game.player && skill === undefined && skillUI) {
...
const RPG_NPC = function(female) {
  ...
  res.oldRpgOnCreation = res.onCreation
...
const EQUIPPABLE = function() {
  ...
  res.onCreation = function(o) {
    o.oldRpgOnCreation(o)
    o.verbFunctions.push(function(o, verbList) {
      if (o.isAtLoc(game.player)) {
        verbList.push(o.equipped ? lang.verbs.unequip : lang.verbs.equip)
      }
    })
    o.nameModifierFunctions.push(function(o, list) {
      if (o.equipped && o.isAtLoc(game.player.name)) list.push(lang.invModifiers.equipped)
    })
  }
  ...
  res.drop = function(multiple, char) {
    ...
    msg(prefix(this, multiple) + lang.drop_successful, tpParams);
    this.moveToFrom(char, char.loc, this.loc);
    return true;
  }
  
  res.equip = function(multiple, char) {
  ...
  res.unequip = function(multiple, char) {
    if (!this.equipped) {
      msg("It already is.");
      return false;
    }
...
commands.push(new Cmd('Attack', {
  ...
  default:function(item, multiple, char) {
    msg(prefix(item, multiple) + "No point attacking " + lang.getName(item, {article:DEFINITE}) + ".");
    return false;
  },
}));
...
commands.push(new Cmd('Equip', {
  ...
  default:function(item, multiple, char) {
    msg(prefix(item, multiple) + lang.pronounVerb(item, "be", true) + " not something you can equip.");
    return false;
  },
}));

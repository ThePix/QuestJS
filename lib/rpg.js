...
class Spell extends Skill {
  constructor(name, data) {
    ...
    if (typeof data.effect === 'object') {
      ...
      skills.add(new Effect(name, {
        modifyOutgoingAttack:data.effect.modifyOutgoingAttack,
        modifyIncomingAttack:data.effect.modifyIncomingAttack,
        start:data.effect.start,
        finish:data.effect.finish,
...
// Expects:
// a getAffected function that takes an attack object, and returns an array of objects
// a effect function that takes an attack object and a single object from that list
// a noEffect string
// Uses could be to extinguish lights, lock or unlock things, raise corpses, destroy paper, melt ise
// Assumes the spell is instant, and the effect lasts indefinitely.
class SpellEnvironment extends Spell {
  constructor(name, data) {
    super(name, data)
  ...
  targetEffect(attack) {
    const list = this.getAffected(attack)
    if (list.length === 0) {
      attack.msg(this.noEffect, 1)
    }
    else {
      for (let el of list) this.effect(attack, el)
    }
  }
}
...
class Effect extends Skill {
  ...
  terminate(target) {
    ...
    if (!s) s = "The {i:{show:effect:alias}} on {nm:target:the} terminates."
...
class Attack {
  ...
  resolve(target, isPrimary, count) {
    ...
    if (this.skill.incompatible) {
      for (let name of target.activeEffects) {
        if (skills.findName(name).incompatible === this.skill.incompatible) terminatingEffects.push(name)
      }
    }
  ...
  applyDamage(target, prefix = 'damage') {
    ...
    if (this[prefix + 'Bonus'] || this[prefix + 'Number']) {
      ...
      this.msg("Armour: " + target.getArmour(), 4)

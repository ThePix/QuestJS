...
class Attack {
  ...
  resolve(target, isPrimary, count) {
    ...
    if (this.skill.automaticSuccess || !target.getDefensiveBonus) {
      ...
      this.result = this.offensiveBonus - this.defensiveBonus + this.roll - 10
      this.msg(processText(this.skill.targetText, {skill:this.skill, attacker:this.attacker, target:target}), 3)
...
const RPG_NPC = function(female) {
  ...
  res.examine = function() {
    ...
    if (this.dead) {
      if (this.exDead) {
        ...
        s = typeof this.ex === 'string' ? this.ex : this.ex() + lang.deadAddendum
      }
    }
    else if (this.asleep) {
      if (this.exAsleep) {
  ...
  res.search = function() {
    //const level = this.level || (this.maxHealth / 5
  }

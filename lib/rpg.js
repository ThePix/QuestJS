...
settings.afterTurn.push(function() {
  ...
  if (currentLocation.activeEffects) {
    for (const effectName of currentLocation.activeEffects) {
      const effect = skills.findEffect(effectName)
...
class Skill {
  constructor(name, data) {
    ...
    //log(this.alias)
    if (data.effect) {
      new Effect(this.name, this.effect, this.element)
      this.targetEffectName = true
    }
  }

  targetEffect(attack, target) {
    if (!this.targetEffectName) return
    const effect = skills.findEffect(this.targetEffectName === true ? this.name : this.targetEffectName)
    if (effect.start) attack.msg(effect.start(target), 1)
    if (this.duration) target['countdown_' + effect.name] = this.duration
    target.activeEffects.push(effect.name)
  }

  testUseable(char) { return skills.defaultSkillTestUseable(char) }
...
class Spell extends Skill {
  ...
  testUseable(char) { return skills.defaultSpellTestUseable(char) }
...
class SpellSummon extends Spell {
  ...
  targetEffect(attack) {
    const item = cloneObject(w[this.prototype], attack.attacker.loc)
    attack.msg("A " + item.alias + " appears before you.", 1)
  }
...
const skills = {
  ...
  effectsList:[],
  add:function(skill) {
    if (skill instanceof Effect) {
      this.effectsList.push(skill)
    }
    else {
  ...
  /*addEffectToTarget:function(target, effectName, turns) {
    target.activeEffects.push(effectName)
    if (turns) target['countdown_' + effectName] = turns
  },*/
  defaultSpellTestUseable:function(name) { return true },
  defaultSkillTestUseable:function(name) { return true },
  
  
  // Use this to, eg, link in a set of skills in the UI
  //getSkill:function() { return skillUI.getSkillFromButtons },
...
class Attack {
  static createAttack(attacker, target, skill) {
    ...
    // Find the skill
    if (attacker === player && skill === undefined && skills.getSkill) {
      attack.skill = skills.getSkill()
    ...
    if (attack.primaryTargets.length === 0) {
      return falsemsg(attack.skill.msgNoTarget ? attack.skill.msgNoTarget : 'No target found for attack')
    }
  ...
  resolve(target, isPrimary, count) {
    ...
    if (this.skill.incompatible) {
      for (let name of target.activeEffects) {
        ...
        if (skill.incompatible === this.skill.incompatible) terminatingEffects.push(name)
      }
    }
    ...
    for (let name of terminatingEffects) {
      this.msg(skills.findEffect(name).terminate(target))
    }
  }
  ...
  applyActiveEffects(source, outgoing) {
    ...
    for (let el of source.activeEffects) {
      const effect = skills.findEffect(el)
...
const RPG_BEAST = function(female, aggressive) {
  ...
  res.canTalk = function() {
    ...
    return falsemsg(lang.cannotTalkToBeast, {item:this})
  }
...
const BLADE_WEAPON = function(damage) {
  const res = Object.assign({}, WEAPON(damage))
  res.weaponType = 'blade'
  return res;
}  

const CRUSH_WEAPON = function(damage) {
  const res = Object.assign({}, WEAPON(damage))
  res.weaponType = 'crush'
  return res;
}  

const POLEARM_WEAPON = function(damage) {
  const res = Object.assign({}, WEAPON(damage))
  res.weaponType = 'polearm'
  return res;
}  
...
const BOW_WEAPON = function(damage, ammo) {
  const res = Object.assign({}, LIMITED_USE_WEAPON(damage, ammo))
  res.weaponType = 'bow'
  return res;
}  

const FIREARM_WEAPON = function(damage, ammo) {
  const res = Object.assign({}, LIMITED_USE_WEAPON(damage, ammo))
  res.weaponType = 'firearm'
  return res;
}  

const SUMMONED = function(duration) {
  const res = {}
  res.summoned = true
  res.summonedCont
  return res;
      ...
      for (let name of el.activeEffects) {
        if (el['countdown_' + name]) {
          ...
          if (el['countdown_' + name] <= 0) {
            msg(skills.findEffect(name).terminate(el))
          }
        }
      }
    }

...
const EQUIPPABLE = function() {
  ...
  res.drop = function(options) {
    ...
    this.moveToFrom(options.char, options.char.loc, options.this.loc);
    return true;
  }
  ...
  res.unequip = function(options) {
    ...
    this.equipped = false
    msg("{nv:char:put:true} away {nm:weapon:the}.", options)
    return true;
  }
...
commands.push(new Cmd('CastSpell', {
  ...
  script:function(objects) {
    ...
    //return skills.castSpell(spell, player, player)
  },
}));

...
const commands = [
  ...
  new Cmd('Say', {
    ...
    script:function(arr) {
      ...
      if (GIVE_PLAYER_SAY_MSG) msg(nounVerb(game.player, arr[0], true) + ", '" + sentenceCase(arr[1]) + ".'");
      for (let chr of l) {
      ...
      if (GIVE_PLAYER_SAY_MSG) {
        ...
        msg(nounVerb(game.player, arr[0], true) + ", '" + sentenceCase(arr[1]) + ",' but no one seemed interested in what you say.");
      }
      return SUCCESS;
    },
    objects:[
...
function handleFillWithLiquid(char, objects) {
  ...
  if (!container.vessel) return failedmsg(NOT_VESSEL(char, container));
  if (container.closed) return  failedmsg(CONTAINER_CLOSED(char, container));
  if (!char.canManipulate(objects[0], "fill")) return FAILED;

  const obj = objects[0][0]
  if (!char.getAgreement("Fill", obj)) continue;
  if (container.testRestrictions && !container.testRestrictions(obj, char)) return;
  if (!obj.isAtLoc(char.name)) return failedmsg(NOT_CARRYING(char, obj));
  
  obj.moveToFrom(container.name, char.name);
  msg(DONE_MSG);
  if (container.putInResponse) container.putInResponse();
  return SUCCESS;
}

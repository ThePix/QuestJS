...
const commands = [
    ...
    script:function(objects, matches) {
      ...
      if (obj.useDefaultsTo) {
        ...
        if (cmd) {
          ...
          throw new Error("Use defaulting to unknown command " + obj.useDefaultsTo())
        }
      }
  ...
  new Cmd('UseWith', {
    ...
    script:function(objects, matches) {
      ...
      if (item1.useWith) {
        const result = item1.useWith(char, item2)
        return result ? world.SUCCESS : world.FAILED
      }
      if (item2.withUse) {
        const result = item2.withUse(char, item1)
        return result ? world.SUCCESS : world.FAILED
      }
      if (item1.useWithDefaultsTo) {
        const result = item1[item1.useWithDefaultsTo](char, item2)
        return result ? world.SUCCESS : world.FAILED
      }
      if (item2.withUseDefaultsTo) {
        const result = item2[item2.withUseDefaultsTo](char, item1)
        return result ? world.SUCCESS : world.FAILED
      }
      else {
        this.default(objects[0][i], false, game.player)
        return world.FAILED; 
      }
    },
    defmsg:lang.cannot_use,

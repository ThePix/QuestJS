...
const commands = [
  ...
  new Cmd('Inv', {
    script:function() {
      const listOfOjects = game.player.getContents(world.ALL);
    ...
    script:function() {
      ...
      for (let key in w) {
        if (parser.isForSale(w[key])) {
          ...
          const row = [sentenceCase(w[key].byname()), world.Money(price)]
  ...
  new Cmd('Drop', {
    ...
    defmsg:function(char, item) {
      if (item.isAtLoc(char)) return lang.cannot_drop(char, item)
      return lang.not_carrying(char, item)
    }
  }),
  new Cmd('Wear2', {
    npcCmd:true,
...
if (settings.debug) {
  commands.push(new Cmd('DebugWalkThrough', {
    ...
    script:function(objects) {
      ...
      if (wt === undefined) {
        debugmsg("No walkthrough found called " + objects[0]);
        return;
      }
      for (let el of wt) {
        if (typeof el === "string") {
...
const handleTieTo = function(char, rope, obj2) {
  if (!rope.rope) return failedmsg("You cannot tie that to anything.")
  if (!obj2.attachable) return failedmsg("That is not something you can tie the {nm:rope:the} to.", {rope:rope})
  ...
  if (rope.tiedTo1 && rope.tiedTo2) return failedmsg("It is already tied to {nm:rope:the} and {nm:rope:the}.", {rope:w[rope.tiedTo1], obj2:w[rope.tiedTo2]})

  rope[rope.tiedTo1 ? 'tiedTo2' : 'tiedTo1'] = obj2.name
  msg("{nv:char:tie:true} {nm:rope:the} to {nm:obj2:the}.", {char:char, rope:rope, obj2:obj2})
  if (rope.onTie) rope.onTie(obj2)
  return world.SUCCESS
}

const handleUntieFrom = function(char, rope, obj2) {
  if (rope !== w.rope) return failedmsg("You cannot tie - or untie - that to anything.")
  if (obj2 === undefined) {
    if (!rope.tiedTo1 && !rope.tiedTo2) return failedmsg("{nv:rope:be:true} not tied to anything.", {rope:rope})
    if (rope.tiedTo1 && !rope.tiedTo2) {
      ...
      return failedmsg("Which end of {nm:rope:the} do you want untied?", {rope:rope})
    }
  }
  else {
    if (rope.tiedTo1 !== obj2.name && rope.tiedTo2 !== obj2.name) {
      return failedmsg("{nv:rope:be:true} not tied to {nm:obj2:the}.", {rope:rope, obj2:obj2})
    }
  }
  
  delete rope[rope.tiedTo1 === obj2.name ? 'tiedTo1' : 'tiedTo2']
  msg("{nv:char:untie:true} {nm:rope:the} from {nm:obj2:the}.", {char:char, rope:rope, obj2:obj2})
  if (rope.onUntie) rope.onUntie(obj2)
  return world.SUCCESS
}

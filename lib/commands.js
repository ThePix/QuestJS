...
const commands = [
  ...
  new Cmd('MetaTranscriptOn', {
    script: function () {
      if (game.transcript) {
        io.metamsg(lang.transcript_already_on)
        return util.FAILED
      }
      game.scriptStart()
      return util.util.SUCCESS_NO_TURNSCRIPTS
  ...
  new Cmd('MetaTranscriptOff', {
    script: function () {
      if (!game.transcript) {
        io.metamsg(lang.transcript_already_off)
        return util.FAILED
      }
      game.scriptEnd()
      return util.util.SUCCESS_NO_TURNSCRIPTS
  ...
  new Cmd('Smell', {
    script: function () {
      if (game.room.onSmell) {
        ...
        io.msg(no_smell(game.player)) //  -review: Way send a function to the messenger
  ...
  new Cmd('PurchaseFromList', {
    script: function () {
      ...
      io.msg(current_money + ': ' + util.util.displayMoney(game.player.money))
      io.msgTable(l, buy_headings)
      return util.util.SUCCESS_NO_TURNSCRIPTS
    }
  }),
  ...
  new Cmd('GetOff', {
    ...
    rules: [cmdRules.canPosture, cmdRules.isHereNotHeld],
    attName: 'getoff',
    cmdCategory: 'Posture', // -fixme: duplicate key
    objects: [
      { scope: parser.isHere, attName: 'assumePosture' }
    ],
    defmsg: lang.already
  ...
  new Cmd('TalkTo', {
    ...
    default: function (item) {
      return io.failedio.msg(cannot_talk_to(game.player, item))
    }
  }),
  ...
  new Cmd('AskAbout', {
    ...
    script: function (arr) {
      ...
      if (!arr[0][0].askabout) return io.failedio.msg(cannot_ask_about(game.player, arr[0][0], arr[1]))
  ...
  new Cmd('TellAbout', {
    ...
    script: function (arr) {
      if (!game.player.canTalk()) return false
      if (!arr[0][0].tellabout) return io.failedio.msg(cannot_tell_about(game.player, arr[0][0], arr[1]))
...
if (settings.debug) {
  commands.push(new Cmd('DebugWalkThrough', {
    ...
    script: function (objects) {
      ...
      for (el of wt) { // -fixme: "el" needs assingment -> const/let
        io.debugmsg(el)
        parser.parse(el)
      }
    }

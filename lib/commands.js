...
function handleFillWithLiquid(char, vessel, liquid) {
  if (!vessel.vessel) return failedmsg(NOT_VESSEL(char, vessel));
  if (vessel.closed) return  failedmsg(CONTAINER_CLOSED(char, vessel));
  if (!char.canManipulate(vessel, "fill")) return FAILED;
  if (!char.getAgreement("Fill", vessel)) return FAILED;
  if (!vessel.isAtLoc(char.name)) return failedmsg(NOT_CARRYING(char, vessel));
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(char.name)) {
        failedmsg(prefix(obj, multiple) + NOT_CARRYING(char, obj));
      }
      else {
        obj.moveToFrom(container.name, char.name);
        msg(prefix(obj, multiple) + DONE_MSG);
        success = true;
      }
    }
  }
  ...
  for (let obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(container.name)) {
        failedmsg(prefix(obj, multiple) + NOT_INSIDE(container, obj));
      }
      else {

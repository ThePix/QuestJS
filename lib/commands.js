...
const commands = [
  ...
  new Cmd('Use', {
    ...
    script:function(objects, matches) {
      ...
      for (let i = 0; i < objects[0].length; i++) {
        if (objects[0][i].use) {
          let result = this.processCommand(game.player, objects[0][i], multi, matches[0][i], 'use')
          success = result || success;
        }
        else if (objects[0][i].useDefaultsTo) {
          let result = this.processCommand(game.player, objects[0][i], multi, matches[0][i], objects[0][i].useDefaultsTo);
          success = result || success
        }
        else {
  ...
  new Cmd('UseWith', {
    ...
    script:function(objects, matches) {
      /*
      const item1 = objects[0][0]
      const item2 = objects[1][0]
      if (item1.useWith) {
        return item1.useWith(char, item2)
      }
      if (item2.withUse) {
        return item2.withUse(char, item1)
      }
      else if (item1.useWithDefaultsTo) {
        return item1[item1.useWithDefaultsTo](char, item2)
        let result = this.processCommand(game.player, objects[0][i], multi, matches[0][i], objects[0][i].useDefaultsTo);
        success = result || success
      }
      else {

...
const commands = [
  ...
  new Cmd('Map', {
    script: function () {
      if (typeof showMap !== 'undefined') {
        ...
        return io.failedmsg(lang.no_map)
      }
    }
  }),
  new Cmd('Smell', {
  ...
  new Cmd('PurchaseFromList', {
    script: function () {
      ...
      if (l.length === 0) {
        return io.failedmsg(lang.nothing_for_sale)
      }
  ...
  new Cmd('TalkTo', {
    ...
    default: function (item) {
      return io.failedmsg(cannot_talk_to(game.player, item))
    }
  }),
  ...
  new Cmd('Topics', {
    ...
    default: function (item) {
      return io.failedmsg(lang.no_topics(game.player, item))
    }
  }),
  ...
  new Cmd('AskAbout', {
    ...
    script: function (arr) {
      ...
      if (!arr[0][0].askabout) return io.failedmsg(cannot_ask_about(game.player, arr[0][0], arr[1]))
  ...
  new Cmd('TellAbout', {
    ...
    script: function (arr) {
      if (!game.player.canTalk()) return false
      if (!arr[0][0].tellabout) return io.failedmsg(cannot_tell_about(game.player, arr[0][0], arr[1]))
...
function handleFillWithLiquid (char, vessel, liquid) {
  if (!vessel.vessel) return io.failedmsg(lang.not_vessel(char, vessel))
  if (vessel.closed) return io.failedmsg(lang.container_closed(char, vessel))
  if (!char.canManipulate(vessel, 'fill')) return FAILED
  if (!char.getAgreement('Fill', vessel)) return FAILED
  if (!vessel.isAtLoc(char.name)) return io.failedmsg(lang.not_carrying(char, vessel))
...
function handlePutInContainer (char, objects) {
  ...
  if (!container.container) {
    io.failedmsg(lang.not_container(char, container))
    return FAILED
  }
  if (container.closed) {
    io.failedmsg(lang.container_closed(char, container))
    return FAILED
  }
  ...
  for (const obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(char.name)) {
        io.failedmsg(prefix(obj, multiple) + lang.not_carrying(char, obj))
      } else {
  ...
  for (const obj of objects[0]) {
    ...
    if (flag) {
      if (!obj.isAtLoc(container.name)) {
        io.failedmsg(prefix(obj, multiple) + lang.not_inside(container, obj))
      } else {
...
function handleGiveToNpc (char, objects) {
  ...
  if (!npc.npc && npc !== game.player) {
    io.failedmsg(lang.not_npc_for_give(char, npc))
    return FAILED
  }
  for (const obj of objects[0]) {
    let flag = true

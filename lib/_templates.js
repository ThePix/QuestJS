...
const TAKEABLE_DICTIONARY = {
  ...
  drop:function(options) {
    ...
    this.moveToFrom(options.char, options.char.loc, options.char.name);
    return true;
  },
...
const createEnsemble = function(name, ensembleMembers, dict) {
  ...
  res.drop = function(options) {
    ...
    msg(this.msgDrop, options);
    for (let member of this.ensembleMembers) {
      member.moveToFrom(options.char, options.char.loc);
    }
    return true;
  }
...
const MERCH = function(value, locs) {
  const res = {
    ...
    purchase:function(options) {
      ...
      this.purchaseScript(options, options.char, cost)
    },
    
    purchaseScript:function(options, char, cost) {
      char.money -= cost;
      msg(lang.purchase_successful, options);
      if (this.doNotClone) {
    ...
    sell:function(options) {
      ...
      msg(lang.sell_successful, options);
      if (this.doNotClone) {
        this.moveToFrom(options.char, options.char.loc, options.char.name);
...
const COUNTABLE = function(countableLocs) {
  ...
  res.take = function(options) {
    ...
    this.giveTo(options.char.name, n)
    if (this.scenery) this.scenery = false
    return true
  }
  ...
  res.drop = function(options) {
    let n = this.extractNumber();
    let m = this.countAtLoc(options.char.name);
    if (m === 0) {
      msg(lang.none_held, options);
      return false;
    }
    const dest = w[options.char.loc]
    ...
    msg(this.msgDrop, options);
    this.takeFrom(options.char.name, n);
    this.giveTo(options.char.loc, n);
    return true;
  };
...
const CONTAINER = function(openable) {
  ...
  res.openMsg = function(options) {
    ...
    msg(this.msgOpen + " " + (options.list === lang.list_nothing ? lang.it_is_empty : lang.look_inside), options)
  };
...
const VESSEL = function() {
  ...
  res.doEmpty = function(options) {
    ...
    else if (options.sink.vessel) {
      if (options.sink.containedFluidName) return falsemsg(lang.already_full, {char:options.char, item:options.sink, fluid:options.sink.containedFluidName})
      msg(lang.empty_into_successful, options)
      options.sink.containedFluidName = this.containedFluidName
    }
    else if (options.sink.sink) {
      if (!options.sink.sink(this.containedFluidName, options.char, this)) return false
    }
    else {
      msg(lang.empty_onto_successful, options)
    }
    if (this.afterEmpty) this.afterEmpty(options.char, {fluid:this.containedFluidName, sink:options.sink})
...
const TRANSIT_BUTTON = function(nameOfTransit) {
  ...
  res.push = function(options) {
    ...
    if (transit.transitAutoMove) {
      player.moveChar(player.previousLoc, transit[transit.transitDoorDir])
    }
    else {
...
const TRANSIT = function(exitDir) {
  const res = {
    ...
    getDestLocation:function() { return w[this[this.transitDoorDir].name] },
    getDestButton:function() { return w[this.currentButtonName] },
    ...
    transitOfferMenu:function() {
      if (this.testTransit && !this.testTransit(player)) {
        if (this.transitAutoMove) player.moveChar(player.previousLoc, this[this.transitDoorDir])
        return false
      }

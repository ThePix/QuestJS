...
const COUNTABLE = function(countableLocs) {
  ...
  res.takeFrom = function(loc, count) {
    ...
    w[loc].afterItemTakenFrom(player, {item:this, count:count})
  };
  
  res.giveTo = function(loc, count) {
    if (!this.countableLocs[loc]) { this.countableLocs[loc] = 0; }
    if (this.countableLocs[loc] !== 'infinity') this.countableLocs[loc] += count;
    w[loc].afterItemDroppedHere(player, {item:this, count:count});
  };
...
const ROPE = function(tetheredTo) {
  ...
  res.moveToFrom = function(char, toLoc, fromLoc) {
    ...
    w[fromLoc].afterItemTakenFrom(char, {item:this, end:end});
    w[toLoc].afterItemDroppedHere(char, {item:this, end:end});

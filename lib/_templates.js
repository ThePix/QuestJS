...
const TAKEABLE_DICTIONARY = {
  ...
  drop:function(multiple, char) {
    if (this.testDrop && !this.testDrop(char, {multiple:multiple})) return false
    const dest = w[char.loc]
    if (dest.testDropIn && !dest.testDropIn(char, {multiple:multiple, item:this})) return false

    const tpParams = {char:char, item:this}
    msg(prefix(this, multiple) + this.msgDrop, tpParams);
    this.moveToFrom(char, char.loc, char.name);
    return true;
  },
  ...
  take:function(multiple, char) {
    ...
    if (this.testTake && !this.testTake(char, {multiple:multiple})) return false
    if (w[char.loc].testTakeOut && !w[char.loc].testTakeOut(char, {multiple:multiple, item:this})) return false
    
    msg(prefix(this, multiple) + this.msgTake, tpParams)
    this.moveToFrom(char, char.name, this.takeFromLoc(char))
...
const createEnsemble = function(name, ensembleMembers, dict) {
  ...
  res.drop = function(multiple, char) {
    const dest = w[char.loc]
    if (dest.testDrop && !dest.testDrop(char, {multiple:multiple})) return false
    if (dest.testDropIn && !dest.testDropIn(char, {multiple:multiple, item:this})) return false
    const tpParams = {char:char, item:this}
    msg(prefix(this, multiple) + this.msgDrop, tpParams);
    for (let member of this.ensembleMembers) {
      member.moveToFrom(char, char.loc);
    }
    return true;
  }
  ...
  res.take = function(multiple, char) {
    ...
    msg(prefix(this, multiple) + this.msgTake, tpParams);
    for (let member of this.ensembleMembers) {
      member.moveToFrom(char, char.name);
...
const MERCH = function(value, locs) {
  const res = {
    ...
    purchaseScript:function(multiple, char, cost, tpParams) {
      ...
      if (this.doNotClone) {
        this.moveToFrom(char, char.name, char.loc);
        this.salesLoc = false
      }
      else {
...
const COUNTABLE = function(countableLocs) {
  ...
  res.moveToFrom = function(char, toLoc, fromLoc, count) {
  ...
  res.take = function(multiple, char) {
    ...
    if (this.testTake && !this.testTake(char, {multiple:multiple, count:n})) return false
    if (w[sourceLoc].testTakeOut && !w[sourceLoc].testTakeOut(char, {multiple:multiple, item:this, count:n})) return false
  ...
  res.drop = function(multiple, char) {
    ...
    if (this.testDrop && !this.testDrop(char, {multiple:multiple, count:n})) return false
    if (dest.testDropIn && !dest.testDropIn(char, {multiple:multiple, destination:dest, count:n})) return false
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.testWear = function(char) { return true }
  res.testRemove = function(char) { return true }
  
  res._canWearRemove = function(char, toWear, multiple) {
    if (toWear) {
      if (!this.testWear(char, {multiple:multiple})) return false
    }
    else {
      if (!this.testRemove(char, {multiple:multiple})) return false
    }
    const outer = this.getWearRemoveBlocker(char, toWear)
    if (outer) {
      const tpParams = {char:char, item:this, outer:outer}
      if (toWear) {
  ...
  res.wear = function(multiple, char) {
    ...
    msg(prefix(this, multiple) + this.msgWear, {item:this, char:char})
    this.worn = true
  ...
  res.remove = function(multiple, char) {
    ...
    msg(prefix(this, multiple) + this.msgRemove, {item:this, char:char});
    this.worn = false;
...
const OPENABLE_DICTIONARY = {
  ...
  openMsg:function(multiple, tpParams) { msg(prefix(this, multiple) + this.msgOpen, tpParams) },
  open:function(multiple, char) {
    ...
    else if (this.testOpen && !this.testOpen(char, {multiple:multiple})) {
      return false
    }
    if (this.locked) {
      if (this.testKeys(char)) {
        this.locked = false
        this.closed = false
        msg(prefix(this, multiple) + this.msgUnlock, tpParams)
  ...
  close:function(multiple, char) {
    ...
    else if (this.testClose && !this.testClose(char, {multiple:multiple})) {
      return false
    }
    this.closed = true;
    msg(prefix(this, multiple) + this.msgClose, tpParams)
...
const CONTAINER = function(openable) {
  ...
  res.openMsg = function(multiple, tpParams) {
    ...
    msg(prefix(this, multiple) + this.msgOpen + " " + (tpParams.list === lang.list_nothing ? lang.it_is_empty : lang.look_inside), tpParams)
  };
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.msgOpenMsg = function(multiple, tpParams) {
  ...
  const res = {
    ...
    lock:function(multiple, char) {
      ...
      if (!this.closed) {
        this.closed = true
        this.locked = true
        msg(this.msgCloseAndLock, tpParams)
      }
      else {
        this.locked = true
        msg(this.msgLock, tpParams)
      }
      if (this.afterLock) this.afterLock(char, {})
      return true
    },
    ...
    unlock:function(multiple, char) {
      ...
      msg(this.msgUnlock, tpParams)
      this.locked = false
      if (this.afterUnlock) this.afterUnlock(char, {})
      return true
    },
...
const FURNITURE = function(options) {
  const res = {
    testPostureOn:(char, {posture:posture}) => true,
  ...
  res.assumePosture = function(multiple, char, posture, name, adverb) {
    ...
    if (!this.testPostureOn(char, {posture:posture})) return false
    ...
    if (this.afterPostureOn) this.afterPostureOn(char, {posture:posture})
    return true
  }
...
const SWITCHABLE = function(alreadyOn, nameModifier) {
  ...
  res.switchon = function(multiple, char) {
    ...
    if (!this.testSwitchOn(char, {multiple:multiple})) return false
    if (!char.getAgreement("SwitchOn", this, true)) return false
    
    if (!this.suppressMsgs) char.msg(this.msgSwitchOn, tpParams);
    this.doSwitchon(char);
    return true;
  };
  ...
  res.doSwitchon = function() {
    ...
    if (this.afterSwitchOn) this.afterSwitchOn(char, {})
  };
  ...
  res.switchoff = function(multiple, char) {
    ...
    if (!this.testSwitchOff(char, {multiple:multiple})) return false
    if (!char.getAgreement("SwitchOn", this, false)) return false

    if (!this.suppressMsgs) char.msg(this.msgSwitchOff, tpParams);
    this.doSwitchoff(char);
    return true;
  };
  
  res.doSwitchoff = function(char) {
    let lighting = game.dark;
    ...
    if (this.afterSwitchOff) this.afterSwitchOff(char, {})
  };
...
const EDIBLE = function(isLiquid) {
  ...
  res.eat = function(multiple, char) {
    ...
    msg(prefix(this, multiple) + this.msgIngest, tpParam);
    this.loc = null;
    if (this.afterIngest) this.afterIngest(char, {});
    return true;
  };
...
const VESSEL = function() {
  ...
  res.findSource = function(chr, fluid) { return util.findSource(chr, fluid) },
  ...
  res.doFill = function(multiple, char, source, fluid) {
    ...
    if (this.testFill && !this.testFill(char, {multiple:multiple}, {fluid:fluid})) return false
    ...
    if (this.afterFill) this.afterFill(char, {fluid:this.containedFluidName, source:source})
    return true
  }
  ...
  res.doEmpty = function(multiple, char, sink) {
    ...
    if (!this.containedFluidName) return falsemsg(lang.already_empty, tpParams)
    if (this.testEmpty && !this.testEmpty(char, {multiple:multiple}, {fluid:this.containedFluidName})) return false

    if (!sink) {
      msg(lang.empty_successful, tpParams)
    }
    else if (sink.vessel) {
      if (sink.containedFluidName) return falsemsg(lang.already_full, {char:char, item:sink, fluid:sink.containedFluidName})
      msg(lang.empty_into_successful, tpParams)
      sink.containedFluidName = this.containedFluidName
    }
    else if (sink.sink) {
      if (!sink.sink(this.containedFluidName, char, this)) return false
    }
    else {
      msg(lang.empty_onto_successful, tpParams)
    }
    if (this.afterEmpty) this.afterEmpty(char, {fluid:this.containedFluidName, sink:sink})
...
const ROPE = function(length, tetheredTo) {
  const res = $.extend({
    ...
    simpleIsAtLoc:function(loc, situation) {
      if (this.loc) {
        this.locs = [this.loc]
        this.loc = false
      }
    ...
    handleTieTo:function(char, obj) {
      ...
      if (!obj.attachable) return failedmsg(lang.rope_not_attachable_to, tpParams)
      ...
      if (!this.suppessMsgs) msg(this.msgAttach, tpParams)
      return world.SUCCESS
    },
  ...
  res.moveToFrom = function(char, toLoc, fromLoc) {
...
/*  res.takeFromLoc = function(char) {
  ...
  res.drop = function(multiple, char) {
    const tpParams = {char:char, item:this}
    if (!this.isAtLoc(char.name)) return falsemsg(prefix(this, multiple) + lang.not_carrying, tpParams)
    ...
    msg(prefix(this, multiple) + this.msgDrop, tpParams)
    if (w[char.loc].afterDropIn) w[char.loc].afterDropIn(char, {item:this, end:end});
    if (w[char.name].afterTakeFrom) w[char.name].afterTakeFrom(char, {item:this, end:end});
    if (this.afterMove !== undefined) this.afterMove(char, {toLoc:char.name, fromLoc:char.loc, end:end})
    return true;
  }
  
  res.take = function(multiple, char) {
    const tpParams = {char:char, item:this}
    if (this.isAtLoc(char.name) && !this.isAtLoc(char.loc)) return falsemsg(prefix(this, multiple) + lang.already_have, tpParams)
    if (!char.canManipulate(this, "take")) return false;
    if (this.tiedTo1 && this.tiedTo2) return falsemsg(prefix(this, multiple) + lang.rope_tied_both_end, tpParams)
    ...
    if (this.locs.length === 1 && !this.tiedTo1 && !this.tiedTo2) {
      ...
      return falsemsg(prefix(this, multiple) + lang.rope_tied_one_end, tpParams)
    }
    else {
      return falsemsg(prefix(this, multiple) + lang.rope_no_end, tpParams)
    }
    
    msg(prefix(this, multiple) + this.msgTake, tpParams)
    if (w[char.loc].afterTakeOut) w[char.loc].afterTakeOut(char, {item:this, end:end});
    if (w[char.name].afterDropIn) w[char.name].afterDropIn(char, {item:this, end:end});
    if (this.afterMove !== undefined) this.afterMove(char, {toLoc:char.name, fromLoc:char.loc, end:end})
    if (this.scenery) this.scenery = false
    return true;
  }
  
  res.testCarry = function(char, options) {
    ...
    msg(lang.rope_cannot_move, {item:this, char:char})
    return false
  }
...
const BUTTON = function() {
  ...
  res.push = function(multiple, char) {
    const tpParams = {char:char, item:this}
    msg(this.msgPress, tpParams)
    if (this.afterPress) this.afterPress(char, {})
  }
...
const TRANSIT_BUTTON = function(nameOfTransit) {
  ...
  res.push = function(multiple, char) {
    ...
    if (this.testTransitButton && !this.testTransitButton(char, {multiple:multiple, transit:transit})) return false
    if (transit.testTransit && !transit.testTransit(char, {multiple:multiple, button:this})) return false
  ...
  const res = {
    ...
    moveChar:function(roomName, exit) {
      ...
      for (const el of this.getCarrying()) {
        if (el.afterCarry) el.afterCarry(this, {exit:exit})
      }
    },
    ...
    handleMovingFollowers:function(room, exit) {
      for (let s of this.followers) {
        ...
        if (!follower.testFollowTo || follower.testFollowTo(room)) {

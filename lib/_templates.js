...
const TAKEABLE_DICTIONARY = {
  ...
  drop:function(isMultiple, char) {
    if (this.testDropRestrictions && !this.testDropRestrictions(char, isMultiple)) return false
    const dest = w[char.loc]
    if (dest.testDropInRestrictions && !dest.testDropInRestrictions(char, isMultiple, this)) return false

    const tpParams = {char:char, item:this}
    msg(prefix(this, isMultiple) + this.getDropMsg(char), tpParams);
  ...
  take:function(isMultiple, char) {
    ...
    if (this.testTakeRestrictions && !this.testTakeRestrictions(char, isMultiple)) return false
    if (w[char.loc].testTakeOutRestrictions && !w[char.loc].testTakeOutRestrictions(char, multiple, this)) return false
    
    msg(prefix(this, isMultiple) + this.getTakeMsg(char), tpParams)
...
const createEnsemble = function(name, ensembleMembers, dict) {
  ...
  res.drop = function(isMultiple, char) {
    const dest = w[char.loc]
    if (dest.testDropRestrictions && !dest.testDropRestrictions(char, isMultiple)) return false
    if (dest.testDropInRestrictions && !dest.testDropInRestrictions(char, isMultiple, this)) return false
    const tpParams = {char:char, item:this}
    msg(prefix(this, isMultiple) + this.getDropMsg(char), tpParams);
    for (let member of this.ensembleMembers) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!char.canManipulate(this, "take")) return false;
    if (this.testTakeRestrictions && !this.testTakeRestrictions(char, isMultiple)) return false
    if (w[char.loc].testTakeOutRestrictions && !w[char.loc].testTakeOutRestrictions(char, isMultiple, obj)) return false
    msg(prefix(this, isMultiple) + this.getTakeMsg(char), tpParams);
    for (let member of this.ensembleMembers) {
...
const COUNTABLE = function(countableLocs) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!sourceLoc) return falsemsg(prefix(this, isMultiple) + lang.none_here, tpParams)
    ...
    if (this.testTakeRestrictions && !this.testTakeRestrictions(char, isMultiple, n)) return false
    if (w[sourceLoc].testTakeOutRestrictions && !w[sourceLoc].testTakeOutRestrictions(char, isMultiple, this, n)) return false
    
    tpParams[this.name + '_count'] = n
    msg(prefix(this, isMultiple) +  this.getTakeMsg(char, n), tpParams)
  ...
  res.drop = function(isMultiple, char) {
    ...
    if (this.testDropRestrictions && !this.testDropRestrictions(char, isMultiple, n)) return false
    if (dest.testDropInRestrictions && !dest.testDropInRestrictions(char, isMultiple, dest, n)) return false
    
    tpParams[this.name + '_count'] = n
    msg(prefix(this, isMultiple) + this.getDropMsg(char, n), tpParams);
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res._canWearRemove = function(char, toWear, isMultiple) {
    if (toWear) {
      if (!this.testWearRestrictions(char, isMultiple)) return false
    }
    else {
      if (!this.testRemoveRestrictions(char, isMultiple)) return false
    }
  ...
  res.wear = function(isMultiple, char) {
    if (!this._canWearRemove(char, true, isMultiple)) return false
    if (!char.canManipulate(this, "wear")) return false
    if (this.wearMsg) {
      msg(prefix(this, isMultiple) + this.wearMsg(char, this), {garment:this, actor:char})
    }
    else {
      msg(prefix(this, isMultiple) + this.getWearMsg(char), {garment:this, char:char})
    }
    this.worn = true
  ...
  res.remove = function(isMultiple, char) {
    if (!this._canWearRemove(char, false, isMultiple)) return false
    ...
    if (this.removeMsg) {
      ...
      msg(prefix(this, isMultiple) + this.getRemoveMsg(char), {garment:this, char:char});
    }
    this.worn = false;
...
const VESSEL = function() {
  ...
  res.fill = function(isMultiple, chr) {
    const [source, fluid] = this.findSource(chr)
    if (!source) return falsemsg(lang.no_generic_fluid_here, {item:this});
    return this.doFill(isMultiple, chr, source, fluid)
  },
  
  res.doFill = function(isMultiple, char, source, fluid) {
    const tpParams = {char:char, item:this, fluid:fluid}
    if (this.testFillRestrictions && !this.testFillRestrictions(char, fluid, isMultiple)) return false
  ...
  res.empty = function(isMultiple, char) {
    return this.doEmpty(isMultiple, char)
  }

  res.doEmpty = function(isMultiple, char, sink) {
...
const TRANSIT_BUTTON = function(nameOfTransit) {
  ...
  res.push = function(isMultiple, char) {
    ...
    if (typeof transit.transitCheck === "function" && !transit.transitCheck()) {
      if (transit.transitAutoMove) game.player.moveChar(game.player.previousLoc, transit.transitDoorDir)
      return false;
    }

    const exit = transit[transit.transitDoorDir];
    if (this.transitCheck && !this.transitCheck()) {
      return false;
    }
    if (this.locked) {
  ...
  const res = {
    ...
    transitOfferMenu:function() {
      if (typeof this.transitCheck === "function" && !this.transitCheck()) {
        if (this.transitAutoMove) game.player.moveChar(game.player.previousLoc, this.transitDoorDir)
        return false;
      }
        ...
        for (let button of buttons) {
          if (buttons[i].transitDestAlias === result) {
            if (room[transitDoorDir].name === button.transitDest) {
              ...
              transit.update(button, true)
            }
            
            room[transitDoorDir].name = button.transitDest
            if (room.transitAutoMove) game.player.moveChar(button.transitDest, room[transitDoorDir])
          }
        }
      })
    },
...
const CHARACTER = function() {
  const res = {
    ...
    // Use this to move the NPC
    // if this is an NPC and dir is set, the player is told about it, if in the room
    // The player movement message is handled earlier, with the exit
    moveChar:function(roomName, dir) {
      ...
      if (this.player) {
        ...
        //if (dir) msg(lang.stop_posture(this))
        //if (dir) lang.npc_leaving_msg(this, room)
        this.loc = room.name
        //if (dir) lang.npc_entering_msg(this, this.previousLoc)
        this.handleMovingFollowers(room)
      }

      for (let el of this.afterCarryList) {
        w[el].afterCarry(this, room, dir)
      }
    },
  ...
  handleMovingFollowers:function(room, dir) {
      for (let s of this.followers) {
        ...
        if (!follower.testFollowToRestrictions || follower.testFollowToRestrictions(room)) {
          follower.moveChar(room.name, dir)
        }
      }
    }
  }

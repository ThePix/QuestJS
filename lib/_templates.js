...
const ROPE = function(tetheredTo) {
  const res = $.extend({
    ...
    isUltimatelyHeldBy:function(obj) {
      ...
      if (this.locs.length > 1) {
        let o = w[this.locs[this.locs.length - 1]]
        if (o === obj) return true
        while (o.loc) {
          if (o.loc === obj.name) return true
          o = w[o.loc]
  ...
  res.moveToFrom = function(char, toLoc, fromLoc) {
    ...
    w[fromLoc].afterDropIn(char, {item:this, end:end});
    w[toLoc].afterDropIn(char, {item:this, end:end});
    if (this.afterMove !== undefined) this.afterMove(char, {toLoc:toLoc, fromLoc:fromLoc, end:end})
  }
  ...
  res.take = function(multiple, char) {
    ...
    this.moveToFrom(char, char.name, char.loc) // !!! assuming where it is going from
    if (this.scenery) this.scenery = false
    return true;
  }

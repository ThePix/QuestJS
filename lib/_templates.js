...
const VESSEL = function() {
  ...
  res.doEmpty = function(options) {
    ...
    else if (options.obj.vessel) {
      if (options.obj.containedFluidName) return falsemsg(lang.already_full, {char:options.char, item:options.sink, fluid:options.obj.containedFluidName})
      msg(lang.empty_into_successful, options)
      options.obj.containedFluidName = this.containedFluidName
    }
    else if (options.obj.sink) {
      if (!options.obj.sink(this.containedFluidName, options.char, this)) return false
    }
    else {
      msg(lang.empty_onto_successful, options)
    }
    if (this.afterEmpty) this.afterEmpty(options.char, {fluid:this.containedFluidName, sink:options.obj})

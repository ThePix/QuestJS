...
const OPENABLE = function(alreadyOpen) {
  ...
  res.msgClose = lang.close_successful_item
  res.msgOpen = lang.open_successful_item
  res.msgLock = lang.lock_successful_item
  res.msgUnlock = lang.unlock_successful_item
  res.msgCloseAndLock = lang.close_and_lock_successful_item
...
const FURNITURE = function(options) {
  ...
  const res = {
    ...
    getoff:function(options) {
      if (!options.char.posture || options.char.posture === 'standing') {
  ...
  res.assumePosture = function(options, posture, name, adverb) {
    ...
    if (char.posture && char.posture !== 'standing' && char.postureFurniture !== this.name) {
      char.msg(stop_posture(char))
      char.msg(lang[name + '_on_successful'], options)
    }
    else if (char.posture && char.posture !== 'standing' && this[char.posture + "_to_" + posture] && this.postureChangesImplemented) {
...
const CHARACTER = function() {
  const res = {
    ...
    getAgreement:function(cmdType, options) {
      ...
      if (this.getAgreementDefault) return this.getAgreementDefault(cmdType, options)
      return true
    },
    ...
    getStatusDesc:function() {
      if (!this.posture || (this.posture === 'standing' && !this.postureFurniture)) return false;
    ...
    moveChar:function(exit) {
      ...
      // An exit can do some setting up
      // If it returns a string, that will be used as the exit destination, otherwise stick with what we have
      if (exit.beforeMove) exit_name = exit.beforeMove(this) || exit_name

      //log("Using moveChar for " + this.name + " going to " + exit_name)
      //log("X=" + this.positionX + " Y=" + this.positionY)
    ...
    movingMsg:function(exit) {
      if (this.player) {
        if (exit.msg) {
          ...
          msg(lang.go_successful, {char:this, dir:dir(exit), destination:w[exit.name], exit:exit})
        }
      }
      else {
        if (exit.msgNPC) {
    ...
    search:function(options) {
      return falsemsg(lang.search_npc_failure, options)
    }
  }
  return res
}

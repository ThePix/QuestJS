...
const createEnsemble = function(name, ensembleMembers, dict) {
  ...
  res.take = function(isMultiple, char) {
    ...
    if (!char.canManipulate(this, "take")) return false;
    if (this.testTakeRestrictions && !this.testTakeRestrictions(char)) return false
    msg(prefix(this, isMultiple) + this.getTakeMsg(char), tpParams);
    for (let member of this.ensembleMembers) {
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res._canWearRemove = function(char, toWear) {
    if (toWear) {
      if (!this.testWearRestrictions(char)) return false
    }
    else {
      if (!this.testRemoveRestrictions(char)) return false
    }
  ...
  res.wear = function(isMultiple, char) {
    if (!this._canWearRemove(char, true)) return false
  ...
  res.remove = function(isMultiple, char) {
    if (!this._canWearRemove(char, false)) return false
...
const VESSEL = function() {
  ...
  res.doFill = function(char, source, fluid) {
    ...
    if (this.testFillRestrictions && !this.testFillRestrictions(char, fluid)) return false

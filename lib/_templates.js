...
const CHARACTER = function() {
  const res = {
    ...
    // if dir is set, the player is ytold about it, if in the room
    moveChar:function(roomName, dir) {
      ...
      if (this.player) {
        ...
        this.loc = room.name
        // do followers here because we do them silently for NPCs
        for (let s of this.followers) {
          const follower = w[s]
          if (!follower.testFollowToRestrictions || follower.testFollowToRestrictions(room)) follower.loc = room.name
        }

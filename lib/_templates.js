...
const ROPE = function(tetheredTo) {
  ...
  res.testCarryRestrictions = function(char, exit) {
    ...
    if (this.locs[0] === char.name) {
      if (this.locs[2] === exit.name) return true // heading back where we came from
    }
    else {
      if (this.locs[this.locs.length - 3] === exit.name) return true // heading back where we came from
    }        

...
const saveLoad = {
  ...
  saveGame:function(filename, overwrite) {
    ...
    if (localStorage.getItem(this.getName(filename)) && !overwrite) {
      metamsg("File already exists. To overwrite an existing file, use SAVE [filename] OVERWRITE or SAVE [filename] OW.")
      return
    }
  ...
  decode:function(hash, str) {
    ...
    /*if (settings.playMode === 'dev' && hash[key] === undefined) {
      log("WARNING: Setting '" + key + "' for " + hash.name + " which is currently undefined.")
    }*/

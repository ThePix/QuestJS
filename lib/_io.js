...
function showMenuDiag(title, options, reactFunction) {
  ...
  io.disableTextFunction = function() { $('#sidepane-menu').remove() }
...
{
    $("div.pane-div:nth-child(" + position + ")").before('<div class="pane-div"><h4 class="side-pane-heading">' + title + '</h4><div id="' + id + '">' + content + '</div></div>');
}
...
io.textResponse = function(s) {
  if (s === undefined) s = $('#text-dialog').val()
...
io.appendItem = function(item, htmlDiv, loc, isSubItem, highlight) {
  $('#' + htmlDiv).append('<div id="' + item.name + '-item"><p class="item' + (isSubItem ? ' sub-item' : '') + (highlight ? ' highlight-item' : '') + '" onclick="io.clickItem(\'' + item.name + '\')">' + io.getIcon(item) + item.getListAlias(loc) + "</p></div>");
...
io.togglePlainFontMode = function() {
  ...
  if (settings.plainFontModeActive) {
    $('body').addClass("plain-font-body")
  }
  else {
    $('body').removeClass("plain-font-body")
  }

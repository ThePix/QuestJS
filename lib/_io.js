...
function draw(width, height, data, options) {
  ...
  s += ' ' + width + ' ' + height + '" xmlns="http://www.w3.org/2000/svg">'
  s += data.join('') + '</svg>'
...
io.calcMargins = function() {
  ...
  console.log(io.resizePanesListener.matches)
  if (settings.panes !== 'None') {
    if (io.resizePanesListener.matches) { // If media query matches
      $('#main').css(settings.panes === 'Left' ? 'margin-left' : 'margin-right', (io.mainGutter) + 'px')
      $('#panes').css('display', 'none')
    } else {
      $('#main').css(settings.panes === 'Left' ? 'margin-left' : 'margin-right', (io.panesWidth + io.mainGutter) + 'px')
      $('#panes').css('display', 'block')
    }
  }


  console.log(io.resizeMapImageListener.matches)
  ...
  if (io.resizeMapImageListener.matches) { // If media query matches
    $('#main').css(settings.panes === 'Right' ? 'margin-left' : 'margin-right', (io.mainGutter) + 'px')
    $('#quest-image').css('display', 'none')
    $('#quest-map').css('display', 'none')
  } else {
    $('#main').css(settings.panes === 'Right' ? 'margin-left' : 'margin-right', (mapImageWidth + io.mainGutter) + 'px')
...
/*
  if (settings.panes === 'None') {
    // No side panes, and implicitly the image pane and map are on the right and not moving
    $('#main').css('margin-left', (io.mainGutter) + 'px')
    $('#main').css('margin-right', (mapImageWidth + io.mainGutter) + 'px')
    return
  }


  if (!settings.panesAuto) {
    // We have side panes, and they are staying no matter how narrow.
    if (settings.panes === 'Left') {
      $('#main').css('margin-left', (io.panesWidth + io.mainGutter) + 'px')
      $('#main').css('margin-right', (mapImageWidth + io.mainGutter) + 'px')
    }
    else {
      $('#main').css('margin-right', (io.panesWidth + io.mainGutter) + 'px')
      $('#main').css('margin-left', (mapImageWidth + io.mainGutter) + 'px')
    }
    return
  }
  ...
  if (settings.panes === 'Left') {
    $('#main').css('margin-left', (io.panesWidth + io.mainGutter) + 'px')
    $('#main').css('margin-right', (mapImageWidth + io.mainGutter) + 'px')
    $('.main-narrow').css('margin-left', (io.mainGutter) + 'px')
  }
  else {
    $('#main').css('margin-right', (io.panesWidth + io.mainGutter) + 'px')
    $('#main').css('margin-left', (mapImageWidth + io.mainGutter) + 'px')
    $('.main-narrow').css('margin-right', (io.mainGutter) + 'px')
  }
  //$('.panes-narrow').css('display', 'none')
  $('.panes-narrow').css('color', 'blue')
*/
...
io.appendItem = function(item, htmlDiv, loc, isSubItem) {
  ...
  $('#' + htmlDiv).append('<p class="item' + (isSubItem ? ' sub-item' : '') + '" onclick="io.clickItem(\'' + item.name + '\')">' + item.icon() + item.getListAlias(loc) + "</p>");

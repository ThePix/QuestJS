...
function _msg(s, params, options) {
  ...
  processed = processed.replace(/@@@colon@@@/g, ':').replace(/@@@lcurly@@@/g, '{').replace(/@@@rcurly@@@/g, '}')
  const data = options
  data.text = processed
  if (!data.action) data.action = 'output'
  
  if (test.testing) {
    if (test.ignoreHTML) processed = processed.replace(/(<([^>]+)>)/gi, '')
...
function msg(s, params, cssClass) {
  ...
  for (let line of lines) {
    line = line.replace(/@@@vert@@@/g, '|')
    if (/^#/.test(line) && !cssClass) {
      line = line.replace(/^#/, '')
      _msg(line, params, {cssClass:'default-h default-h4', tag:'h4'})
    }
    else {
      _msg(line, params, {cssClass:cssClass, tag:'p'})
    }
  }
}
...
io.input = function(title, options, disableTextFunction, reactFunction, displayFunction) {
...
io.togglePlainFontMode = function() {
  ...
  if (settings.onPlainFontToggle) settings.onPlainFontToggle()
  msg(lang.done_msg)
  return world.SUCCESS_NO_TURNSCRIPTS
}

...
function errormsg(s, suppressTrace) {
  ...
  io.printError(s, new Error('error state caught by QuestJS runtime'), suppressTrace)
...
function showMenuWithNumbers(title, options, fn, failFunction) {
  const opts = {article:DEFINITE, capital:true, noLinks:true}
...
function showMenuNumbersOnly(title, options, fn) {
  ...
  const disableTextFunction = function(disable) {
    if (disable) {
      ...
      document.querySelector(document).on("keypress", function(e){
      ...
      delete document.querySelector(document).off("keypress")
    }
  }
  io.input(title, options, disableTextFunction, fn, function(options) {
    for (let i = 0; i < options.length; i++) {
...
function showDropDown(title, options, fn, failFunction) {
  const opts = {article:DEFINITE, capital:true, noLinks:true}
  io.input(title, options, false, fn, function(options) {
    let s = '<select id="menu-select" class="custom-select" style="width:400px;" ';
    s += 'onchange=\"io.menuResponse(document.querySelector(\'#menu-select\').find(\':selected\').value)\">';
...
function showMenuDiag(title, options, reactFunction) {
  const opts = {article:DEFINITE, capital:true, noLinks:true}
  io.menuFn = reactFunction;
  io.menuOptions = options;
  io.disableTextFunction = function() { document.querySelector('#sidepane-menu').remove() }
  ...
  if (typeof title === 'string') {
    s += '<p class="sidepane-menu-title">' + title + '</p>'
  }
  else {
    s += '<h4 class="sidepane-menu-title">' + title.title + '</h4>'
    s += '<p class="sidepane-menu-title">' + title.text + '</p>'
  }
  for (let i = 0; i < options.length; i++) {
...
function showTextDiag(title, reactFunction) {
  const opts = {article:DEFINITE, capital:true, noLinks:true}
  io.menuFn = reactFunction
  
  if (test.testing) {
    if (test.textResponse === undefined) {
      debugmsg("Error when testing text dialog")
    }
    else {
      io.textResponse(test.textResponse);
      delete test.textResponse
    }
    return; 
  }
  ...
  s += '<input type="text" id="text-dialog" onclick2="io.textResponse()" class="sidepane-menu-option">'
  s += '<br/>'
  s += '<input type="button" value="Search" onclick="io.textResponse()" class="sidepane-menu-option">'
  s += '</div>'
  document.querySelector('body').innerHTML += s

  document.getElementById("text-dialog").addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
      event.preventDefault();
      io.textResponse()
    }
  })
  document.querySelector('#text-dialog').focus()
  return world.SUCCESS_NO_TURNSCRIPTS
}
  ...
  if (settings.panes !== 'none' && update) {
    ...
    for (let exit of lang.exit_list) {
      if (currentLocation.hasExit(exit.name, {excludeScenery:true}) || exit.type === 'nocmd') {
        document.querySelector('#exit-' + exit.name).style.display = 'block'
      }
      else {
        document.querySelector('#exit-' + exit.name).style.display = 'none'
      }
    }
    io.updateStatus();
...
function createPaneBox(position, title, content, id) {
  const el = document.querySelector("div.pane-div")
  log(el)
  
  const div = document.createElement('div');
  div.innerHTML = '<div class="pane-div"><h4 class="side-pane-heading">' + title + '</h4><div id="' + id + '">' + content + '</div></div>'
  log(div)
...
function createToolbar() {
  ...
  if (settings.toolbar.roomdisplay) {
    html += ' <div>' + sentenceCase(lang.getName(w[player.loc], { article:DEFINITE })) + '</div>';
  }
  html += '</div>';
  ...
  document.querySelector("#output").before(html);
}
...
const io = {
  ...
  scriptShow:function() {
    let html = '';
    html += '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>QuestJS Transcript for '
    html += settings.title
    html += '</title></head><body><h2>QuestJS Transcript for "'
    html += settings.title + '" (version ' + settings.version
    html += ')'


    if (settings.playMode === 'dev' && settings.transcript === 'test') {
      html += ' for unit-tests</h2>'
      html += '<pre>"use strict"\n\ntest.tests = function() {\n  test.assertCmd("script on", ['
      for (let el of this.transcriptText) {
        switch (el.cssClass) {
          case 'input': html += '])\n&nbsp;&nbsp;test.assertCmd("' + el.text + '", ['; break;
          default : html += '"' + el.text.replace(/\"/g, '\\"') + '",'; break;
        }
      }
      html += '])\n}\n</pre>'
    }    
    else if (settings.playMode === 'dev' && settings.transcript === 'walk') {
      log('here')
      const lines = []
      for (let el of this.transcriptText) {
        ...
        if (el.cssClass === 'menu') {
          const previous = lines.pop().replace(/\,$/, '').trim()
          lines.push('    {cmd:' + previous + ', menu:' + el.n + '},')
        }
      }
      html += ' for Walk-Through</h2><pre>const walkthroughs = {\n  recorded:[\n' + lines.join('\n') + '\n  ],\n}</pre>'
    }
    else {
      html += '</h2>'
      for (let el of this.transcriptText) {
        switch (el.cssClass) {
          ...
          case 'meta': html += '<p style="color:blue">' + el.text + '</p>'; break;
          case 'error': html += '<details><summary style="color:red">Error encountered</summary><pre>' + el.text + '\n' + el.stack + '</pre></details>'; break;
          case 'debug': html += '<p style="color:grey">' + el.text + '</p>'; break;
          case 'parser': html += '<p style="color:magenta">' + el.text + '</p>'; break;
          case 'input': html += '<p style="color:cyan">' + el.text + '</p>'; break;
          case 'menu': html += '<p style="color:green">Menu option ' + (el.n + 1) + ': ' + el.text + ' selected</p>'; break;
    ...
    //const uriContent = "data:text/html," + encodeURIComponent(html);
    //const newWindow = window.open(uriContent, 'Transcript');
...
// This is used by the various menu functions.
io.input = function(title, options, disableTextFunction, reactFunction, displayFunction, failFunction) {
  ...
  io.menuOptions = options;
  io.disableTextFunction = disableTextFunction
...
io.outputFromQueue = function() {
  ...
  if (data.action === 'output') {
    ...
    if (io.spoken) io.speak(data.text);
...
io.calcMargins = function() {
  ...
  log(document.querySelector('#main').getAttribute('style'))
...
io.disable = function(level) {
  ...
  document.querySelector('.compass-button .dark-body').style.color = '#808080'
  document.querySelector('.item').style.color = '#808080'
  document.querySelector('.item-action').style.color = '#808080'
};
...
io.enable = function(level) {
  ...
  document.querySelector('.compass-button').style.color = io.textColour
  document.querySelector('.item').style.color = io.textColour
  document.querySelector('.item-action').style.color = io.textColour
};
  ...
  for (let key in w) {
    ...
    for (let inv of settings.inventoryPane) {
      ...
      if (inv.test(item) && !item.inventorySkip) {
        io.appendItem(item, inv.alt, loc, false, inv.highlight && item[inv.highlight]);
      }
    }
  }
...
io.textResponse = function(s) {
  if (s === undefined) s = document.querySelector('#text-dialog').value
  document.querySelector('#sidepane-menu').remove()

  // stop disabling input
  io.enable()

  if (io.transcript) io.scriptAppend({cssClass:'menu', text:s})
  io.menuFn(s);
  endTurnUI(true);
  if (settings.textInput) document.querySelector('#textbox').focus()
}
...
io.appendItem = function(item, htmlDiv, loc, isSubItem, highlight) {
  document.querySelector('#' + htmlDiv).innerHTML += '<div id="' + item.name + '-item"><p class="item' + (isSubItem ? ' sub-item' : '') + (highlight ? ' highlight-item' : '') + '" onclick="io.clickItem(\'' + item.name + '\')">' + io.getIcon(item) + item.getListAlias(loc) + "</p></div>"
  ...
  for (let verb of verbList) {
    ...
    s += verb.name;
    s += '</div>';
    document.querySelector('#' + htmlDiv).innerHTML += s
  }
...
io.createPanes = function() {
  ...
  document.querySelector('#input').innerHTML = '<span id="cursor">' + settings.cursor + '</span><input type="text" name="textbox" id="textbox"  autofocus/>'
  if (!settings.textInput) document.querySelector('#input').style.display = 'none'
...
window.addEventListener('load', function() {
  ...
  //const e = document.querySelector('#textbox').addEventListener('keydown', function(event){
...
io.finish = function() {
  ...
  document.querySelector('.pane-div').style.display = 'none'
  document.querySelector('.pane-div-finished').style.display = 'block'
  if (settings.afterFinish) settings.afterFinish()
  if (io.transcriptFlag) metamsg(processText("To see the transcript, click {cmd:SCRIPT SHOW:here}."))
}
...
io.toggleDarkMode = function() {
  ...
  io.textColour = document.querySelector(".side-panes").getComputedStyle("color")
  msg(lang.done_msg)
  return world.SUCCESS_NO_TURNSCRIPTS
}
...
io.addSlider = function(id, values, func) {
  ...
  slider.innerHTML += '<p id="' + id + '-text">' + name + ': ' + (typeof values === 'number' ? 0 : values[0]) + '</p>'
  if (func) func(0)
  slider.innerHTML += '<div id="' + id + '-slider"></div>'
  slider.style.paddingLeft = '10px'
  slider.style.paddingRight = '10px'
  
  document.querySelector('#' + id + '-slider').slider({max:max})
  document.querySelector('#' + id + '-slider').slider({
    slide: function(event, ui) {
      const id = event.target.id.replace('-slider', '')
      const value = ui.value
      document.querySelector('#' + id + '-text').innerHTML = settings.sliders[id].name + ': ' + (settings.sliders[id].values ? settings.sliders[id].values[value] : value)
...
io.printError = function(msg, err, suppressTrace) {
  console.error("ERROR: " + msg)

  if (world.isCreated) {
    io.print({tag:'p', cssClass:"error", text:lang.error})
    if (io.transcript) {
      io.transcriptText.push({
        cssClass:'error',
        text:msg,
        stack:err.stack,
      })

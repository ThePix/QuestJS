...
function _msg(s, params, options) {
  ...
  for (let line of processed.split('|')) {
    ...
    if (test.testing) {
      ...
      if (settings.outputPostProcessor) data.text = settings.outputPostProcessor(data.text)
...
function showMenuWithNumbers(title, options, fn) {
  ...
  const failFunction = function(input) {
    metamsg(lang.disambigToCmd, {input:input})
    runCmd(input)
...
function showDiag(title, text, submitButton) {
  ...
  askDiag({title:title, text:text, width:400, height:'auto'}, null, {submit:submitButton})
}



function askDiag(title, fn, options = {}) {
  io.menuFns.push(fn)
  io.showMenuDiagTitle = title
  io.showMenuDiagOptions = options
  world.suppressEndTurn = true
  const disableTextFunction = function(disable) {
    if (disable) {
      io.disable(3)
  ...
  const displayFunction = function() {
    ...
    if (io.showMenuDiagOptions.comment) {
      s += '<div id="dialog-footer" style="padding:10px;font-size:small"><p id="dialog-commentfooter">' + io.showMenuDiagOptions.comment + '</p></div>'
    }
    if (io.showMenuDiagOptions.submit) {
      s += '<div id="dialog-footer" style="text-align:right"><hr>'
      s += '<input type="button" onclick="io.textResponse()" value="' + io.showMenuDiagOptions.submit + '" class="sidepane-menu-button"></div>'
    }
    s += '</div>'
    document.querySelector('body').innerHTML += s
    
    io.sidepaneMenuSuppressBackgroundColor = document.querySelector('#sidepane-menu').style.backgroundColor
    ...
    if (fn) {
      ...
      el.addEventListener("keydown", function(event) {
        ...
        if (io.showMenuDiagOptions.filter) {
          const key = event.key
          if (key.length === 1 && !io.showMenuDiagOptions.filter(key)) io.sidepaneMenuSuppress(event)
        }
      })
      el.focus()
      io.menuFns.pop()
...
io.menuResponse = function(n) {
  ...
  // handle bad number
  if (n === undefined || n >= io.menuOptions.length || n < 0) {
    io.menuFailFn(input)
  }
  ...
  // handle good number
    ...
    const fn = io.menuFns.pop()
    fn(io.menuOptions[n], input)
  }
  
  // Do not end turn if disambiguating (parser.parseSingle in _parser.js)
  if (io.disableWorldEndTurn) {
    io.disableWorldEndTurn = false
  }
  else {
    world.endTurn(world.SUCCESS)
  }
...
io.textResponse = function(s) {
  ...
  if (io.showMenuDiagOptions.validator) {
    if (!io.showMenuDiagOptions.validator(s)) {
      io.sidepaneMenuSuppress()
      return
    }
  }
...
io.createPanes = function() {
  ...
  document.querySelector('#input').innerHTML = '<span id="cursor">' + settings.cursor + '</span><div id="inner-input"><input type="text" name="textbox" id="textbox" autocomplete="off" /></div>'
  if (!settings.textInput) document.querySelector('#input').style.display = 'none'
...
io.msgInputText = function(s) {
  ...
  document.querySelector("#output").innerHTML += '<p id="n' + io.nextid + '" class="input-text">' + settings.cursor + ' ' + s + "</p>"
  io.nextid++
  io.speak(s, true)
}
  ...
  document.addEventListener('keydown', function(event){
    ...
    if (keycode === 13){
      ...
      if (event.ctrlKey && (settings.playMode === 'dev' || settings.playMode === 'beta')) {
        // [CTRL]-[ENTER] does SHOW SCRIPT
        parser.parse("script show")
      }
      else {
        // [ENTER] sub,mits the command
        const s = io.filterInput(document.querySelector('#textbox').value)
        log(s)
        io.msgInputText(s)
        if (s) {
    ...
    if (!io.disableLevel) {
      ...
      // these features disabled in "play" mode
...
io.sidepaneMenuSuppress = function(event) {
  if (event) event.preventDefault()
  document.querySelector('#sidepane-menu').style.backgroundColor = 'red'
  setTimeout(function() {
    document.querySelector('#sidepane-menu').style.backgroundColor = io.sidepaneMenuSuppressBackgroundColor
  }, 50)
}
...
io.unfinish = function() {
  ...
  settings.cmdEcho = settings.finished.cmdEcho
  ...
  for (const el of settings.afterUnfinish) el()
  return world.SUCCESS_NO_TURNSCRIPTS
}
...
io.scrollToEnd = function() {
  if (settings.autoscroll) window.scrollTo({left:0, top:document.getElementById('main').scrollHeight, behavior: "smooth"});  
}
...
io.dialog = function(data) {
  ...
  html += '<input type="button" value="Okay" style="color:grey;float: right;" onclick="io.dialogOkay()"/></p>'
  diag.innerHTML = html
...
io.setWidget = function(options) {
  ...
  if (type === 'radio') {
    let checked = typeof options.checked === 'number' ? options.checked : random.int(0, Object.keys(options.data).length - 1)
  ...
  if (type === 'dropdown') {
    let checked = typeof options.checked === 'number' ? options.checked : random.int(0, Object.keys(options.data).length - 1)
    html += '<select name="' + options.name + '" id="' + options.name + '"><br/>'
    for (const key in options.data) {
      html += '<option value="' + key + '"'
      if (!checked) html += ' selected="selected" '
      checked--
  ...
  if (type === 'dropdownPlus') {
    let checked = typeof options.checked === 'number' ? options.checked : random.int(0, Object.keys(options.data).length - 1)
    const index = checked
    html += '<select name="' + options.name + '" id="' + options.name + '" onchange="io.dropdownChange(\'' + options.name + '\')"><br/>'
  ...
  if (type === 'color' || type === 'colour') {
    ...
    if (options.checked) html += 'value="' + options.checked + '" '
    html += '/></div>'
  }
  
  if (type === 'range') {
    let checked = typeof options.checked === 'number' ? options.checked : random.int(0, 100)
    html += '<input type="range" name="' + options.name + '" id="' + options.name + '" '
    html += 'value="' + options.checked + '" '
...
io.htmlValue = function(options) {
  ...
  else if (type === 'text' || type === 'number' || type === 'password' || type === 'range' || type === 'color') {
    value = document.querySelector('#' + options.name).value
...
io.dropdownChange = function(id) {
  const dd = document.getElementById(id)
  const widget = io.dialogWidgets.find(el => el.name === id)
  const ddText = document.getElementById(id + '-text')
  ddText.innerHTML = widget.data[dd.selectedIndex].text
}  
  
io.wordDblClick = function(s) {
  document.querySelector('#textbox').value += ' ' + s.replace(/[^0-9A-Za-zÀ-ÖØ-öø-ÿ]/, '')
}
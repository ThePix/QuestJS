  ...
  res.talkto = function() {
    ...
    if (USE_DROPDOWN_FOR_CONV) {
      ...
      showMenu(CMD_SPEAK_TO_MENU_TITLE(this), topics, function(result) {
        if (result !== NEVER_MIND) {
          this.pause();  // does this still refer to the NPC???
          result.runscript();
        }
      });
    }
...
const AGENDA_FOLLOWER = function(agendaList) {
  var res = {
    agenda:agendaList,
    runTurnscript:function() { return true; },
    
    turnscript:function() {
      if (this.paused || this.agenda.length === 0) return;
      debugmsg("agenda: " + this.name);
      var arr = this.agenda[0].split(":");
      var fn = arr.shift();
      if (typeof agenda[fn] !== "function") {
        errormsg(ERR_GAME_BUG, "Unknown function `" + fn + "' in agenda for " + this.name);
        return;
      }
      debugmsg("fn=" + fn);

...
const ROPE = function(tetheredTo) {
  ...
  // This MUST be sent the fromLoc, and you must already have established it is not attached
  res.moveToFrom = function(toLoc, fromLoc) {
    if (fromLoc === undefined) throw "Need a fromLoc here for a rope object"
    ...
    if (this.locs[0] === fromLoc) {
      this.locs.shift()
      this.locs.unshift(toLoc)
    }
    if (this.locs[this.locs.length - 1] === fromLoc) {
      this.locs.pop()

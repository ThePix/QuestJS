...
const TAKEABLE_DICTIONARY = {
  getVerbs:function() {
    ...
    if (this.read) verbs.push(VERBS.read);
    return verbs;
  },
...
const VESSEL = function(capacity) {
  ...
  res.fill = function(isMultiple, char, liquid) {
    ...
    if (!source) return falsemsg("None here.");
    if (!this.mix && this.contents !== liquid.name && this.volumeContained > 0) return falsemsg("No mixing.");
    ...
    if (this.mix && liquid.name !== this.contents !== liquid.name) {
      ...
      // does not behave as a full vessel, so if the vessel is pretty much full set the volme contained to the capacity

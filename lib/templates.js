...
import { io, game, lang, util, world, w, defaults } from './main'
const TAKEABLE_DICTIONARY = {
  ...
  drop: function (isMultiple, char) {
    io.msg(util.prefix(this, isMultiple) + lang.drop_successful(char, this))
  ...
  take: function (isMultiple, char) {
    if (this.isAtLoc(char.name)) {
      io.msg(util.prefix(this, isMultiple) + lang.already_have(char, this))
      return false
    }
    if (!char.canManipulate(this, 'take')) return false
    io.msg(util.prefix(this, isMultiple) + lang.take_successful(char, this))
...
const TAKEABLE = () => TAKEABLE_DICTIONARY /// / -fixme: EWWWWWWWWWWWWWW
...
const createEnsemble = function (name, members, dict) {
  const res = world.createItem(name, dict)
  res.ensemble = true
  res.members = members
  ...
  res.byname = function (options) {
    if (!options) options = {}
    let s = ''
    if (options.article === util.DEFINITE) {
      s = lang.addDefiniteArticle(this)
    }
    if (options.article === util.INDEFINITE) {
  ...
  res.isAtLoc = function (loc, situation) {
    if (situation !== util.display.PARSER && situation !== util.display.SCOPING) return false
...
const MERCH = function (value, locs) {
  const res = {
    ...
    // The price when the player lang.buys the item
    ...
    purchase: function (isMultiple, char) {
      if (!this.isForSale(char.loc)) return io.failedio.msg(util.prefix(this, isMultiple) + lang.cannot_purchase_here(char, this))
      const cost = this.getBuyingPrice(char)
      if (char.money < cost) return io.failedio.msg(util.prefix(this, isMultiple) + lang.cannot_afford(char, this, cost))
      this.purchaseScript(isMultiple, char, cost)
    },

    purchaseScript: function (isMultiple, char, cost) {
      char.money -= cost
      io.msg(util.prefix(this, isMultiple) + lang.purchase_successful(char, this, cost))
      if (this.doNotClone) {
        this.moveToFrom(char.name, char.loc)
        delete this.salesLoc
      } else {
        world.cloneObject(this, char.name)
      }
      return util.SUCCESS
    },
    ...
    sell: function (isMultiple, char) {
      if (!this.canBeSoldHere(char.loc)) {
        return io.failedio.msg(util.prefix(this, isMultiple) + lang.cannot_sell_here(char, this))
      }
      const cost = this.getSellingPrice(char)
      char.money += cost
      io.msg(util.prefix(this, isMultiple) + lang.sell_successful(char, this, cost))
      if (this.doNotClone) {
...
const COUNTABLE = function (countableLocs) {
  ...
  res.byname = function (options) {
    ...
    if (options.count) {
      count = options.count
      s = (count === util.INFINITY ? this.infinity : lang.toWords(count)) + ' '
    } else if (options.article === util.DEFINITE) {
      s = 'the '
    } else if (options.article === util.INDEFINITE) {
      if (count) {
        switch (count) {
          case 1: s = 'a '; break
          case util.INFINITY: s = this.infinity + ' '; break
  ...
  res.getListAlias = function (loc) {
    return util.sentenCecase(this.pluralAlias ? this.pluralAlias : this.listalias + 's') + ' (' + this.countAtLoc(loc) + ')'
  }
  ...
  res.takeFrom = function (loc, count) {
    if (this.countableLocs[loc] !== util.INFINITY) this.countableLocs[loc] -= count
  ...
  res.findSource = function (sourceLoc, tryContainers) {
    ...
    if (tryContainers) {
      const containers = util.scopeReachable().filter(el => el.container)
  ...
  res.take = function (isMultiple, char) {
    ...
    if (!sourceLoc) {
      io.msg(util.prefix(this, isMultiple) + lang.none_here(char, this))
      return false
    }
    ...
    io.msg(util.prefix(this, isMultiple) + lang.take_successful(char, this, n))
  ...
  res.drop = function (isMultiple, char) {
    ...
    if (m === 0) {
      io.msg(util.prefix(this, isMultiple) + lang.none_held(char, this))
      return false
    }
    ...
    io.msg(util.prefix(this, isMultiple) + lang.drop_successful(char, this, n))
...
const WEARABLE = function (wearLayer, slots) {
  const res = $.extend({}, TAKEABLE_DICTIONARY)
  res.wearable = true
  res.wear_layer = wearLayer || false
  res.slots = slots && wearLayer ? slots : []
  ...
  res.canWearRemove = function (char, toWear) {
    ...
    if (garment) {
      if (toWear) {
        io.msg(lang.cannot_wear_over(char, this, garment))
      } else {
        io.msg(lang.cannot_remove_under(char, this, garment))
      }
      return false
    }
    return true
  ...
  // Assumes the item is lang.already held
  res.wear = function (isMultiple, char) {
    ...
    io.msg(util.prefix(this, isMultiple) + this.wearMsg(char, this), { garment: this, actor: char })
    this.worn = true
  ...
  res.remove = function (isMultiple, char) {
    ...
    io.msg(util.prefix(this, isMultiple) + this.removeMsg(char, this), { garment: this, actor: char })
    this.worn = false
...
const CONTAINER = function (openable) {
  const res = util.CONTAINER_BASE
  res.closed = openable
  res.openable = openable
  res.contentsType = 'container' // -review:  theFuck?
  res.listContents = util.listContents // -review: also theFuck?
  ...
  res.byname = function (options) {
    if (!options) options = {}
    let prefix = '' // -fixme: confusing name
    if (options.article === util.DEFINITE) {
    ...
    const contents = this.getContents(util.display.LOOK)
    let s = prefix + this.alias
    if (options && options.possessive) s += "'s"
    if (contents.length > 0 && options.modified && (!this.closed || this.transparent)) {
      s += ' (' + lang.contentsForData[this.contentsType].util.prefix + this.listContents(contents) + lang.contentsForData[this.contentsType].suffix + ')'
    }
    if (options && options.capital) s = util.sentenCecase(s)
    return s
  }
  ...
  res.lookinside = function (isMultiple, char) {
    if (this.closed && !this.transparent) {
      io.msg(util.prefix(this, isMultiple) + lang.nothing_inside(char))
      return false
    }
    io.msg(util.prefix(this, isMultiple) + lang.look_inside(char, this))
    return true
  }

  res.open = function (isMultiple, char) {
    if (!this.openable) {
      io.msg(util.prefix(this, isMultiple) + lang.cannot_open(char, this))
      return false
    } else if (!this.closed) {
      io.msg(util.prefix(this, isMultiple) + lang.already(this))
      return false
    }
    if (this.locked) {
      if (this.util.testKeys(char)) {
        this.closed = false
        io.msg(util.prefix(this, isMultiple) + lang.unlock_successful(char, this))
        this.openMsg(isMultiple, char)
        return true
      } else {
        io.msg(util.prefix(this, isMultiple) + lang.locked(char, this))
        return false
      }
    }
    this.closed = false
  ...
  res.openMsg = function (isMultiple, char) {
    io.msg(util.prefix(this, isMultiple) + lang.open_successful(char, this))
  }

  res.close = function (isMultiple, char) {
    if (!this.openable) {
      io.msg(util.prefix(this, isMultiple) + lang.cannot_close(char, this))
      return false
    } else if (this.closed) {
      io.msg(util.prefix(this, isMultiple) + lang.already(this))
      return false
  ...
  res.closeMsg = function (isMultiple, char) {
    io.msg(util.prefix(this, isMultiple) + lang.close_successful(char, this))
  }
...
const SURFACE = function () {
  const res = defaults.CONTAINER_BASE
  res.closed = false
  res.openable = false
  res.byname = CONTAINER().byname
  res.listContents = util.listContents
  res.contentsType = 'surface' // -fixme: garbage
...
const lockedWith = function (keyNames) {
  ...
  const res = {
    ...
    lock: function (isMultiple, char) {
      if (this.locked) {
        io.msg(lang.already(this))
        return false
      }
      if (!this.util.testKeys(char, true)) {
        io.msg(lang.no_key(char, this))
        return false
      }
      if (!this.closed) {
        this.closed = true
        io.msg(lang.close_successful(char, this))
      }
      io.msg(lang.lock_successful(char, this))
      this.locked = true
      return true
    },
    unlock: function (isMultiple, char) {
      ...
      if (!this.util.testKeys(char, false)) {
        io.msg(lang.no_key(char, this))
        return false
      }
      io.msg(lang.unlock_successful(char, this))
      this.locked = false
      return true
    },
    testKeys: function (char, toLock) {
      for (const s of keyNames) {
        if (!w[s]) {
          io.errorio.msg('The key name for this container, `' + s + '`, does not match any key in the game.')
          return false
        }
...
const FURNITURE = function (options) {
  const res = {
    ...
    getoff: function (isMultiple, char) {
      if (!char.posture) {
        char.io.msg(lang.already(char))
        return false
      }
      if (char.posture) {
        char.io.msg(lang.stop_posture(char)) // stop_posture handles details
        return true
      }
    }
  }
  res.assumePosture = function (isMultiple, char, posture, success_msg, adverb) {
    ...
    if (!this.util.testForPosture(char, posture)) {
      return false
    }
    if (char.posture) {
      char.io.msg(lang.stop_posture(char))
    }
    char.posture = posture
    char.postureFurniture = this.name
    char.postureAdverb = adverb === undefined ? 'on' : adverb
    char.io.msg(success_msg(char, this))
...
const SWITCHABLE = function (alreadyOn) {
  ...
  res.switchon = function (isMultiple, char) {
    if (this.switchedon) {
      char.io.msg(util.prefix(this, isMultiple) + lang.already(this))
      return false
    }
    if (!this.checkCanSwitchOn()) {
      return false
    }
    char.io.msg(lang.turn_on_successful(char, this))
    this.doSwitchon()
    return true
  }
  ...
  res.switchoff = function (isMultiple, char) {
    ...
    char.io.msg(lang.turn_off_successful(char, this))
    this.doSwitchoff()
    return true
  }
...
const COMPONENT = function (nameOfWhole) {
  const res = {
    ...
    isAtLoc: function (loc, situation) {
      ...
      if (situation !== util.display.PARSER) return false
      const cont = w[this.loc]
      if (cont.isAtLoc(loc)) { return true }
      return cont.isAtLoc(loc)
    },
    take: function (isMultiple, char) {
      io.msg(util.prefix(this, isMultiple) + lang.cannot_take_component(char, this))
      return false
    }
  }
  if (!w[nameOfWhole]) io.debugmsg('Whole is not define: ' + nameOfWhole)
...
const EDIBLE = function (isLiquid) {
  ...
  res.eat = function (isMultiple, char) {
    if (this.isLiquid) {
      io.msg(util.prefix(this, isMultiple) + lang.cannot_eat(char, this))
      return false
    }
    io.msg(util.prefix(this, isMultiple) + lang.eat_successful(char, this))
    this.loc = null
  ...
  res.drink = function (isMultiple, char) {
    if (!this.isLiquid) {
      io.msg(util.prefix(this, isMultiple) + lang.cannot_drink(char, this))
      return false
    }
    io.msg(util.prefix(this, isMultiple) + lang.drink_successful(char, this))
    this.loc = null
    if (this.onIngesting) this.onIngesting(char)
    return true
  }
...
const VESSEL = function (capacity) {
  ...
  res.byname = function (options) {
    ...
    if (options.modified && this.volumeContained && this.volumeContained > 0) {
      if (this.volumeContained >= this.capacity) {
        ...
        s += ' (' + this.volumeContained + ' ' /* +VOLUME_UNITS */ + ' ' + this.containedLiquidName + ')' // -// -fixme:  this
      }
    }
    if (options && options.capital) s = util.sentenCecase(s)
    return s
  ...
  res.fill = function (isMultiple, char, liquid) {
    if (this.util.testRestrictions && !this.util.testRestrictions(liquid, char)) return false
    const source = liquid.source(char)
    if (!source) return io.falsemsg(lang.none_here(char, liquid))
    if (!this.mix && this.containedLiquidName !== liquid.name && this.volumeContained > 0) return io.falsemsg(lang.cannot_mix(char, this))
    if (this.volumeContained >= this.capacity) return io.falsemsg(util.prefix(this, isMultiple) + lang.already(this))
    ...
    if (this.mix && liquid.name !== this.containedLiquidName !== liquid.name) {
      ...
      io.msg(util.prefix(this, isMultiple) + lang.fill_successful(char, this))
  ...
  res.empty = function (isMultiple, char) {
    ...
    io.msg(util.prefix(this, isMultiple) + lang.empty_successful(char, this))
    return true
  }
...
const LIQUID = function (locs) {
  ...
  res.drink = function (isMultiple, char, options) {
    io.msg(util.prefix(this, isMultiple) + 'drink: ' + options.verb + ' char: ' + char.name)
  }
  ...
  res.source = function (chr) {
    ...
    const items = util.scopeHeldBy(chr).concat(util.scopeHeldBy(chr.loc))
...
const TRANSIT_BUTTON = function (nameOfTransit) {
  const res = {
    ...
    push: function () {
      ...
      if (this.locked) {
        io.printOrRun(game.player, this, 'transitLocked')
        return false
      } else if (exit.name === this.transitDest) {
        io.printOrRun(game.player, this, 'transitAlreadyHere')
        return false
      } else {
        io.printOrRun(game.player, this, 'transitGoToDest')
...
const TRANSIT = function (exitDir) {
  const res = {
    ...
    getTransitButtons: function (includeHidden, includeLocked) {
      return this.getContents(util.display.LOOK).filter(function (el) {
...
function transitOfferMenu () {
  ...
  const room = this
  io.showMenu(this.transitMenuPrompt, buttons.map(el => el.transitDestAlias), function (result) {
    for (const button of buttons) {
      if (buttons[button].transitDestAlias === result) {
        if (room[transitDoorDir].name === button.transitDest) {
          io.printOrRun(game.player, button, 'transitAlreadyHere')
        } else {
          io.printOrRun(game.player, button, 'transitGoToDest')
...
const PLAYER = function () {
  const res = {
    ...
    canSeeThrough: () => true,
    getAgreement: () => true,
    getContents: defaults.CONTAINER_BASE.getContents,
    pause: util.NULL_FUNC,
    canManipulate: () => true,
    canMove: () => true,
    canTalk: () => true,
    canPosture: () => true,
    ...
    getHolding: function () {
      return this.getContents(util.display.LOOK).filter(function (el) { return !el.getWorn() })
    },

    getWearing: function () {
      return this.getContents(util.display.LOOK).filter(function (el) { return el.getWorn() && !el.ensemble })
    },

    getStatusDesc: function () {
      if (!this.posture) return false
      return this.posture + ' ' + this.postureAdverb + ' ' + w[this.postureFurniture].byname({ article: util.DEFINITE })
    },

    isAtLoc: function (loc, situation) {
      if (situation === util.display.LOOK) return false
      if (situation === util.display.SIDE_PANE) return false
      if (typeof loc !== 'string') loc = loc.name
      return (this.loc === loc)
    },
    ...
    // console.log("npc byname " + this.name)
      ...
      if (options.group && this.followers.length > 0) {
        ...
        let s = util.formatList(this.followers, options)
      ...
      if (options.modified) {
        ...
        if (held.length > 0) {
          list.push('holding ' + util.formatList(held, { article: util.INDEFINITE, lastJoiner: lang.list_and, modified: false, nothing: lang.list_nothing, loc: this.name, npc: true }))
        }
        if (worn.length > 0) {
          list.push('wearing ' + util.formatList(worn, { article: util.INDEFINITE, lastJoiner: lang.list_and, modified: false, nothing: lang.list_nothing, loc: this.name, npc: true }))
        }
        if (list.length > 0) s += ' (' + util.formatList(list, { lastJoiner: ';' + lang.list_and, sep: ';' }) + ')'
      }

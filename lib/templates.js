...
const LIQUID = function(locs) {
  ...
  res.sourcedHere = function(chr) { 
    if (chr === undefined) chr = game.player;
    // Is character a source?
    if (chr.isSourceOf && chr.isSourceOf(this.name)) return true;
    // Is the room a source?
    if (w[chr.loc].isSourceOf && w[chr.loc].isSourceOf(this.name)) return true;
    const items = scopeHeldBy(chr).concat(scopeHeldBy(chr.loc));
    for (let obj of held) {
      if (obj.isSourceOf && obj.isSourceOf(this.name)) return true;
    }
    return false;
  }

...
const COUNTABLE = function (countableLocs) {
  ...
  res.drop = function (isMultiple, char) {
    ...
    if (!n) { m === util.INFINITY ? 1 : n = m } // no number specified
    if (n > m) { n = m } // too big number specified
...
const CONTAINER = function (openable) {
  ...
  res.byname = function (options) {
    ...
    let util.prefix = ''
    if (options.article === util.DEFINITE) {
      util.prefix = lang.addDefiniteArticle(this)
    }
    if (options.article === util.INDEFINITE) {
      util.prefix = lang.addIndefiniteArticle(this)
    }
    const contents = this.getContents(util.display.LOOK)
    let s = util.prefix + this.alias
    if (options && options.possessive) s += "'s"
  ...
  const res = {
    ...
    lock: function (isMultiple, char) {
      ...
      if (!this.util.testKeys(char, true)) {
        io.msg(no_key(char, this))
        return false
      }
      if (!this.closed) {
        this.closed = true
        io.msg(close_successful(char, this))
      }
      io.msg(lock_successful(char, this))
      this.locked = true
      return true
    },
    unlock: function (isMultiple, char) {
      ...
      io.msg(unlock_successful(char, this))
      this.locked = false
      return true
    },
    util.testKeys: function (char, toLock) {
...
const FURNITURE = function (options) {
  const res = {
    util.testForPosture: (char, posture) => true,
    getoff: function (isMultiple, char) {
      if (!char.posture) {
        char.io.msg(already(char))
        return false
      }
      if (char.posture) {
  ...
  res.assumePosture = function (isMultiple, char, posture, success_io.msg, adverb) {
...
const COMPONENT = function (nameOfWhole) {
  ...
  if (!w[nameOfWhole]) debugio.msg('Whole is not define: ' + nameOfWhole)
...
const VESSEL = function (capacity) {
  ...
  res.fill = function (isMultiple, char, liquid) {
    ...
    if (!source) return falseio.msg(lang.none_here(char, liquid))
    if (!this.mix && this.containedLiquidName !== liquid.name && this.volumeContained > 0) return falseio.msg(lang.cannot_mix(char, this))
    if (this.volumeContained >= this.capacity) return falseio.msg(util.prefix(this, isMultiple) + lang.already(this))
  ...
  const res = {
    ...
    // console.log("npc byname " + this.name)
      ...
      if (options.group && this.followers.length > 0) {
        ...
        const s = util.formatList(this.followers, options)

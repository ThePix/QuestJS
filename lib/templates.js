...
const MERCH = function (value, locs) {
  const res = {
    ...
    purchase: function (isMultiple, char) {
      if (!this.isForSale(char.loc)) return io.failedmsg(prefix(this, isMultiple) + lang.cannot_purchase_here(char, this))
      const cost = this.getBuyingPrice(char)
      if (char.money < cost) return io.failedmsg(prefix(this, isMultiple) + lang.cannot_afford(char, this, cost))
    ...
    sell: function (isMultiple, char) {
      if (!this.canBeSoldHere(char.loc)) {
        return io.failedmsg(prefix(this, isMultiple) + lang.cannot_sell_here(char, this))
      }

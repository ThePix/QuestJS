...
const createEnsemble = function(name, members, dict) {
  ...
  res.getNameModifier = function(options) {
    return this.members[0].getWorn() && this.isAllTogether() && this.members[0].isAtLoc(game.player.name) ? " (" + lang.invWornModifier + ")" : ''
  }
...
const WEARABLE = function(wear_layer, slots) {
  ...
  res.getNameModifier = function(options) {
    if (this.worn && this.isAtLoc(game.player.name)) return ' (' + lang.invWornModifier + ')'
    return ''
  };
...
const CONTAINER = function(openable) {
  ...
  res.getNameModifier = util.getNameModifierForContainer
...
const OPENABLE = function(alreadyOpen) {
  ...
  res.getNameModifier = function(options) {
    return (this.closed ? '' : ' (' + lang.invOpenModifier + ')')
  };
...
const VESSEL = function(capacity) {
  ...
  res.getNameModifier = function(options) {
    if (this.volumeContained && this.volumeContained > 0) {
      if (this.volumeContained >= this.capacity) {
        return " (full of " + this.containedLiquidName + ")"
      }
      else {
        return " (" + this.volumeContained + " " + VOLUME_UNITS + " " + this.containedLiquidName + ")";
      }
    }
    return ''
  };
  ...
  const res = {
    ...
    push:function() {
      ...
      if (this.locked) {
        ...
        transit.update(this.transitDest, true)
      }
    },
  };
  return res;
...
const TRANSIT = function(exitDir) {
  const res = {
    ...
    update:function(transitDest, callEvent) {
      const exit = this[this.transitDoorDir]
      const previousDest = exit.name
      exit.name = transitDest
      
      if (typeof map !== 'undefined') {
        console.log("MAPPING")
        console.log(transitDest)
        
        this.mapCurrentConnection = this.locations.findIndex(el => el.connectedRoom.name === transitDest)
        if (this.mapCurrentConnection === -1) {
          errormsg('Failed to find a location called "' + transitDest + '"')
...
/*        // find exit from destination to here
        const reverseExitObj = util.reverseDirectionObj(this.transitDoorDir)
        console.log(reverseExitObj)
        const dest = w[this[exit.dir].name]
        console.log(dest)
        const reverseExit = dest[reverseExitObj.name]
        console.log(reverseExit)
        delete dest.mapX  // so the location is updated rather than checked
        map.mapRoomFromExit(dest, this, reverseExit, reverseExitObj)*/
      }
      if (callEvent && this.transitOnMove) this.transitOnMove(transitDest, previousDest)
    },
...
const PLAYER = function() {
  ...
  const res = {
    ...
    getNameModifier:function(options) {
      ...
      if (worn.length > 0) {
        list.push(lang.invWearingPrefix + ' ' + formatList(worn, {article:INDEFINITE, lastJoiner:lang.list_and, modified:false, nothing:lang.list_nothing, loc:this.name, npc:true}));
      }

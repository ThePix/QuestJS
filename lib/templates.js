...
const CONTAINER = function(openable) {
  ...
  res.openMsg = function(isMultiple, char) {
    msg(prefix(this, isMultiple) + lang.open_successful(char, this) + " " + lang.look_inside(char, this));
  };
...
const ROPE = function(tetheredTo) {
  const res = $.extend({
    ...
    onGoAction:function(char) {
      if (this.locs.length === 1) return
  ...
  res.moveToFrom = function(toLoc, fromLoc) {
    ...
    if (w[fromLoc].onGoCheckList && w[fromLoc].onGoCheckList.includes(this.name)) array.remove(w[fromLoc].onGoCheckList, this.name)
    if (w[toLoc].onGoCheckList && !w[toLoc].onGoCheckList.includes(this.name)) w[toLoc].onGoCheckList.push(this.name)
    if (w[fromLoc].onGoActionList && w[fromLoc].onGoActionList.includes(this.name)) array.remove(w[fromLoc].onGoActionList, this.name)
...
/*  res.drop = function(isMultiple, char) {
  ...
  res.take = function(isMultiple, char) {
    if (this.isAtLoc(char.name)) return falsemsg(prefix(this, isMultiple) + lang.already_have(char, this))
    if (!char.canManipulate(this, "take")) return false;

...
const COUNTABLE = function(locs) {
  ...
  res.byname = function(options) {
    ...
    if (count && options.modified) { s += " (" + count + ")"; }
    return s;
  };
  
  res.getListAlias = function(loc) {
    return (this.pluralAlias ? this.pluralAlias : this.listalias + "s") + " (" + this.countAtloc + ")";
  };
...
const CONTAINER = function(alreadyOpen) {
  ...
  res.getContents = function() {
    var list = [];
    for (var key in w) {
      if (w[key].loc === this.name && w[key].display >= DSPY_SCENERY) {
        list.push(w[key]);
      }
    }
    return list;
...
const COMPONENT = function(nameOfWhole) {
  var res = {
    ...
    isAtLoc:function(loc) {
      ...
      if (cont.altLocs && cont.altLocs.includes(loc))  { return true; }
      return false;
    },
    take:function(isMultiple, char) {

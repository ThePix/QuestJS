...
const TAKEABLE_DICTIONARY = {
  ...
  take:function(isMultiple, char) {
    ...
    if (this.getDisplay() === DSPY_SCENERY) {
...
const createEnsemble = function(name, members, dict) {
  ...
  res.takeable = true;
  res.getWorn = function() { return this.isAtLoc(this.members[0].loc) && this.members[0].getWorn(); }
  ...
  res.byname = function(options) {
    ...
    if (this.members[0].getWorn() && options.modified && (this.isAtLoc(game.player.name))) { s += " (worn)"; }
    return s;
  };
  ...
  res.isAtLoc = function(loc, preferEnsemble) {
  ...
  res.take = function(isMultiple, char) {
    ...
    for (let i = 0; i < this.members.length; i++) {
      ...
      if (this.members[i].getDisplay() === DSPY_SCENERY) {
        this.members[i].display = DSPY_DISPLAY;
      }
    }
    return true;
  }
...
const PLAYER = function() {
  ...
  const res = {
    ...
    getHolding:function() {
      return this.getContents().filter(function(el) { return !el.getWorn(); });
    },
    ...
    getOuterWearable:function(slot) {
      ...
      */
      ...
      const clothing = scope(isWornBy, {npc:this}).filter(el => el.getSlots().includes(slot));
    ...
    byname:function(options) {
      ...
      if (this.pronouns === PRONOUNS.firstperson || this.pronouns === PRONOUNS.secondperson) {
        return options && options.possessive ? this.pronouns.poss_adj : this.pronouns.subjective;
      }
      ...
      if (options.modified) {
        ...
        if (held.length > 0) {
          list.push("holding " + formatList(held, {article:INDEFINITE, lastJoiner:LIST_AND, modified:false, nothing:LIST_NOTHING, loc:this.name}));
        }
        if (worn.length > 0) {
          list.push("wearing " + formatList(worn, {article:INDEFINITE, lastJoiner:LIST_AND, modified:false, nothing:LIST_NOTHING, loc:this.name}));
        }

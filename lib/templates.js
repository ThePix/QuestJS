...
const MERCH = function(value, locs) {
  res = {
    ...
    getBuyingPrice:function(char) { return this.price; },
    
    isForSale:function(loc) {
      if (this.locs.includes(loc)) return true;
      // ultimately want to allow for items that can sell thing
      return false;
    },
    
    purchase:function(isMultiple, char) {
      if (!this.isForSale(char.loc)) {
        failedmsg(prefix(this, isMultiple) + CANNOT_PURCHASE_HERE(char, this));
        return FAILED;
      }
      const cost = this.getBuyingPrice(char);
      if (char.money < cost) {
        failedmsg(prefix(this, isMultiple) + CANNOT_AFFORD(char, this, cost));
        return FAILED;
      }
      char.money -= cost;
    ...
    sell:function(isMultiple, char) {
      if (!this.isForSale(char.loc)) {
        failedmsg(prefix(this, isMultiple) + CANNOT_SELL_HERE(char, this));
        return FAILED;
      }

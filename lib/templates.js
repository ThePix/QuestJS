...
const TAKABLE_DICTIONARY = {
  ...
  drop:function(isMultiple, char) {
    if (this.worn) {
      msg(prefix(this, isMultiple) + CMD_WEARING(char, this));
      return false;
    };
    if (this.loc !== game.player.name) {
      msg(prefix(this, isMultiple) + CMD_NOT_CARRYING(char, this));
      return false;
    };

    msg(prefix(this, isMultiple) + CMD_NPC_DROP_SUCCESSFUL(char, this));
...
const WEARABLE = function() {
  ...
  res.wear = function(isMultiple, char) {
    ...
    if (this.worn) {
      msg(prefix(this, isMultiple) + CMD_ALREADY_WEARING(char, this));
      return false;
    }
    if (this.loc !== game.player.name) {
      msg(prefix(this, isMultiple) + CMD_NOT_CARRYING(char, this));
  ...
  res.remove = function(isMultiple, char) {
    if (!this.worn) {
      msg(prefix(this, isMultiple) + CMD_NOT_WEARING(char, this));
      return false;
    }
...
const CONTAINER = function(alreadyOpen) {
  ...
  res.open = function(isMultiple) {
    ...
    if (this.locked) {
      if (this.testKeys()) {
        this.closed = false;
        msg(prefix(this, isMultiple) + CMD_UNLOCK_SUCCESSFUL(game.player, this));
        msg(prefix(this, isMultiple) + CMD_OPEN_SUCCESSFUL(game.player, this));
        return true;
      }
      else {
  ...
  res.close = function(isMultiple) {
    ...
    this.closed = true;
    msg(prefix(this, isMultiple) + CMD_CLOSE_SUCCESSFUL(game.player, this));
    return true;
  };
...
const LOCKED_WITH = function(keyNames) {
  ...
  var res = {
    ...
    lock:function() {
      ...
      if (!this.closed) {
        this.closed = true;
        msg(CMD_CLOSE_SUCCESSFUL(game.player, this));
      }      
      msg(CMD_LOCK_SUCCESSFUL(game.player, this));
      this.locked = true;
      return true;
    },
    unlock:function() {
      ...
      msg(CMD_UNLOCK_SUCCESSFUL(game.player, this));
      this.locked = false;
      return true;
    },
...
const SWITCHABLE = function(alreadyOn) {
  ...
  res.switchon = function(isMultiple) {
    ...
    msg(CMD_TURN_ON_SUCCESSFUL(game.player, this));
    this.doSwitchon();
  };
  ...
  res.switchoff = function(isMultiple) {
    ...
    msg(CMD_TURN_OFF_SUCCESSFUL(game.player, this));
    this.doSwitchoff();
  };

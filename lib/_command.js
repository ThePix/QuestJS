...
function Cmd(name, hash) {
  ...
  this.objects = [];
  this.rules = [];
  this.default = function(item, multiple, char) { 
    if (typeof this.defmsg === "string") {
      failedmsg(prefix(item, multiple) + this.defmsg, {char:char, item:item});
    }
    else if (typeof this.defmsg === "function") {
      failedmsg(prefix(item, multiple) + this.defmsg(char, item), {char:char, item:item});
    }
    else {
...
function NpcExitCmd(name, dir, hash) {
  ...
  this.script = function(objects) {
    ...
    if (npc.testMoveRestrictions && !npc.testMoveRestrictions(this)) return world.FAILED
...
cmdRules.isHeldNotWorn = function(cmd, char, item, multiple) {
  if (!item.getWorn() && item.isAtLoc(char.name, world.PARSER)) return true

  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, multiple) + lang.already_wearing, {char:char, garment:item})

  if (item.loc) {
    const holder = w[item.loc];
    if (holder.npc || holder.player) return falsemsg(prefix(item, multiple) + lang.char_has_it, {holder:holder, item:item})
  }
    
  return falsemsg(prefix(item, multiple) + lang.not_carrying, {char:char, item:item})
}

// Item's location is the char and it is worn
cmdRules.isWorn = function(cmd, char, item, multiple) {
  if (item.getWorn() && item.isAtLoc(char.name, world.PARSER)) return true

  if (item.isAtLoc(char.name, world.PARSER)) return falsemsg(prefix(item, multiple) + lang.not_wearing, {char:char, item:item})
...
cmdRules.isHeld = function(cmd, char, item, multiple) {
...
cmdRules.isHere = function(cmd, char, item, multiple) {
  ...
  return falsemsg(prefix(item, multiple) + lang.not_here, {char:char, item:item})
}

// Item's location is the char's location or the char
// or item is reachable, but not held by someone else
cmdRules.isHereNotHeld = function(cmd, char, item, multiple, already) {
  if (item.isAtLoc(char.loc, world.PARSER)) return true

  if (item.loc) {
    const holder = w[item.loc]
    if (already && holder === game.player) return falsemsg(prefix(item, multiple) + lang.already_have, {char:holder, item:item})
    if (holder.npc || holder.player) return falsemsg(prefix(item, multiple) + lang.char_has_it, {holder:holder, item:item})
  }
...
cmdRules.isHereNotHeldAlready = function(cmd, char, item, multiple) {
  return cmdRules.isHereNotHeld(cmd, char, item, multiple, true)
}

  ...
  this.matchItems = function(s) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (cmdParams.special) {
        ...
        score = 2;
        this.tmp.objects.push(scope.map(function(el) { return [el]; }))
      }
...
function createVerb(name, words, held, defmsg) {
  if (words === undefined) words = name.toLowerCase()
  if (defmsg === undefined) defmsg = name + "ing {nm:item:the} is not going to achieve much."
  if (defmsg === true) defmsg = "{pv:item:'be:true} not something you can do that with."
  commands.unshift(new Cmd(name, {
    regex:new RegExp("^(?:" + words + ") (.+)$"),
    objects:[
      {scope:held ? parser.isHeld : parser.isHere},
    ],
    defmsg:defmsg
  }))
}

...
// In the command, you can have `msg(prefix(item, multiple) + "Done"), and it is sorted.
function prefix(item, multiple) {
  if (!multiple) { return ""; }
...
util.defaultExitUse = function(char, dir, exit) {
  if (!exit) exit = this
  if (char.testMoveRestrictions && !char.testMoveRestrictions(exit)) return false
  ...
  for (const el of char.testCarryRestrictionsList) {
    if (!w[el].testCarryRestrictions(char, exit.name, dir)) return false
  }

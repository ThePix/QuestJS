...
// It options.multi is true, the object name is prefixed.
...
function scopeHereListed() {
  ...
  for (let key in w) {
    const o = w[key]
    if (!o.player && o.isAtLoc(game.player.loc, world.LOOK) && world.ifNotDark(o)) {
      list.push(o);
    }
  }
  return list;
...
function scopeHereParser() {
  ...
  for (let key in w) {
    const o = w[key]
    if (!o.player && o.isAtLoc(game.player.loc, world.PARSER)) {
      list.push(o);
    }
  }
  return list;
...
function scopeAllNpcHere(ignoreDark) {
  ...
  for (let key in w) {
    const o = w[key]
    if (o.isAtLoc(game.player.loc, world.PARSER) && o.npc && (world.ifNotDark(o) || ignoreDark)) {
      list.push(o);
    }
  }
  return list;
...
util.changePOV = function(char, pronouns) {
  ...
  if (game.player) {
    ...
    game.player.pronouns = game.player.npcPronouns
    game.player.regex = new RegExp('^(' + (char.npcAlias ? char.npcAlias : char.alias) + ')$')
  }
  char.player = true
...
util.getObj = function(name) {
  if (typeof name === 'string') {
    const room = w[name]
    if (room === undefined) throw "Failed to find room: " + name + "."
    return room
  }
  else {
...
util.defaultExitUse = function(char, dir, exit) {
  if (!exit) exit = this
  if (char.testMoveRestrictions && !char.testMoveRestrictions()) return false
  ...
  if (exit.isLocked()) {
    if (exit.lockedmsg) {
      ...
      msg(lang.locked_exit, {char:char, exit:exit})
    }
    return false
  }
  ...
  if (exit.restrictions) {
    for (const el of exit.restrictions) {
      if (!el.condition(char, dir)) return falsemsg(el.msg, {char:char, dir:dir})
    }
  }
  ...
  return this.simpleUse ? this.simpleUse(char, dir, exit) : util.defaultSimpleExitUse(char, dir, exit)
}



util.defaultSimpleExitUse = function(char, dir, exit) {
  char.msg(lang.stop_posture(char))
  if (exit.msg) {
    printOrRun(char, exit, "msg")
  }
  else {
    char.msg(lang.go_successful, {char:char, dir:dir})
  }
  char.moveChar(exit.name, false)
  return true  
}
...
util.useWithDoor = function(char, dir) {
  ...
  if (!obj.closed) {
    char.moveChar(this.name, dir)
    return true
  }
  if (!obj.locked) {
    obj.closed = false
    msg(lang.open_and_enter, tpParams)
    char.moveChar(this.name, false)
    return true
  }

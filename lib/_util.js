...
random.dice = function(s) {
  ...
  for (let dice of s.split("+")) {
    ...
    total += negative * random.diceSubPart(dice)
  }
  return total
}
...
random.diceSubPart = function(dice) {
  // just a number
  ...
  if (match[3] === 'r') {
    ...
    for (let i = 0; i < rolls.length; i++) {
      if (rolls[i] <= rerollOn) rolls[i] = random.diceRoll(sides)
    }
  }
  if (match[3] === 'R') {
    const rerollOn = match[4] ? parseInt(match[4]) : 1
    for (let i = 0; i < rolls.length; i++) {
      while (rolls[i] <= rerollOn) rolls[i] = random.diceRoll(sides)
    }
  }
  if (match[3] === 'C') {
...
// a number, the numberof sides for the dice, so returns a random numbger from 1 to that number
// or a set of numbers separated by colon, so returns one picked at random
random.diceRoll = function(s) {
...
function respond(params, list) {
  ...
  if (!response) {
    if (params.extraScript) params.extraScript(params)
  ...
  if (params.extraScript) params.extraScript.bind(params.char)(params, response)

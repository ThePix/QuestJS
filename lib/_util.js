...
function c(str) {
  if (saveLoad.transcript) {
   ...
   return 'This function is designed for adding comments whilst recording a transcript. You are not currently recording a transcript.'
  }
}
...
// Most exits have no alias, and using dir is fine. For climb_up and climb_down, you will want the alias.
function dir(exit) { return exit.alias ? exit.alias : exit.dir }
...
random.seed = function(str) {
  if (str) {
    let seed = random.cyrb128(str)
    random.rand = random.sfc32(seed[0], seed[1], seed[2], seed[3])
    random.int = function(n1, n2) {
      if (this.buffer.length > 0) return this.buffer.shift()
      if (n2 === undefined) {
        n2 = n1;
      ...
      return Math.floor(random.rand() * (n2 - n1 + 1)) + n1;
    }
  }
  else {
    random.int = function(n1, n2) {
...
// Two seeding functions adapted from here:
// https://stackoverflow.com/questions/521295/seeding-the-random-number-generator-in-javascript

random.cyrb128 = function(str) {
...
function warningFor(o, msg) { log('Warning for ' + o.name + ': ' + msg) }
...
/*
parser 201
    if (settings.playMode === 'dev' && !test.testing) log("Command was [" + inputText + "]")

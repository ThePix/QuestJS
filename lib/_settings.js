...
const settings = {
  ...
  writeScript:function(folder) {
    ...
    if (settings.tests && settings.playMode === 'dev') {
      html += '<script src="lib/test-lib.js"></scr' + "ipt>"
      html += '<script src="' + settings.folder + 'tests.js"></scr' + "ipt>"
    }
    html += '<script src="' + (folder ? 'lang/' : '' ) + settings.lang + '.js"></scr' + "ipt>"
    if (settings.customExits) {
      html += '<script src="' + settings.folder + settings.customExits + '.js"></scr' + "ipt>"
    }
    for (let file of settings.libraries) {
      html += '<script src="' + (folder ? 'lib/' : '' ) + file + '.js"></scr' + "ipt>"
    }
    for (let lib of settings.customLibraries) {
      for (let file of lib.files) {
        html += '<script src="' + (folder ? lib.folder + '/' : '') + file + '.js"></scr' + "ipt>"
      }
    }
    for (let file of settings.files) {
      html += '<script src="' + settings.folder + file + '.js"></scr' + "ipt>"
    }
    document.write(html)
  }
}

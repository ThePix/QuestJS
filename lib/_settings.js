...
const settings = {
  ...
  isHere:function(item) {
    //return item.isAtLoc(player.loc, settings.sceneryInSidePane ? world.SIDE_PANE_WITH_SCENERY : world.SIDE_PANE) && world.ifNotDark(item);
    return item.isAtLoc(player.loc, world.SIDE_PANE) && world.ifNotDark(item);
  },

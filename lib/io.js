...
function msgDiv(arr, params, cssClass) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const processed = processText(arr[i], params).trim();
...
function msgTable(arr, headings, params) {
  ...
  if (headings) {
    ...
    for (let j = 0; j < headings.length; j++) {
      const processed = processText(headings[j], params).trim();
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < arr[i].length; j++) {
      const processed = processText(arr[i][j], params).trim();
...
const typeWriter = {
  ...
  print:function() {
    ...
    typeWriter.pos++;
    //if (typeWriter.pos === typeWriter.line.length) typeWriter.pos = 0;
    //setTimeout(typeWriter.print, typeWriter.speed);
    if (typeWriter.pos > typeWriter.line.length) {
      typeWriter.pos = 0;
      setTimeout(typeWriter.print, 5 * TYPEWRITER);
    }
    else {
      setTimeout(typeWriter.print, TYPEWRITER);
    }
  }
}
...
io.appendItem = function(item, htmlDiv, loc, isSubItem) {
  ...
  if (verbs === undefined) { errormsg("No verbs for " + item.name); console.log(item); }

...
// Puts a pause instruction into the queue; output will pause when the queue gets to that point.
io.pause = function() {
  io.outputQueue.push({action:'pause'})
}
...
io.outputFromQueue = function() {
  if (io.outputQueue.length === 0 || io.outputSuspended) return
...
io.unscambleEffect = function(data) {
  console.log("in effect")
  // Set it the system
  if (!data.count) {
    ...
    for (let i = 0; i < data.text.length; i++) {
      if (data.text.charAt(i) === ' ') {
        ...
        data.scrambled += io.unscamblePick()
        data.mask += 'x'
        data.count++
      }
    }
...
io.unscambleScramble = function(data) {
  let s = ''
  for (let i = 0; i < data.text.length; i++) {
    s += (data.mask.charAt(i) === ' ' ? data.text.charAt(i) : io.unscamblePick())
  }
  return s
}

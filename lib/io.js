...
function msgList(arr, params, cssClass, ordered) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (cssClass === undefined) {
      io.outputRaw('<li id="n' + io.nextid + '">' + processed + "</li>");
    }
    else {
      io.outputRaw('<li id="n' + io.nextid + '" class="' + cssClass + '">' + processed + "</li>");
    }
    io.nextid++;
...
function msgTable(arr, params, cssClass) {
  if (test.testing) return
  io.outputRaw(cssClass ? '<table>' : '<table class="' + cssClass + '">');
  for (let i = 0; i < arr.length; i++) {
    io.outputRaw('<tr>');
    for (let j = 0; i < arr[i].length; j++) {
      const s = arr[i][j];
      io.outputRaw("<td>" + s + "</td>");
      if (game.spoken) io.speak(s);
      if (game.transcript) game.scriptAppend("|" + s);
    }
    io.outputRaw("</tr>");
  }
  io.outputRaw('</table>');

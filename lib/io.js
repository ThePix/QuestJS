...
const io = {
  ...
  scriptShow:function(opts) {
    ...
    let html = '';
    html += '<!DOCTYPE html><html lang="en-US"><head><meta charset="utf-8"/><title>Quest 6 Transcript for '
    html += settings.title
    ...
    //const newWindow = window.open("data:text/html," + encodeURIComponent(html), "_blank", "width=200,height=100");
    //myWindow.focus();    
...
io.createPanes = function() {
  ...
  document.writeln('<div id="panes" class="side-panes side-panes' + settings.panes + '">');
...
io.writeExit = function(n) {
  document.writeln('<td class="compass-button">');
  document.writeln('<span class="compass-button" id="exit' + lang.exit_list[n].name + '" onclick="io.clickExit(\'' + lang.exit_list[n].name + '\')">' + lang.exit_list[n].abbrev + '</span>');
  document.writeln('</td>');
};
  ...
  $('#textbox').keydown(function(event){
    const keycode = (event.keyCode ? event.keyCode : event.which);
      ...
      if(keycode === 96 && settings.debug){
        parser.parse(settings.shortcutCommand);

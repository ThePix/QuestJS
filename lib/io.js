...
io.writeExit = function(n) {
  ...
  document.writeln('<span class="compassbutton" id="exit' + exit_list[n].name + '" onclick="io.clickExit(\'' + exit_list[n].name + '\')">' + exit_list[n].abbrev + '</span>');
  document.writeln('</td>');
};
...
io.keycodes = {
  97:exit_list[10].name,
  98:exit_list[11].name,
  99:exit_list[12].name,
  100:exit_list[5].name,
  101:exit_list[6].name,
  102:exit_list[7].name,
  103:exit_list[0].name,
  104:exit_list[1].name,
  105:exit_list[2].name,  
};
...
$(document).ready(function() {
  $('#textbox').keydown(function(event){
    ...
    if(keycode === 37){
      // left arrow
      if (event.ctrlKey) {
        io.msgInputText(exit_list[5].name);
        parser.parse(exit_list[5].name);
        $('#textbox').val('');
      }
    }
    if(keycode === 38){
      // up arrow
      if (event.ctrlKey) {
        io.msgInputText(exit_list[1].name);
        parser.parse(exit_list[1].name);
        $('#textbox').val('');
      }
      else {
    ...
    if(keycode === 39){
      // right arrow
      if (event.ctrlKey) {
        io.msgInputText(exit_list[7].name);
        parser.parse(exit_list[7].name);
        $('#textbox').val('');
      }
    }
    if(keycode === 40){
      // down arrow
      if (event.ctrlKey) {
        io.msgInputText(exit_list[11].name);
        parser.parse(exit_list[11].name);
        $('#textbox').val('');
      }
      else {
        io.savedCommandsPos += 1;

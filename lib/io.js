...
io.outputFromQueue = function() {
  ...
  if (data.action === 'output') {
    ...
    if (game.transcript) game.scriptAppend(data);
    io.outputFromQueue()
  }
...
io.createPanes = function() {
  ...
  if (settings.compass) {
    if (settings.divider) document.writeln('<img src="' + settings.imagesFolder + '/' + settings.divider + '" />');
  ...
  document.writeln('<div class="paneDivFinished">');
...
io.msgInputText = function(s) {
  ...
  if (game.transcript) game.scriptAppend({cssClass:'input', text:s});
};

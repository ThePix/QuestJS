...
function msg(s, params, cssClass) {
  ...
  var lines = processed.split(SPLIT_LINES_ON);
  for (var i = 0; i < lines.length; i++) {
    if (cssClass === undefined) {
      $("#output").append('<p id="n' + io.nextid + '">' + lines[i] + "</p>");
    }
    else {
      $("#output").append('<p id="n' + io.nextid + '" class="' + cssClass + '">' + lines[i] + "</p>");
    }
    io.nextid++;
...
function errormsg(s) {
  ...
  $("#output").append('<p id="n' + io.nextid + '" class="error">' + s + '</p>');
  io.nextid++;
...
function parsermsg(s) {
  ...
  $("#output").append('<p id="n' + io.nextid + '" class="parser">' + s + '</p>');
  io.nextid++;
...
function printOrRun(char, item, attname, isMultiple) {
  ...
  if (Array.isArray(attname)) {
    ...
    var res = item[attname](isMultiple, char);
    return res;
  }
  else {
...
function showDropDown(title, options, fn) {
  ...
  io.disable();
  msg(title, {}, 'menutitle');
  s = '<select id="menu-select" onchange=\"io.menuResponse($(\'#menu-select\').find(\':selected\').val())\">';
...
function endTurnUI(update) {
  if (PANES !== 'None' && update) {
    ...
    for (var i = 0; i < EXITS.length; i++) {
      if (game.room.hasExit(EXITS[i].name) || EXITS[i].nocmd) {

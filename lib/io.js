...
function printOrRun(item, attname, isMultiple, npc) {
  if (Array.isArray(item[attname])) {
    debugmsg(0, "Array: " + attname);
    var flag = true;
    for (var i = 0; i < item[attname].length; i++) {
      flag = printOrRun(item, item[attname][i], isMultiple, npc) && flag;
    }
    return flag;
  }
  if (Array.isArray(attname)) {
    ...
    for (var i = 0; i < attname.length; i++) {
      flag = printOrRun(item, attname[i], isMultiple, npc) && flag;
    }
    return flag;
  }
  else if (!item[attname]) {
    if (typeof attname === "function"){
      return attname(item, isMultiple, npc);
    }
    else {
      msg(attname);
      return true;
    ...
    return item[attname](isMultiple, npc);
  }
  else {

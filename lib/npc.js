...
const NPC = function(isFemale) {
  ...
  res.getWearingVisible = function() {
    return this.getContents(false).filter(function(el) { return el.worn; });
  };
  ...
  res.byname = function(options) {
    ...
    if (this.getHolding().length === 0) {
      s += " (wearing " + formatList(this.getWearing(), {article:INDEFINITE, lastJoiner:LIST_AND, modified:true, nothing:LIST_NOTHING, loc:this.name}) + ")";
      if (options && options.possessive) s += "'s";
      return s;
    }
    if (this.getWearingVisible().length === 0) {
      s += " (holding " + formatList(this.getHolding(), {article:INDEFINITE, lastJoiner:LIST_AND, modified:true, nothing:LIST_NOTHING, loc:this.name}) + ")";
      if (options && options.possessive) s += "'s";
      return s;
    }

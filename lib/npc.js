...
const NPC = function(isFemale) {
  ...
  res.topics = function() {
    ...
    for (let action of ['ask', 'tell']) {
      ...
      const arr2 = []
      for (let res of arr) {
        if (res.silent && !this.mentionedTopics.includes(res.name)) continue
        arr2.push(res.name)
      }
      if (arr2.length !== 0) {
...
const CONSULTABLE = function() {
  ...
  res.asktellabout = function(text1, text2, intro, list, action) {
    ...
    if (GIVE_PLAYER_ASK_TELL_MSG) msg(intro(this, text1, text2));
  ...
  res.asktelldone = function(params, response) {
    ...
    if (response.mentions) {
      for (let s of response.mentions) {
        if (!params.actor.mentionedTopics.includes(s)) params.actor.mentionedTopics.push(s)
      }
    }
    params.actor.pause();
  } 

...
const NPC = function(isFemale) {
  ...
  res.moveWithDescription = function(dest) {
    ...
    var s = nounVerb(this, "leave", true) + " " + w[this.loc].byname({article:DEFINITE});
    var exit = w[this.loc].findExit(dest);
    if (exit) s += ", heading " + exit.dir;
    s += ".";
    agenda.giveText(this, [s]);
    this.loc = dest;
    for (var i = 0; i < this.followers.length; i++) this.followers[i].loc = dest
    s = nounVerb(this, "enter", true) + " " + w[this.loc].byname({article:DEFINITE}) + ".";
    agenda.giveText(this, [s]);
  };
...
const agenda = {
  ...
  giveText:function(npc, arr) {
    ...
    if (w[game.player.loc].canViewLocs && w[game.player.loc].canViewLocs.includes(npc.loc)) {
      msg(w[game.player.loc].canViewMsg);
      flag = true;
    }
    if (npc.here() || flag) {

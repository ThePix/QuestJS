...
const NPC = function(isFemale) {
  ...
  res.suspended = false;
  res.getVerbs = () => no_talk_to ? [verbs.lookat] : [verbs.lookat, verbs.talkto];
  ...
  res.topics = function() {
    ...
    let flag = false;
    let arr = [];
    for (let opt of this.askOptions) {
      if (opt.name !== undefined) arr.push(opt.name);
    }
    if (arr.length !== 0) {
    ...
    arr = [];
    for (let opt of this.tellOptions) {
      if (opt.name !== undefined) arr.push(opt.name);
    }
...
const npc_utilities = {
  talkto:function() {
    ...
    if (topics.length === 0) return failedmsg(no_topics(game.player, this));
...
const CONSULTABLE = function() {
  ...
  res.askabout = function(text) { return this.asktellabout(text, ask_about_intro, this.askOptions, "ask"); },
  res.tellabout = function(text) { return this.asktellabout(text, tell_about_intro, this.tellOptions, "tell"); },

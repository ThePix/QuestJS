...
const NPC = function(isFemale) {
  ...
  res.byname = function(options) {
    if (options.group && this.followers.length > 0) {
      ...
      var s = formatList(this.followers, options);
    ...
    if (this.getWearingVisible().length === 0 || !options.modified) {
  ...
  res.turnscript = function() {
    ...
    // for (var i = 0; i < w.Arthur.agenda.length; i++) {
      // debugmsg(">>>" + w.Arthur.agenda[i]);
    // }
    var arr = this.agenda[0].split(":");
    var fn = arr.shift();
  ...
  res.moveWithDescription = function(dest) {
    ...
    debugmsg("move to " + dest);
    if (this.here()) msg(nounVerb(this, "leave", true) + " " + w[this.loc].byname({article:DEFINITE}) + ".");
    this.loc = dest;
    for (var i = 0; i < this.followers.length; i++) this.followers[i].loc = dest
    if (this.here()) msg(nounVerb(this, "enter", true) + " " + w[this.loc].byname({article:DEFINITE}) + ".");
  };
  ...
  res.askabout = function(text) {
    ...
    msg(CMD_ASK_ABOUT_INTRO(this, text));
  ...
  res.tellabout = function(text) {
    ...
    msg(CMD_TELL_ABOUT_INTRO(this, text));
  ...
  res.talkto = function() {
    ...
    var topics = this.getTopics(this);
    
    topics.push(NEVER_MIND);
    if (USE_DROPDOWN_FOR_CONV) {
      showDropDown(CMD_SPEAK_TO_MENU_TITLE(this), topics, function(result) {
      ...
      showMenu(CMD_SPEAK_TO_MENU_TITLE(this), topics, function(result) {
        if (result !== NEVER_MIND) {
          result.runscript();
        }
      });
...
const agenda = {
  ...
  walkRandom:function(npc, arr) {
    ...
    var exit = w[npc.loc].getRandomExit(true);
    var dest = w[npc.loc][exit].name;
    debugmsg("dest:" + dest);
    if (!w[dest]) errormsg("Location '" + dest + "' not recognised in the agenda of " + npc.name);
    npc.moveWithDescription(dest);
    this.giveText(npc, arr);
  ...
  walkTo:function(npc, arr) {
    ...
    debugmsg("dest:" + dest);
    if (dest === "player") dest = game.player.loc;
    if (!w[dest].room) dest = w[dest].loc;
    debugmsg("dest:" + dest);
    if (!w[dest]) errormsg("Location '" + dest + "' not recognised in the agenda of " + npc.name);
    if (npc.isAtLoc(dest)) {
      this.giveText(npc, arr);

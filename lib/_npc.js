...
const NPC = function(isFemale) {
  ...
  /*
  This will be called by agenda actions and NPC moving (and maybe reactions) to decide if the NPC's action can be seen.
  This includes when in the same room as well as when seen through a window, video link or other end of the corridor
  Ideally some will be handled via RoomSet
  
  Needs a mechanism to modify the text so it is clear when this is seen at a distance
  This might be a prefix, or it could select from options, or using a template, or something else
    
  The text can be in the agenda item, or the reaction, or the moving text of the exit, or the moving text for the NPC (is that a thing?)
  
  Needs to default to the plain message - or telling the caller to use that.
  The extra could be added at the begining, or via the TP
  ...
  */
  ...
  // do we need a prefix?
  // request will be the string "moving" if this is the NPC moving from one location to another,
  // or an array of parameters if this comes from the agenda "text" option.
  // Should return true or a string if it wants the caller to print a message, false otherwise
  // Note that false could mean "do not print message because I have done that"
  res.inSight = function(room, request, otherRoom) {
    if (!this.loc) return false
    if (request === 'moving' && otherRoom.name === player.loc) return false
    if (!room) room = w[this.loc]
    if (player.loc === room.name) return true
    if (room.visibleFrom === undefined) return false
    if (typeof room.visibleFrom === 'function') return room.visibleFrom({seenFrom:currentLocation, request:request, seenBy:player})
...
const createMale = function(name, data) { return createItem(name, NPC(), data) }
const createFemale = function(name, data) { return createItem(name, NPC(true), data) }
...
const npc_utilities = {
  ...
  talkto:function() {
    ...
    if (topics.length === 0) return failedmsg(this.no_topics ? this.no_topics : lang.no_topics_now, {char:player, item:this})
...
const agenda = {
  ...
  text:function(npc, arr) {
    ...
    const flag = npc.inSight(w[npc.loc], 'text')
    if (!flag) return true
  ...
  // otherwise this will be skipped
  // Used by several other functions, so this applies to them too
  altText:function(npc, arr) {
    const flag = npc.inSight(w[npc.loc], arr)
    if (flag) {
      msg(arr.join(':'), {npc:npc, mod:typeof flag === 'string' ? ' ' + flag + ' ' : ''})
    }
  ...
  // Move to the given location, then print the rest of the array as text.
  // There must be an exit from the current room to that room.
  head:function(npc, arr) {
    let dir = arr.shift()
    for (let exit of lang.exit_list) {
      if (exit.abbrev.toLowerCase() !== dir) dir = exit.name
    }

    if (!w[npc.loc][dir]) return errormsg("Direction '" + dir + "' not found in the agenda of " + npc.name + " for room " + w[npc.loc].name)
    const exit = w[npc.loc][dir]
  log(exit)
    npc.movingMsg(exit) 
    npc.moveChar(exit)
    this.text(npc, arr)

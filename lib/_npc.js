...
const TOPIC = function(fromStart) {
  const res = {
    ...
    runscript:function() {
      ...
      if (obj === undefined) errormsg("No conversing NPC called " + game.player.conversingWithNpc + " found.");
      obj.pause();
      this.hideTopic = this.hideAfter;
      if (typeof this.script !== "function") errormsg("script for topic " + this.name + " is not a function.");
      this.script(obj);
      if (typeof this.nowShow === "string") errormsg("nowShow for topic " + this.name + " is a string.");
      for (let s of this.nowShow) {
        const o = w[s];
        if (o === undefined) errormsg("No topic called " + s + " found.");
        o.show();
      }

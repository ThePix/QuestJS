  ...
  res.doReactions = function() {
    ...
    const params = {
      ...
      extraScript:function(params, response) {
        if (!response) return
        params.actor.reactionFlags.push(response.name)
        if (!response.noPause) params.actor.pause()
        if (response.override) params.actor.reactionFlags = params.actor.reactionFlags.concat(response.override)
      },
      extraTest:function(params, response, ) {
        return params.actor.reactionFlags.includes(response.name)
      },
    }
